...
def test_team_without_mode(capsys):
    ...
    tasks = [
        ...
        Task(
            description="Write a blog post about quantum computing for general audience"
        ),
    ]
    ...
    assert agent_started_count == 5, (
        f"Expected 5 agents executed (2 user-defined + 3 coordinators), got {agent_started_count}"
    )
...
def test_team_with_mode(capsys):
    ...
    team = Team(
        agents=[researcher, analyst, writer], mode="coordinate", model="openai/gpt-4o"
    )
    ...
    agent_started_count = output.count("Agent Started")
    assert agent_started_count == 4, (
        f"Expected 4 agents executed (1 coordinator + 3 user-defined), got {agent_started_count}"
    )
...
def test_team_with_response_format(capsys):
    ...
    team = Team(agents=[analyst, summarizer], response_format=AnalysisResult)
...
def test_agent_name_verification_with_mock():
    ...
    assert writer.name == "Writer", (
        f"Expected writer name to be 'Writer', got '{writer.name}'"
    )
    assert editor.name == "Editor", (
        f"Expected editor name to be 'Editor', got '{editor.name}'"
    )
    ...
    assert team.agents[0].name == "Writer", (
        f"Expected first agent to be 'Writer', got '{team.agents[0].name}'"
    )
    assert team.agents[1].name == "Editor", (
        f"Expected second agent to be 'Editor', got '{team.agents[1].name}'"
    )

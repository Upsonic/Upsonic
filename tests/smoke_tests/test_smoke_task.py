...
class TestUpsonicBasicFlow:
    ...
    async def test_task_creation(self):
        ...
        task = Task(
            "List exactly 3 prime numbers between 10 and 30, then calculate their sum."
        )
...
class TestTaskWithTools:
    ...
    async def test_task_with_tools(self, capsys):
        ...
        task = Task(description="Turkeys capital city", tools=[web_search])
...
class TestContextChain:
    ...
    async def test_task_with_context(self, capsys):
        ...
        task3 = Task(
            description="What is the second biggest city in that country?",
            context=[task1, task2],
        )
    ...
    async def test_task_complex_chain(self, capsys):
        ...
        task1 = Task(
            description="which company is the best for building AI applications?"
        )
...
def test_tool_confirmation(monkeypatch, setup_agent_and_task):
    ...
    assert "⚠️ Confirmation Required" in result, (
        "Expected confirmation indicator missing"
    )

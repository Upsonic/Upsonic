...
from upsonic.safety_engine.policies.adult_content_policies import (
...
def test_agent_memory_and_task_chaining():
    ...
    agent = Agent(memory=memory, feed_tool_call_results=True)
...
def test_agent_crypto_policy():
    ...
    agent = Agent(user_policy=CryptoBlockPolicy, reliability_layer=reliability_layer)
...
def test_agent_adult_content_policy():
    ...
    agent = Agent(
        agent_policy=AdultContentBlockPolicy, reliability_layer=reliability_layer
    )

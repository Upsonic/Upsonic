...
class TestTaskStringContextHandling:
    ...
    def test_agent_can_access_single_string_context(self, mock_infer_model):
        ...
        mock_response = ModelResponse(
            parts=[
                TextPart(
                    content="Found various resources in New York including libraries, community centers, and business districts."
                )
            ],
    ...
    def test_agent_can_access_multiple_string_contexts(self, mock_infer_model):
        ...
        mock_response = ModelResponse(
            parts=[
                TextPart(
                    content="Comprehensive analysis of London's technology sector in 2024 shows significant growth and innovation opportunities."
                )
            ],
            model_name="test-model",
            timestamp="2024-01-01T00:00:00Z",
    ...
    def test_agent_context_integration_simulation(self, mock_infer_model):
        ...
        mock_response = ModelResponse(
            parts=[
                TextPart(
                    content="Best restaurants in Tokyo include traditional sushi bars, ramen shops, and modern fusion cuisine establishments."
                )
            ],
            model_name="test-model",
            timestamp="2024-01-01T00:00:00Z",
    ...
    def test_context_with_non_string_values(self, mock_infer_model):
        ...
        mock_response = ModelResponse(
            parts=[
                TextPart(
                    content="Handled mixed context including valid strings, numbers, and null values appropriately."
                )
            ],
            model_name="test-model",
            timestamp="2024-01-01T00:00:00Z",

...
class TestMemoryInitialization:
    ...
    def test_memory_init_with_session_id(self):
        ...
        storage = MockStorage()
        memory = Memory(
            storage=storage, session_id="test-session", full_session_memory=True
        )
    ...
    def test_memory_init_with_user_id(self):
        ...
        storage = MockStorage()
        memory = Memory(storage=storage, user_id="test-user", user_analysis_memory=True)
    ...
    def test_memory_init_session_id_required_error(self):
        ...
        with pytest.raises(ValueError, match="session_id.*required"):
            Memory(storage=storage, full_session_memory=True)
    ...
    def test_memory_init_user_id_required_error(self):
        ...
        with pytest.raises(ValueError, match="user_id.*required"):
            Memory(storage=storage, user_analysis_memory=True)
    ...
    def test_memory_init_with_custom_schema(self):
        ...
        memory = Memory(storage=storage, user_profile_schema=CustomSchema)
    ...
    def test_memory_init_dynamic_profile(self):
        ...
        memory = Memory(
            storage=storage, user_profile_schema=CustomSchema, dynamic_user_profile=True
        )
    ...
    def test_memory_init_with_model(self):
        ...
        memory = Memory(storage=storage, model=model)
...
class TestMemoryPrepareInputs:
    ...
    async def test_prepare_inputs_with_user_profile(self, storage):
        ...
        assert (
            inputs["system_prompt_injection"]
            == "<UserProfile>\n- name: John\n- age: 30\n</UserProfile>"
        )
    ...
    async def test_prepare_inputs_with_session_summary(self, storage):
        ...
        memory = Memory(storage=storage, session_id="test-session", summary_memory=True)
    ...
    async def test_prepare_inputs_with_chat_history(self, storage):
        ...
        chat_history = [
            ...
            {
                "kind": "response",
                "parts": [{"part_kind": "text", "content": "Assistant response"}],
            },
        ]

        session = InteractionSession(
            session_id=SessionId("test-session"), chat_history=chat_history
        )
        storage._data[(SessionId("test-session"), InteractionSession)] = session
    ...
    async def test_prepare_inputs_with_tool_call_filtering(self, storage):
        ...
        chat_history = [
            {"parts": [{"part_kind": "user-prompt", "content": "User message"}]},
            {
                "parts": [
...
class TestMemoryUpdateMemories:
    ...
    async def test_update_memories_with_session(self, storage, mock_run_result):
        ...
        session = await storage.read_async(
            SessionId("test-session"), InteractionSession
        )
    ...
    @pytest.mark.asyncio
    async def test_update_memories_user_memory_mode_replace(
        self, storage, mock_run_result
    ):
        """Test update_memories_after_task with replace mode."""
        # Create existing profile
        existing_profile = UserProfile(
            user_id=UserId("test-user"), profile_data={"old_key": "old_value"}
        )
    ...
    @pytest.mark.asyncio
    async def test_update_memories_user_memory_mode_update(
        self, storage, mock_run_result
    ):
        ...
        messages = [
            ModelRequest(
                parts=[
                    ...
                    UserPromptPart(content="Message 1"),
                ]
            ),
            ModelResponse(parts=[TextPart(content="Response 1")]),
            ModelRequest(parts=[UserPromptPart(content="Message 2")]),
...
class TestMemoryUserProfileAnalysis:
    ...
    async def test_analyze_interaction_for_traits_with_prompts(self, memory):
        ...
        session = InteractionSession(
            session_id=SessionId("test-session"),
            chat_history=[
                ModelRequest(parts=[UserPromptPart(content="I love programming")])
            ],
        )
...
class TestMemoryDynamicProfile:
    ...
    async def test_dynamic_profile_schema_generation(self, dynamic_memory):
        ...
        with patch("upsonic.agent.agent.Agent") as mock_agent_class:
            ...
            schema_response = ProposedSchema(
                fields=[
                    FieldDefinition(name="profession", description="User's profession"),
                    FieldDefinition(
                        name="experience_level", description="Years of experience"
                    ),
                ]
            )
            ...
            traits = await dynamic_memory._analyze_interaction_for_traits(
                {}, mock_result
            )
...
class TestMemoryIntegration:
    ...
    async def test_full_memory_workflow(self, full_memory):
        ...
        session = await full_memory.storage.read_async(
            SessionId("integration-session"), InteractionSession
        )
        assert session is not None
        assert len(session.chat_history) == 2
        ...
        profile = await full_memory.storage.read_async(
            UserId("integration-user"), UserProfile
        )
        assert profile is not None
    ...
    async def test_memory_with_multiple_sessions(self, storage):
        ...
        memory1 = Memory(
            storage=storage, session_id="session-1", full_session_memory=True
        )

        memory2 = Memory(
            storage=storage, session_id="session-2", full_session_memory=True
        )

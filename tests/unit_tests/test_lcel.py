...
"""
    ...
    assert isinstance(chain.steps[0], RunnableParallel), (
        "First step should be RunnableParallel"
    )
...
def test_runnablepassthrough():
    ...
    prompt = ChatPromptTemplate.from_messages(
        [("system", "You are a helpful assistant."), ("human", "{equation_statement}")]
    )
...
def test_runnablebranch():
    ...
    langchain_chain = (
        ChatPromptTemplate.from_template("LangChain expert: {question}") | model
    )

    anthropic_chain = (
        ChatPromptTemplate.from_template("Anthropic expert: {question}") | model
    )

    general_chain = ChatPromptTemplate.from_template("General: {question}") | model
...
def test_dynamic_chain_construction():
    ...
    assert (
        result_with_history != result_without_history or True
...
def test_complex_nested_parallel():
    ...
    assert result["outer1"]["inner1"] == 10, (
        f"inner1 should be 10, got {result['outer1']['inner1']}"
    )
    assert result["outer1"]["inner2"] == 15, (
        f"inner2 should be 15, got {result['outer1']['inner2']}"
    )

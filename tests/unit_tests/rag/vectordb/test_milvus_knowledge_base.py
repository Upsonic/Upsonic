...
class TestMilvusKnowledgeBaseIntegration:
    ...
    @pytest.fixture
    def knowledge_base(
        self, milvus_provider, mock_embedding_provider, mock_chunker, mock_loader
    ):
    ...
    @pytest.mark.skipif(
        sys.platform == "win32", reason="milvus-lite not available on Windows"
    )
    ...
    async def test_knowledge_base_setup_with_milvus(self, knowledge_base):
        ...
        knowledge_base.embedding_provider.embed_documents = AsyncMock(
    ...
    async def test_knowledge_base_query_with_milvus(self, knowledge_base):
        ...
        knowledge_base.vectordb.collection_exists = Mock(return_value=False)
        knowledge_base.vectordb.is_ready = Mock(return_value=True)
        knowledge_base.vectordb.search = Mock(
            return_value=[
        ...
        knowledge_base.embedding_provider.embed_query = AsyncMock(
            return_value=[0.15] * 384
        )
    ...
    def test_milvus_hybrid_search(self, milvus_provider):
        ...
        milvus_provider.hybrid_search = Mock(
            return_value=[
                create_mock_vector_search_result("id1", 0.9, "Test result 1"),
                create_mock_vector_search_result("id2", 0.8, "Test result 2"),
            ]
        ...
        milvus_provider.hybrid_search.assert_called_once_with(
            query_vector, query_text, top_k=2
        )
        assert len(results) == 2
    ...
    def test_milvus_full_text_search(self, milvus_provider):
        ...
        milvus_provider.full_text_search = Mock(
            return_value=[
                create_mock_vector_search_result("id1", 0.9, "Test result 1"),
                create_mock_vector_search_result("id2", 0.8, "Test result 2"),
            ]
    ...
    def test_milvus_sparse_vectors(self, milvus_provider):
        ...
        milvus_provider.upsert(
            vectors, payloads, ids, chunks, sparse_vectors=sparse_vectors
        )
    ...
    def test_milvus_filter_operations(self, milvus_provider):
        ...
        milvus_provider.dense_search = Mock(
            return_value=[
                create_mock_vector_search_result("id1", 0.9, "Test result 1"),
        ...
        results = milvus_provider.dense_search(
            query_vector, top_k=5, filter=filter_dict
        )

        # Verify search was called with filter
        milvus_provider.dense_search.assert_called_once_with(
            query_vector, top_k=5, filter=filter_dict
        )
        assert len(results) == 2
    ...
    def test_milvus_payload_indexes(self, milvus_provider):
        ...
        payload_indexes = [
            PayloadIndexConfig(field_name="category", field_schema_type="keyword"),
            PayloadIndexConfig(
        ...
        assert payload_indexes[1].enable_full_text_index
    ...
    def test_milvus_configuration_validation(self):
        ...
        with patch(
            "upsonic.vectordb.providers.milvus.MilvusProvider._validate_config"
        ) as mock_validate:

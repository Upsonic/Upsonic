...
class TestChromaKnowledgeBaseIntegration:
    ...
    @pytest.fixture
    def knowledge_base(
        self, chroma_provider, mock_embedding_provider, mock_chunker, mock_loader
    ):
    ...
    async def test_knowledge_base_query_with_chroma(self, knowledge_base):
        ...
        knowledge_base.vectordb.collection_exists = AsyncMock(return_value=False)
        knowledge_base.vectordb.is_ready = AsyncMock(return_value=True)
        knowledge_base.vectordb.search = AsyncMock(
            return_value=[
    ...
    async def test_chroma_hybrid_search(self, chroma_provider):
        ...
        chroma_provider.dense_search = AsyncMock(
            return_value=[create_mock_vector_search_result("id1", 0.9, "Test result 1")]
        )
        chroma_provider.full_text_search = AsyncMock(
    ...
    def test_chroma_configuration_validation(self):
        ...
        valid_config = ChromaConfig(
            connection=valid_connection, collection_name="test", vector_size=384
        )

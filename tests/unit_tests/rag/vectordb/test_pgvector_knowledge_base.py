...
class TestPgvectorKnowledgeBaseIntegration:
    ...
    @pytest.fixture
    def knowledge_base(
        self, pgvector_provider, mock_embedding_provider, mock_chunker, mock_loader
    ):
    ...
    def test_pgvector_search_operations(self, pgvector_provider):
        ...
        pgvector_provider.connect = Mock()
        pgvector_provider.create_collection = Mock()
        pgvector_provider.dense_search = Mock(
            return_value=[
    ...
    def test_pgvector_hybrid_search(self, pgvector_provider):
        ...
        pgvector_provider.full_text_search = Mock(
        ...
        pgvector_provider.hybrid_search(query_vector, query_text, top_k=2)
    ...
    def test_pgvector_full_text_search(self, pgvector_provider):
        ...
        results = pgvector_provider.full_text_search(
            "chunk", top_k=2, fts_field="chunk"
        )

        # Verify search was called
        pgvector_provider.full_text_search.assert_called_once_with(
            "chunk", top_k=2, fts_field="chunk"
        )
        assert len(results) == 2
        assert all(isinstance(result, VectorSearchResult) for result in results)

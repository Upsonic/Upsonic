...
from upsonic.embeddings.base import (
    EmbeddingProvider,
    EmbeddingConfig,
    EmbeddingMode,
    EmbeddingMetrics,
...
from upsonic.embeddings.azure_openai_provider import (
    AzureOpenAIEmbedding,
    AzureOpenAIEmbeddingConfig,
)
from upsonic.embeddings.bedrock_provider import BedrockEmbedding, BedrockEmbeddingConfig
from upsonic.embeddings.huggingface_provider import (
...
from upsonic.embeddings.factory import (
...
class TestEmbeddingProviders:
    ...
    def test_openai_embedding_creation(self, mock_openai_client):
        ...
        with patch(
            "upsonic.embeddings.openai_provider.AsyncOpenAI",
    ...
    def test_azure_openai_embedding_creation(self, mock_azure_client):
        ...
        with patch(
            "upsonic.embeddings.azure_openai_provider.AsyncAzureOpenAI",
    ...
    def test_gemini_embedding_creation(self, mock_gemini_client):
        ...
        with patch(
            "upsonic.embeddings.gemini_provider.genai.Client",
            return_value=mock_gemini_client,
        ):
            with patch.dict(os.environ, {"GOOGLE_API_KEY": "test-key"}):
                config = GeminiEmbeddingConfig(
                    model_name="gemini-embedding-001", api_key="test-key"
                )
    ...
    def test_huggingface_embedding_creation(self, mock_huggingface_model):
        ...
        with patch(
            "upsonic.embeddings.huggingface_provider.AutoModel.from_pretrained",
            return_value=mock_model,
        ):
            with patch(
                "upsonic.embeddings.huggingface_provider.AutoTokenizer.from_pretrained",
    ...
    def test_fastembed_embedding_creation(self, mock_fastembed_model):
        ...
        with patch(
            "upsonic.embeddings.fastembed_provider.TextEmbedding",
            return_value=mock_fastembed_model,
        ):
            config = FastEmbedConfig(model_name="BAAI/bge-small-en-v1.5")
    ...
    async def test_embedding_caching(self):
        ...
        config = EmbeddingConfig(cache_embeddings=True, model_name="test-model")
...
class TestKnowledgeBaseIntegration:
    ...
    def mock_vectordb_config(self, temp_dir):
        ...
        indexing_config = IndexingConfig(
            index_config=FlatTuningConfig(index_type=IndexType.FLAT), quantization=None
        )

        search_config = SearchConfig(default_top_k=10, default_similarity_threshold=0.7)

        return Config(core=core_config, indexing=indexing_config, search=search_config)
    ...
    def test_knowledge_base_creation(
        self, mock_embedding_provider, mock_vectordb, temp_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_knowledge_base_health_check(
        self, mock_embedding_provider, mock_vectordb, temp_dir
    ):
    ...
    def test_knowledge_base_config_summary(
        self, mock_embedding_provider, mock_vectordb, temp_dir
    ):
...
class TestEmbeddingProviderFactory:
    ...
    def test_create_embedding_provider_fastembed(self):
        ...
        with patch("upsonic.embeddings.fastembed_provider.TextEmbedding"):
            provider = create_embedding_provider(
                "fastembed", model_name="BAAI/bge-small-en-v1.5"
            )
    ...
    def test_create_embedding_provider_ollama(self):
        ...
        with patch("aiohttp.ClientSession"):
            provider = create_embedding_provider(
                "ollama", model_name="nomic-embed-text"
            )
    ...
    def test_create_embedding_provider_gemini(self):
        ...
        with patch.dict(os.environ, {"GOOGLE_API_KEY": "test-key"}):
            with patch("upsonic.embeddings.gemini_provider.genai.Client"):
                provider = create_embedding_provider(
                    "gemini", model_name="gemini-embedding-001"
                )
...
class TestEmbeddingProviderErrorHandling:
    ...
    async def test_embedding_rate_limit_handling(self):
        ...
        with patch.object(
            provider,
            "_embed_batch",
            side_effect=ModelConnectionError("Rate limit exceeded"),
        ):
            with pytest.raises(ModelConnectionError):
                await provider.embed_texts(["test"])
...
class TestEmbeddingProviderIntegration:
    ...
    async def test_embedding_provider_switching(self):
        ...
        provider1 = MockEmbeddingProvider(
            config=EmbeddingConfig(model_name="provider1")
        )
        provider2 = MockEmbeddingProvider(
            config=EmbeddingConfig(model_name="provider2")
        )
    ...
    async def test_embedding_provider_with_different_modes(self):
        ...
        document_embeddings = await provider.embed_texts(
            texts, mode=EmbeddingMode.DOCUMENT
        )
        query_embeddings = await provider.embed_texts(texts, mode=EmbeddingMode.QUERY)
        symmetric_embeddings = await provider.embed_texts(
            texts, mode=EmbeddingMode.SYMMETRIC
        )

        assert (
            len(document_embeddings)
            == len(query_embeddings)
            == len(symmetric_embeddings)
        )

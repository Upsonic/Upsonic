...
class TestPdfPlumberLoaderOCRSafe(unittest.TestCase):
    ...
    def test_ocr_with_tables(self):
        ...
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", extract_tables=True, table_format="markdown"
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_with_crop_box(self):
        ...
        config = PdfPlumberLoaderConfig(extraction_mode="ocr_only", crop_box=crop_box)
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_error_handling(self):
        ...
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", error_handling="warn"
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_with_whitespace_removal(self):
        ...
        pdf_path = self.test_pdfs["text_heavy"]
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", extra_whitespace_removal=True
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_layout_modes(self):
        ...
        config_layout = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", layout_mode="layout"
        )
        ...
        config_simple = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", layout_mode="simple"
        )
    ...
    def test_ocr_text_flow(self):
        ...
        pdf_path = self.test_pdfs["text_heavy"]
        config = PdfPlumberLoaderConfig(extraction_mode="ocr_only", use_text_flow=True)
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_character_margins(self):
        ...
        pdf_path = self.test_pdfs["text_heavy"]
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", char_margin=5.0, line_margin=1.0
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_precision(self):
        ...
        pdf_path = self.test_pdfs["simple_ocr"]
        config = PdfPlumberLoaderConfig(extraction_mode="ocr_only", precision=2.0)
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_keep_blank_chars(self):
        ...
        pdf_path = self.test_pdfs["text_heavy"]
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", keep_blank_chars=True
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_table_settings(self):
        ...
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", table_settings=custom_table_settings
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_skip_empty_content(self):
        ...
        config = PdfPlumberLoaderConfig(
            extraction_mode="ocr_only", skip_empty_content=True
        )
        loader = PdfPlumberLoader(config)
    ...
    def test_ocr_performance_single(self):
        ...
        self.assertLess(
            processing_time, 30
        )  # Should complete within 30 seconds for OCR

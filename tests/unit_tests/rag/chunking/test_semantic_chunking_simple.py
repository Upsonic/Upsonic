...
Reinforcement learning algorithms learn through interaction with an environment. They receive rewards or penalties based on their actions.""",
    ...
    def test_basic_semantic_chunking(self):
        ...
        config = SemanticChunkingConfig(
            embedding_provider=self.embedding_provider, chunk_size=200, chunk_overlap=50
        )
    ...
    def test_single_topic_document(self):
        ...
        config = SemanticChunkingConfig(
            embedding_provider=self.embedding_provider, chunk_size=150, chunk_overlap=30
        )
        chunker = SemanticChunker(config)
    ...
    def test_empty_document(self):
        ...
        empty_doc = Document(
            content="", metadata={"source": "empty"}, document_id=str(uuid.uuid4())
        )
    ...
    def test_batch_processing(self):
        ...
        config = SemanticChunkingConfig(
            embedding_provider=self.embedding_provider, chunk_size=150
        )
        chunker = SemanticChunker(config)
    ...
    def test_async_chunking(self):
        ...
        async def _test():
            config = SemanticChunkingConfig(
                embedding_provider=self.embedding_provider, chunk_size=200
            )
            chunker = SemanticChunker(config)

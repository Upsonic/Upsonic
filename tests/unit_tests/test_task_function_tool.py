...
class AgentToolTestCase(TestCase):
    ...
    def test_agent_tool_function_call(self, mock_infer_model):
        ...
        final_response = ModelResponse(
            parts=[
                TextPart(
                    content=f"The sum of {num_a} and {num_b} is {expected_result}."
                )
            ],
            model_name="test-model",
            timestamp="2024-01-01T00:00:00Z",
        ...
        task = Task(
            f"What is the sum of {num_a} and {num_b}? Use Tool", tools=[tracker]
        )
        ...
        self.assertEqual(
            tracker.call_count, 1, "The tool function was not called exactly once."
        )
        self.assertEqual(
            tracker.called_with,
            (num_a, num_b),
            f"Function was called with wrong arguments: {tracker.called_with}",
        )
        self.assertIn(
            str(expected_result),
            str(result),
            f"Expected result '{expected_result}' not found in agent output: {result}",
        )

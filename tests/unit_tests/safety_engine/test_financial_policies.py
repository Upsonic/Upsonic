...
async def test_financial_info_block_credit_card(mock_infer_model):
    ...
    agent_with_financial_policy = Agent(
        model=mock_model, user_policy=FinancialInfoBlockPolicy, debug=True
    )

    credit_card_task = Task(
        description="My credit card number is 4532-1234-5678-9012. Can you help me with this?"
    )
...
async def test_financial_info_block_ssn(mock_infer_model):
    ...
    ssn_task = Task(
        description="My social security number is 123-45-6789. Please help me with my account."
    )
...
async def test_financial_info_anonymize_bank_account(mock_infer_model):
    ...
    mock_response = ModelResponse(
        parts=[
            TextPart(
                content="I can help you with your bank account inquiry. The account number you provided has been processed."
            )
        ],
        model_name="test-model",
        timestamp="2024-01-01T00:00:00Z",
    ...
    agent_with_anonymize_policy = Agent(
        model=mock_model, user_policy=FinancialInfoAnonymizePolicy, debug=True
    )

    bank_account_task = Task(
        description="My bank account number is 1234567890. Can you help me check my balance?"
    )
...
async def test_financial_info_replace_crypto(mock_infer_model):
    ...
    mock_response = ModelResponse(
        parts=[
            TextPart(
                content="I can help you with cryptocurrency information. The address you mentioned has been processed."
            )
        ],
        model_name="test-model",
        timestamp="2024-01-01T00:00:00Z",
    ...
    agent_with_replace_policy = Agent(
        model=mock_model, user_policy=FinancialInfoReplacePolicy, debug=True
    )

    crypto_task = Task(
        description="My Bitcoin address is 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa. Can you help me track it?"
    )
...
async def test_financial_info_agent_policy_exception(mock_infer_model):
    ...
    mock_response = ModelResponse(
        parts=[
            TextPart(
                content="Your credit card number 4532-1234-5678-9012 has been processed successfully."
            )
        ],
        model_name="test-model",
        timestamp="2024-01-01T00:00:00Z",
    ...
    agent_with_financial_exception = Agent(
        model=mock_model, agent_policy=FinancialInfoRaiseExceptionPolicy, debug=True
    )
...
async def test_financial_info_llm_policy(mock_infer_model):
    ...
    agent_with_llm_policy = Agent(
        model=mock_model, user_policy=FinancialInfoBlockPolicy_LLM, debug=True
    )

    complex_financial_task = Task(
        description="I need help with my investment portfolio. My account balance is $50,000 and my routing number is 123456789."
    )

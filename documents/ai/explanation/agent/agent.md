...
The default `run()` wrapper just delegates to `execute()`; each step is responsible for self-finalizing by calling `_finalize_step_result()` in its own `finally` (the canonical pattern in §5.4), which appends the `StepResult` to `context.step_results` and increments `context.execution_stats`. Error injection for durable-execution tests is no longer part of production; the same surface API (`inject_error_into_step`, `clear_error_injection`) lives in `tests/_pipeline_injection.py`, which monkey-patches `Step.run` on first use, raises `INJECTED ERROR: <msg>` on the first N invocations of a named step, and finalizes an ERROR `StepResult` on the context first so `get_problematic_step()` / `continue_run_async` can still find a resume point.
...
- **Heartbeat support.** `heartbeat`, `heartbeat_period` (minutes), `heartbeat_message` — `aexecute_heartbeat()` / `execute_heartbeat()` run the heartbeat message as a `do_async` call and return the agent's text response, suitable for UI integrations that want a periodic "what's the agent doing?" probe. By default the run is silent (console panels suppressed); pass `print=True` to render the normal panels for that heartbeat run.

...
| --- | --- |
| Model | `model`, `fallback_model`, `settings`, `profile`, `reasoning_effort`, `reasoning_summary`, `thinking_enabled`, `thinking_budget`, `thinking_include_thoughts`, `reasoning_format` |
...
`fallback_model` accepts the same forms as `model` (a `"provider/model"` string or a `Model` instance) and is `None` by default. When set, `infer_model(fallback_model)` resolves it once at construction and stores the result on `self.fallback_model` — a **parallel field** sibling to `self.model`, NOT a wrapper. Each `model.request(...)` invocation in the agent (the four auxiliary call sites in `agent.py` plus the main `ModelExecutionStep` and `PolicyCorrectionStep` sites in `pipeline/steps.py`) routes through the private helper `Agent._request_with_fallback(...)`. The helper tries `self.model.request(...)` first and, on `ModelHTTPError` with `status_code in {401, 403, 404}` (invalid API key, no access to the model, or model identifier not recognised for this account), emits a `warning_log(..., "ModelFallback")` and retries the same request against `self.fallback_model.request(...)`. Every other exception (`ModelHTTPError` with other codes, `ModelAPIError`, non-HTTP errors) propagates unchanged. Bedrock `ValidationException` and Vertex `FAILED_PRECONDITION` cases that surface as HTTP 400 are intentionally NOT in the trigger set (400 is too ambiguous — schema errors silently switching to a different provider's tolerance is a worse failure mode); v2 may inspect `ModelHTTPError.body` for known access-denied indicators per provider. The constructor's `settings` and `profile` parameters describe the PRIMARY model only — they land on `self.model._settings` / `self.model._profile`. The fallback is NOT auto-populated with the primary's settings; users configure the fallback post-construction via the `fallback_settings` / `fallback_profile` Python properties (whose setters raise `AttributeError` when no `fallback_model` is configured). The four properties on `Agent` are: `settings` and `profile` (primary's), `fallback_settings` and `fallback_profile` (fallback's). Reasoning settings (`reasoning_effort`, `thinking_enabled`, `thinking_budget`, `thinking_include_thoughts`, `reasoning_format`, `reasoning_summary`) are **agent-level intent** — they apply to both models, but each model receives its own provider-shape reasoning dict via `_get_model_specific_reasoning_settings_for(target_model)` so primary's `openai_reasoning_effort` keys don't pollute an Anthropic fallback's `_settings` and vice-versa. Per-call override (`agent.do(task, model="...")`) swaps the primary in `_apply_model_override` and inherits the construction-time `self.fallback_model` automatically — the helper reads `self.model` and `self.fallback_model` at call time. **Streaming calls** (`agent.stream()`, `agent.astream()`) go through `model.request_stream(...)` in `StreamModelExecutionStep` and do NOT currently trigger fallback — the retry semantics for streaming are nuanced (mid-stream provider-swap would stitch tokens from two providers, corrupting output) and deferred to v2. Locked by a streaming-gap test in `tests/unit_tests/agent/test_agent_fallback.py`.

The `WrapperModel.__setattr__` override that mirrors writes to `_settings` / `_profile` onto `self.wrapped` is unrelated to `fallback_model` after this change but is preserved because it fixes a latent silent-drop bug: `Agent(..., settings=..., instrument=True)` previously wrote settings onto the `InstrumentedModel` wrapper, while `model.settings` reads delegated through to the wrapped primary — so user-supplied settings were silently dropped at request time. The `__setattr__` mirror-and-forward keeps the wrapper's `_settings` in sync with the inner primary at every level of any future wrapper chain.

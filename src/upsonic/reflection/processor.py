...
class ReflectionProcessor:
    ...
    async def process_with_reflection(
        self, agent: "Agent", task: "Task", initial_response: Any
    ) -> Any:
        """
        ...
        return self._convert_to_response_format(
            state.final_response, initial_response, task
        )
    ...
    async def _evaluate_response(
        self, evaluator: "Agent", task: "Task", response: str, state: ReflectionState
    ) -> EvaluationResult:
        ...
        evaluation_prompt = ReflectionPrompts.EVALUATION_PROMPT.format(
            task_description=task.description, response=response, context=context
        )
    ...
    async def _generate_improved_response(
        ...
        improvement_prompt = ReflectionPrompts.IMPROVEMENT_PROMPT.format(
            ...
            improvements="\n".join(
                f"- {imp}" for imp in evaluation.suggested_improvements
            ),
            context=context,
        )
    ...
    def _convert_to_response_format(
        self, response_text: str, original_response: Any, task: "Task"
    ) -> Any:
    ...
    def _create_fallback_evaluation(
        ...
        return EvaluationResult(
            ...
            suggested_improvements=[
                "Review response manually for accuracy and completeness"
            ],

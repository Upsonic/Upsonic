...
_PROVIDER_IMPORTERS: Dict[
    str, Callable[[], Tuple[Type[EmbeddingProvider], Type[EmbeddingConfig]]]
] = {}
...
def _lazy_import_provider(
    provider_name: str,
) -> Tuple[Type[EmbeddingProvider], Type[EmbeddingConfig]]:
...
def _setup_provider_importers():
    ...
    def _import_azure_openai():
        from .azure_openai_provider import (
...
def create_best_available_embedding(
    use_case: str = "general", preference: str = "balanced", **kwargs
) -> EmbeddingProvider:
    """
    ...
    preference_order = preferences.get(use_case, preferences["general"]).get(
        preference, preferences["general"]["balanced"]
    )

    for provider in preference_order:
        if provider in available:
            info_log(
                f"Selected {provider} embedding provider for {use_case} use case with {preference} preference",
                context="EmbeddingFactory",
            )
    ...
    if available:
        ...
        info_log(
            f"No preferred provider available, using {provider}",
            context="EmbeddingFactory",
        )
...
def get_embedding_recommendations(
    use_case: str = "general", budget: str = "medium", privacy: str = "standard"
) -> List[Dict[str, Any]]:
    """

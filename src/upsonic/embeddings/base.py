...
class EmbeddingConfig(BaseModel):
    ...
    dimension: Optional[int] = Field(
        None, description="Expected embedding dimension (for validation)"
    )
    ...
    show_progress: bool = Field(
        True, description="Whether to show progress during batch operations"
    )
    cache_embeddings: bool = Field(False, description="Whether to cache embeddings")
    truncate_input: bool = Field(
        True, description="Whether to truncate inputs that exceed max length"
    )
    max_input_length: Optional[int] = Field(
        None, description="Maximum input length in tokens/characters"
    )

    enable_retry_with_backoff: bool = Field(
        True, description="Enable exponential backoff for retries"
    )
    enable_adaptive_batching: bool = Field(
        True, description="Dynamically adjust batch size based on performance"
    )
    enable_compression: bool = Field(
        False, description="Enable embedding compression for storage efficiency"
    )
    compression_ratio: float = Field(
        0.5, description="Target compression ratio (0.0-1.0)"
    )
...
class EmbeddingProvider(BaseModel, ABC):
    ...
    async def embed_texts(
        ...
        if show_progress and len(uncached_texts) > 1:
            self._create_progress_display(len(uncached_texts))
        ...
        if self.config.cache_embeddings:
            self._update_cache(
                uncached_texts, all_embeddings, metadata, uncached_indices
            )
    ...
    def _check_cache(
        self, texts: List[str], metadata: Optional[List[Dict[str, Any]]]
    ...
    def _update_cache(
        ...
        for i, (text, embedding) in enumerate(zip(texts, embeddings)):
            ...
            cache_key = self._get_cache_key(
                text, metadata[meta_idx] if metadata else None
            )

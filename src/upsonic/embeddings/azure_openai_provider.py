...
class AzureOpenAIEmbeddingConfig(EmbeddingConfig):
    ...
    client_id: Optional[str] = Field(
        None, description="Azure client ID for managed identity"
    )
    ...
    enable_content_filtering: bool = Field(
        True, description="Enable Azure content filtering"
    )
    data_residency_region: Optional[str] = Field(
        None, description="Data residency region"
    )

    parallel_requests: int = Field(
        3, description="Parallel requests (Azure has lower limits)"
    )
...
class AzureOpenAIEmbedding(EmbeddingProvider):
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            debug_log(
                f"Azure OpenAI connection validation failed: {str(e)}",
                context="AzureOpenAIEmbedding",
            )
            return False
    ...
    async def close(self):
        ...
        if hasattr(self, "client") and self.client:
            try:
                ...
                warning_log(
                    f"Error closing Azure OpenAI client: {e}",
                    context="AzureOpenAIEmbedding",
                )
        ...
        if hasattr(self, "_credential") and self._credential:
            try:
                ...
                warning_log(
                    f"Error closing Azure credential: {e}",
                    context="AzureOpenAIEmbedding",
                )
...
def create_azure_embedding_with_managed_identity(
    azure_endpoint: str, deployment_name: str, **kwargs
) -> AzureOpenAIEmbedding:

...
class FastEmbedConfig(EmbeddingConfig):
    ...
    model_name: str = Field(
        "BAAI/bge-small-en-v1.5", description="FastEmbed model name"
    )

    cache_dir: Optional[str] = Field(None, description="Model cache directory")
    threads: Optional[int] = Field(
        None, description="Number of threads (auto-detected if None)"
    )
    ...
    enable_parallel_processing: bool = Field(
        True, description="Enable parallel text processing"
    )
    doc_embed_type: str = Field(
        "default", description="Document embedding type (default, passage)"
    )
    ...
    enable_sparse_embeddings: bool = Field(
        False, description="Use sparse embeddings for better performance"
    )
    sparse_model_name: Optional[str] = Field(
        None, description="Sparse model name if different from dense"
    )
    ...
    def validate_providers(cls, v):
        ...
        for provider in v:
            if provider not in valid_providers:
                raise ValueError(
                    f"Invalid ONNX provider: {provider}. Valid options: {valid_providers}"
                )
    ...
    def validate_embed_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(
                f"Invalid doc_embed_type: {v}. Valid options: {valid_types}"
            )
        return v
...
class FastEmbedProvider(EmbeddingProvider):
        ...
        if self.config.enable_gpu:
            try:
                ...
                if (
                    ...
                    debug_log(
                        "CUDA provider available and enabled", context="FastEmbed"
                    )
                elif (
                    "ROCMExecutionProvider" in available_providers
                    and "ROCMExecutionProvider" not in self.providers
                ):
                    self.providers.insert(0, "ROCMExecutionProvider")
                    debug_log(
                        "ROCm provider available and enabled", context="FastEmbed"
                    )
                elif (
                    "CoreMLExecutionProvider" in available_providers
                    and "CoreMLExecutionProvider" not in self.providers
                ):
                    self.providers.insert(0, "CoreMLExecutionProvider")
                    debug_log(
                        "CoreML provider available and enabled", context="FastEmbed"
                    )
                else:
                    debug_log(
                        "GPU requested but no GPU providers available, using CPU",
                        context="FastEmbed",
                    )

            except ImportError:
                debug_log(
                    "ONNXRuntime not available for provider detection",
                    context="FastEmbed",
                )
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            info_log(
                f"Initializing FastEmbed model: {self.config.model_name}",
                context="FastEmbed",
            )
            ...
            if self.config.enable_sparse_embeddings:
                sparse_model_name = (
                    self.config.sparse_model_name or self.config.model_name
                )
                try:
                    ...
                    success_log(
                        f"Initialized sparse embedding model: {sparse_model_name}",
                        "FastEmbedProvider",
                    )
                    ...
                    warning_log(
                        f"Could not initialize sparse model: {e}", "FastEmbedProvider"
                    )
            ...
            success_log(
                "FastEmbed models initialized successfully", "FastEmbedProvider"
            )
            ...
            try:
                if hasattr(self.embedding_model, "model"):
                    ...
                    if hasattr(model, "get_input_details"):
                        ...
                        if input_details:
                            info["input_shape"] = input_details[0].get(
                                "shape", "unknown"
                            )
                    ...
                    if hasattr(model, "get_output_details"):
                        ...
                        if output_details:
                            info["output_shape"] = output_details[0].get(
                                "shape", "unknown"
                            )
                ...
                warning_log(
                    f"Could not get detailed model info: {e}", "FastEmbedProvider"
                )
    ...
    def _process_embeddings(
        self, embeddings_iterator: Iterator[np.ndarray]
    ) -> List[List[float]]:
    ...
    async def _embed_batch(
        self, texts: List[str], mode: EmbeddingMode = EmbeddingMode.DOCUMENT
    ) -> List[List[float]]:
        """
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            test_result = await self._embed_batch(
                ["test connection"], EmbeddingMode.QUERY
            )
                ...
                models.append(
                    {
                        ...
                        "size_mb": model_info.get("size_in_GB", 0) * 1024
                        if model_info.get("size_in_GB")
                        else "unknown",
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        try:
            if os.path.exists(cache_dir):
                ...
                base_info.update({"model_cache_dir": cache_dir, "cache_exists": False})
        except Exception as e:
    ...
    async def close(self):
        ...
        if hasattr(self, "sparse_model") and self.sparse_model:
            try:
                ...
                warning_log(
                    f"Error closing FastEmbed sparse model: {e}", "FastEmbedProvider"
                )
...
def create_bge_small_embedding(**kwargs) -> FastEmbedProvider:
    ...
    config = FastEmbedConfig(model_name="BAAI/bge-small-en-v1.5", **kwargs)
...
def create_bge_large_embedding(**kwargs) -> FastEmbedProvider:
    ...
    config = FastEmbedConfig(model_name="BAAI/bge-large-en-v1.5", **kwargs)
    return FastEmbedProvider(config=config)


def create_e5_embedding(**kwargs) -> FastEmbedProvider:
    """Create E5 FastEmbed provider (multilingual)."""
    config = FastEmbedConfig(model_name="intfloat/e5-large-v2", **kwargs)
    return FastEmbedProvider(config=config)
...
def create_sparse_embedding(**kwargs) -> FastEmbedProvider:
    ...
    config = FastEmbedConfig(
        model_name="prithivida/Splade_PP_en_v1", enable_sparse_embeddings=True, **kwargs
    )
    return FastEmbedProvider(config=config)


def create_gpu_accelerated_embedding(
    model_name: str = "BAAI/bge-large-en-v1.5", **kwargs
) -> FastEmbedProvider:

...
class HuggingFaceEmbeddingConfig(EmbeddingConfig):
    ...
    use_api: bool = Field(
        False, description="Use HuggingFace Inference API instead of local model"
    )
    use_local: bool = Field(True, description="Use local model execution")

    device: Optional[str] = Field(
        None, description="Device to run model on (auto-detected if None)"
    )
    torch_dtype: str = Field(
        "float32", description="PyTorch data type (float16, float32, bfloat16)"
    )
    ...
    pooling_strategy: str = Field(
        "mean", description="Pooling strategy (mean, cls, max)"
    )
    normalize_embeddings: bool = Field(
        True, description="Normalize embeddings to unit length"
    )
    ...
    wait_for_model: bool = Field(
        True, description="Wait for model to load if using API"
    )
    ...
    def validate_pooling_strategy(cls, v):
        ...
        if v not in valid_strategies:
            raise ValueError(
                f"Invalid pooling strategy: {v}. Valid options: {valid_strategies}"
            )
        return v
...
class HuggingFaceEmbedding(EmbeddingProvider):
    ...
    def _setup_device(self):
        ...
        info_log(
            f"HuggingFace embedding using device: {self.device}",
            context="HuggingFaceEmbedding",
        )
    ...
    def _setup_authentication(self):
        ...
        if hf_token:
            try:
                ...
                warning_log(
                    f"HuggingFace authentication failed: {e}", "HuggingFaceProvider"
                )
        ...
        try:
            ...
            if self.config.enable_quantization:
                if self.config.quantization_bits == 8:
                    debug_log(
                        "Configuring 8-bit quantization using bitsandbytes...",
                        context="HuggingFaceEmbedding",
                    )
                    ...
                    debug_log(
                        "Configuring 4-bit quantization using bitsandbytes...",
                        context="HuggingFaceEmbedding",
                    )
            ...
            tokenizer_kwargs = {
                ...
                if k not in ["quantization_config", "device_map", "load_in_8bit"]
            }
            ...
            self.model = AutoModel.from_pretrained(
                self.config.model_name, torch_dtype=torch_dtype, **model_kwargs
            )

            if "device_map" not in model_kwargs:
                self.model = self.model.to(self.device)
                debug_log(
                    f"Model moved to device: {self.device}",
                    context="HuggingFaceEmbedding",
                )
    ...
    def _setup_api_session(self):
        ...
        self._inference_client = InferenceClient(
            token=hf_token, timeout=getattr(self.config, "timeout", 30)
        )
        ...
        if self._model_info is None:
            ...
            if hasattr(self, "model") and self.model:
                try:
                    ...
                    info["hidden_size"] = getattr(
                        self.model.config, "hidden_size", "unknown"
                    )
            ...
            if hasattr(self, "tokenizer") and self.tokenizer:
                ...
                info["model_max_length"] = getattr(
                    self.tokenizer, "model_max_length", "unknown"
                )
        ...
        try:
            ...
            def sync_api_call():
                return self._inference_client.feature_extraction(
                    text=texts, model=self.config.model_name
                )
            ...
            raise ModelConnectionError(
                f"Failed to get embeddings via API: {str(e)}", original_error=e
            )
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            debug_log(
                f"HuggingFace connection validation failed: {str(e)}",
                context="HuggingFaceEmbedding",
            )
            return False
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            if self.device.startswith("cuda"):
                ...
                return {
                    "memory_usage": "Memory tracking not available for this device",
                    "device": self.device,
                }

        except Exception as e:
            return {
                "memory_usage": f"Error getting memory info: {e}",
                "device": self.device,
            }
    ...
    async def close(self):
        ...
        if hasattr(self, "model") and self.model:
            try:
                ...
                warning_log(
                    f"Error closing HuggingFace model: {e}", "HuggingFaceProvider"
                )
        ...
        if hasattr(self, "tokenizer") and self.tokenizer:
            try:
                ...
                warning_log(
                    f"Error closing HuggingFace tokenizer: {e}", "HuggingFaceProvider"
                )
        ...
        if hasattr(self, "_inference_client") and self._inference_client:
            try:
                ...
                warning_log(
                    f"Error closing HuggingFace InferenceClient: {e}",
                    "HuggingFaceProvider",
                )
    ...
    async def remove_local_cache(self) -> bool:
        ...
        try:
            model_path = try_to_load_from_cache(
                repo_id=self.config.model_name, cache_dir=self.config.cache_dir
            )
            if model_path and os.path.exists(model_path):
                ...
                success_log(
                    f"Local cache removed for {self.config.model_name} at {root}",
                    "HuggingFaceProvider",
                )
                return True
            else:
                ...
                info_log(
                    f"No local cache found for {self.config.model_name}",
                    "HuggingFaceProvider",
                )
                return False
        except Exception as e:
            ...
            error_log(
                f"Error removing local cache for {self.config.model_name}: {e}",
                "HuggingFaceProvider",
            )
            return False
...
def create_sentence_transformer_embedding(
    model_name: str = "sentence-transformers/all-MiniLM-L6-v2", **kwargs
...
def create_mpnet_embedding(**kwargs) -> HuggingFaceEmbedding:
    ...
    return create_sentence_transformer_embedding(
        model_name="sentence-transformers/all-mpnet-base-v2", **kwargs
    )
...
def create_huggingface_api_embedding(
    model_name: str, hf_token: Optional[str] = None, **kwargs
) -> HuggingFaceEmbedding:

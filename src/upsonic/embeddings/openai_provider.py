...
class OpenAIEmbeddingConfig(EmbeddingConfig):
    ...
    enable_rate_limiting: bool = Field(
        True, description="Enable intelligent rate limiting"
    )
...
class OpenAIEmbedding(EmbeddingProvider):
    ...
    def _setup_client(self):
        ...
        client_params = {"api_key": api_key, "timeout": self.config.request_timeout}
    ...
    def pricing_info(self) -> Dict[str, float]:
        ...
        return {"per_million_tokens": price, "currency": "USD", "updated": "2024-01-01"}
            ...
            if "retry-after" in str(e):
                try:
                    ...
                    wait_match = re.search(
                        r"retry-after[:\s]+(\d+)", str(e), re.IGNORECASE
                    )
                    if wait_match:
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            debug_log(
                f"OpenAI connection validation failed: {str(e)}",
                context="OpenAIEmbedding",
            )
            return False
    ...
    def estimate_cost_detailed(
        self, num_texts: int, avg_text_length: int = 100
    ) -> Dict[str, Any]:
        ...
        return {
            **base_estimate,
            ...
            "comparison": {
                "vs_ada_002": self._compare_model_cost(
                    "text-embedding-ada-002", num_texts, avg_text_length
                ),
                "vs_3_small": self._compare_model_cost(
                    "text-embedding-3-small", num_texts, avg_text_length
                ),
                "vs_3_large": self._compare_model_cost(
                    "text-embedding-3-large", num_texts, avg_text_length
                ),
            },
        }

    def _compare_model_cost(
        self, model_name: str, num_texts: int, avg_text_length: int
    ) -> Dict[str, float]:
        ...
        return {"cost": cost, "difference": difference, "percentage": percentage}
    ...
    async def close(self):
        ...
        if hasattr(self, "client") and self.client:
            try:
                ...
                warning_log(
                    f"Error closing OpenAI client: {e}", context="OpenAIEmbedding"
                )
...
def create_openai_embedding(
    model_name: str = "text-embedding-3-small", api_key: Optional[str] = None, **kwargs
) -> OpenAIEmbedding:
    """
    ...
    """
    config = OpenAIEmbeddingConfig(model_name=model_name, api_key=api_key, **kwargs)

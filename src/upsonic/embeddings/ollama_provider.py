...
class OllamaEmbeddingConfig(EmbeddingConfig):
    ...
    model_name: str = Field(
        "nomic-embed-text", description="Ollama embedding model name"
    )

    auto_pull_model: bool = Field(
        True, description="Automatically pull model if not available"
    )
    keep_alive: Optional[str] = Field(
        "5m", description="Keep model loaded for duration"
    )
    ...
    enable_keep_alive: bool = Field(
        True, description="Keep model loaded between requests"
    )
    ...
    def validate_model_name(cls, v):
        ...
        warning_log(
            f"'{v}' may not be a standard Ollama embedding model. Known models: {known_models}",
            context="OllamaEmbedding",
        )
        return v
...
class OllamaEmbedding(EmbeddingProvider):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        connection_info(
                            "Ollama", version_info.get("version", "unknown")
                        )
                        return True
                    else:
    ...
    def _ensure_model_available_sync(self):
        ...
        try:
            ...
            for model_name in model_names:
                if self.config.model_name in model_name or model_name.startswith(
                    ...
                    success_log(
                        f"Model {self.config.model_name} found: {model_name}",
                        "OllamaProvider",
                    )
                    break
            ...
            if not model_found:
                ...
                info_log(
                    f"Model {self.config.model_name} not found, pulling...",
                    "OllamaProvider",
                )
    ...
    def _list_models_sync(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.session.get(
                f"{self.config.base_url}/api/tags", timeout=self.config.request_timeout
            )
            if response.status_code == 200:
                data = response.json()
    ...
    def _pull_model_sync(self):
        ...
        try:
            ...
            response = self.session.post(
                f"{self.config.base_url}/api/pull", json=pull_payload, timeout=600
            )
                            ...
                            if line:
                                try:
                                    ...
                                    if "status" in progress:
                                        debug_log(
                                            f"Pull progress: {progress['status']}",
                                            context="OllamaEmbedding",
                                        )
    ...
    async def _preload_model(self):
        ...
        try:
            ...
            info_log(
                f"Model {self.config.model_name} preloaded successfully",
                context="OllamaEmbedding",
            )
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            debug_log(
                f"Ollama connection validation failed: {str(e)}",
                context="OllamaEmbedding",
            )
            return False
...
def create_nomic_embedding(**kwargs) -> OllamaEmbedding:
    ...
    config = OllamaEmbeddingConfig(model_name="nomic-embed-text", **kwargs)
...
def create_mxbai_embedding(**kwargs) -> OllamaEmbedding:
    ...
    config = OllamaEmbeddingConfig(model_name="mxbai-embed-large", **kwargs)
    return OllamaEmbedding(config=config)


def create_arctic_embedding(**kwargs) -> OllamaEmbedding:
    """Create Snowflake Arctic embedding provider."""
    config = OllamaEmbeddingConfig(model_name="snowflake-arctic-embed", **kwargs)
    return OllamaEmbedding(config=config)

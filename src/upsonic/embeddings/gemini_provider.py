...
class GeminiEmbeddingConfig(EmbeddingConfig):
    ...
    model_name: str = Field(
        "gemini-embedding-001", description="Gemini embedding model"
    )

    enable_safety_filtering: bool = Field(
        True, description="Enable Google's safety filtering"
    )
    ...
    enable_batch_processing: bool = Field(
        True, description="Enable batch processing optimization"
    )

    use_google_cloud_auth: bool = Field(
        False, description="Use Google Cloud authentication"
    )
    ...
    use_vertex_ai: bool = Field(
        False, description="Use Vertex AI API instead of Gemini Developer API"
    )
    api_version: str = Field(
        "v1beta", description="API version to use (v1beta, v1, v1alpha)"
    )
    ...
    output_dimensionality: Optional[int] = Field(
        None, description="Output embedding dimension (128-3072)"
    )
    embedding_config: Optional[Dict[str, Any]] = Field(
        None, description="Additional embedding configuration"
    )
    ...
    def validate_api_version(cls, v):
        ...
        if v not in valid_versions:
            raise ValueError(
                f"Invalid api_version: {v}. Valid options: {valid_versions}"
            )
        return v
    ...
    def validate_output_dimensionality(cls, v):
        ...
        if v is not None:
            if not (128 <= v <= 3072):
                raise ValueError(
                    f"output_dimensionality must be between 128 and 3072, got {v}"
                )
        return v
...
class GeminiEmbedding(EmbeddingProvider):
    ...
    def _setup_authentication(self):
        ...
        if self.config.use_google_cloud_auth or self.config.use_vertex_ai:
            ...
            try:
                ...
                info_log(
                    f"Using Google Cloud authentication for project: {self.project_id}",
                    context="GeminiEmbedding",
                )
            ...
            info_log(
                "Using Google AI API key authentication", context="GeminiEmbedding"
            )
    ...
    def _setup_client(self):
        ...
        try:
            ...
            if self.config.use_google_cloud_auth or self.config.use_vertex_ai:
                ...
                info_log(
                    f"Gemini client configured for Vertex AI with model: {self.config.model_name}",
                    context="GeminiEmbedding",
                )
            else:
                api_key = (
                ...
                info_log(
                    f"Gemini client configured for Gemini Developer API with model: {self.config.model_name}",
                    context="GeminiEmbedding",
                )
    ...
    def _setup_caching(self):
        ...
        try:
            cache_config = types.CacheConfig(ttl_seconds=self.config.cache_ttl_seconds)
            self.cache = self.client.caches.create(config=cache_config)
            info_log(
                f"Response caching enabled with TTL: {self.config.cache_ttl_seconds}s",
                context="GeminiEmbedding",
            )
        except Exception as e:
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self._model_info is None:
            model_info_map = {
                "gemini-embedding-001": {
                    "dimensions": [
                        ...
                        1536,
                        768,
                    ],  # via MRL (Matryoshka Representation Learning)
            ...
            self._model_info = model_info_map.get(
        ...
        try:
            ...
            if self.config.enable_batch_processing and len(texts) > 1:
                try:
                    ...
                    warning_log(
                        f"Batch processing failed, falling back to individual requests: {batch_error}",
                        context="GeminiEmbedding",
                    )
                    for text in texts:
    ...
    async def _embed_single_text(self, text: str, task_type: str) -> List[float]:
        ...
        try:
            ...
            response = self.client.models.embed_content(
                model=self.config.model_name, contents=text, config=config
            )
    ...
    async def _embed_texts_batch(
        ...
        try:
            ...
            warning_log(
                f"Batch embedding failed, using individual requests: {e}",
                context="GeminiEmbedding",
            )
            results = []
            for text in texts:
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            debug_log(
                f"Gemini connection validation failed: {str(e)}",
                context="GeminiEmbedding",
            )
            return False
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_chars_per_request": self._character_count
                    ...
                    models.append(
                        {
                            ...
                            "input_token_limit": getattr(
                                model, "input_token_limit", "unknown"
                            ),
                            "output_token_limit": getattr(
                                model, "output_token_limit", "unknown"
                            ),
                        }
                    )
    ...
    async def close(self):
        ...
        if hasattr(self, "client") and self.client:
            try:
                ...
                warning_log(
                    f"Error closing Gemini client: {e}", context="GeminiEmbedding"
                )
        ...
        if hasattr(self, "cache") and self.cache:
            try:
                ...
                warning_log(
                    f"Error closing Gemini cache: {e}", context="GeminiEmbedding"
                )
        ...
        if hasattr(self, "_credential") and self._credential:
            try:
                ...
                warning_log(
                    f"Error closing Google credential: {e}", context="GeminiEmbedding"
                )
...
def create_gemini_document_embedding(
...
def create_gemini_query_embedding(
    api_key: Optional[str] = None, **kwargs
) -> GeminiEmbedding:
...
def create_gemini_semantic_embedding(
    api_key: Optional[str] = None, **kwargs
) -> GeminiEmbedding:
...
def create_gemini_cloud_embedding(
    project_id: str, location: str = "us-central1", **kwargs
) -> GeminiEmbedding:

    ...
    def calculate_suspicion(self) -> str:
        ...
        if self.url_validation.is_suspicious:
            ...
            validation_details.extend(
                [f"- {point}" for point in self.url_validation.suspicious_points]
            )
        ...
        if self.number_validation.is_suspicious:
            ...
            validation_details.append(
                f"Number Issues: {self.number_validation.feedback}"
            )
            validation_details.extend(
                [f"- {point}" for point in self.number_validation.suspicious_points]
            )
        ...
        if self.information_validation.is_suspicious:
            ...
            validation_details.append(
                f"Information Issues: {self.information_validation.feedback}"
            )
            validation_details.extend(
                [
                    f"- {point}"
                    for point in self.information_validation.suspicious_points
                ]
            )
        ...
        if self.code_validation.is_suspicious:
            ...
            validation_details.extend(
                [f"- {point}" for point in self.code_validation.suspicious_points]
            )
        ...
        if prevent_hallucination > 0:
            if prevent_hallucination == 10:
                ...
                if copy_task.context:
                    context_items = (
                        copy_task.context
                        if isinstance(copy_task.context, list)
                        else [copy_task.context]
                    )
                ...
                context_strings = [
                    strip_context_tags(ctx_str) if isinstance(ctx_str, str) else ctx_str
                ...
                if validation_tasks:
                    ...
                    for i, validation_type in enumerate(validation_types):
                        validation_coroutines.append(
                            validator_agents[validation_type].do_async(
                                validation_tasks[i]
                            )
                        )
                ...
                if validation_result.any_suspicion:
                    editor_agent = AgentConfiguration(
                        model=model, name="Information Editor Agent"
                    )

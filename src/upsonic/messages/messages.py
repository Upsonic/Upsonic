...
from upsonic._utils import generate_tool_call_id as _generate_tool_call_id, now_utc as _now_utc
...
class RetryPromptPart:
    ...
    * a tool raised a [`ModelRetry`][upsonic.exceptions.ModelRetry] exception
    * no tool was found for the tool name
    * the model returned plain text when a structured response was expected
    * Pydantic validation of a structured response failed, here content is derived from a Pydantic
      [`ValidationError`][pydantic_core.ValidationError]
    * an output validator raised a [`ModelRetry`][upsonic.exceptions.ModelRetry] exception
    """
...
class FinalResultEvent:
    ...
    This event is emitted after all content for the result part (text or tool output) has been 
    streamed, signaling that the result is complete and ready for processing."""
...
# Utility functions for extracting parts from ModelResponse
...
def get_text_content(response: ModelResponse) -> str | None:
    ...
    Example:
        ```python
        from upsonic.messages import get_text_content

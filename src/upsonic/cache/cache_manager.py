...
class CacheManager:
    ...
    async def _find_similar_cache_entry(
        ...
        for cache_id, cache_entry in self._cache_data.items():
            ...
            similarity = self._calculate_similarity(
                input_vector, cache_entry["input_vector"]
            )
    ...
    async def _find_exact_cache_entry(
        self, input_text: str, duration_minutes: int, llm_provider: Optional[Any] = None
    ) -> Optional[CacheEntry]:
        ...
        best_match = await self._llm_batch_compare_queries(
            input_text, valid_entries, llm_provider
        )
    ...
    async def _llm_batch_compare_queries(
        ...
        try:
            ...
            comparison_agent = Agent(model=llm_provider, debug=False)
            ...
            comparison_task = Task(
                description=comparison_prompt, enable_cache=False, response_format=str
            )
    ...
    async def get_cached_response(
        ...
        if cache_method == "vector_search":
            ...
            cache_entry = await self._find_exact_cache_entry(
                input_text, duration_minutes, llm_provider
            )

...
class DecisionLLM(BaseModel):
    ...
    description: str
    true_branch: Optional[
        Union["TaskNode", "TaskChain", "DecisionFunc", "DecisionLLM"]
    ] = None
    false_branch: Optional[
        Union["TaskNode", "TaskChain", "DecisionFunc", "DecisionLLM"]
    ] = None
...
"""
    ...
    def if_true(
        self,
        branch: Union["TaskNode", Task, "TaskChain", "DecisionFunc", "DecisionLLM"],
    ) -> "DecisionLLM":
        """
    ...
    def __rshift__(
        self, other: Union["TaskNode", Task, "TaskChain", "DecisionFunc", "DecisionLLM"]
    ) -> "TaskChain":
        """
...
class TaskNode(BaseModel):
    ...
    def __rshift__(
        self, other: Union["TaskNode", Task, "DecisionFunc", "DecisionLLM", "TaskChain"]
    ) -> "TaskChain":
        """
...
class TaskChain:
    ...
    def add(
        self,
        node_or_chain: Union[TaskNode, Task, "TaskChain", DecisionFunc, DecisionLLM],
    ) -> "TaskChain":
        """
        ...
        if isinstance(node_or_chain, (TaskNode, DecisionFunc, DecisionLLM)):
            ...
            if isinstance(new_node, (DecisionFunc, DecisionLLM)):
                for branch in [new_node.true_branch, new_node.false_branch]:
                    ...
                    if isinstance(branch, TaskChain):
                        self.nodes.extend(
                            n for n in branch.nodes if n not in self.nodes
                        )
                        self.edges.update(branch.edges)
                        if branch.nodes:
                            if new_node.id not in self.edges:
            ...
            all_target_ids = {
                target
                for targets in incoming_chain.edges.values()
                for target in targets
            }
            entry_points.extend(
                [n for n in incoming_chain.nodes if n.id not in all_target_ids]
            )
...
class Graph(BaseModel):
    ...
    nodes: List[Union[TaskNode, DecisionFunc, DecisionLLM]] = Field(
    ...
    def __init__(self, **data):
        if "default_agent" in data and data["default_agent"] is not None:
            ...
            if not isinstance(agent, BaseAgent):
                raise TypeError(
                    "default_agent must be an instance of a class that inherits from BaseAgent."
                )
            if not hasattr(agent, "do_async") or not callable(
                getattr(agent, "do_async")
            ):
        ...
        if "storage" in data:
            ...
            if self.storage and not isinstance(self.storage, Storage):
                raise TypeError(
                    "storage must be an instance of a class that inherits from Storage."
                )
        super().__init__(**data)

    def add(
        self, tasks_chain: Union[Task, TaskNode, TaskChain, DecisionFunc, DecisionLLM]
    ) -> "Graph":
        """
    ...
    async def _execute_task(
        ...
        *,
        ...
        if runner is None:
            ...
            if runner is None:
                raise ValueError(
                    f"No agent specified for task '{escape_rich_markup(task.description)}' and no default agent set"
                )
        ...
        try:
            if verbose:
                ...
                table.add_row(
                    "[bold]Task:[/bold]",
                    f"[cyan]{escape_rich_markup(task.description)}[/cyan]",
                )
                runner_type = (
                    runner.__class__.__name__
                    if hasattr(runner, "__class__")
                    else type(runner).__name__
                )
                table.add_row(
                    "[bold]Agent:[/bold]",
                    f"[yellow]{escape_rich_markup(runner_type)}[/yellow]",
                )
                if task.tools:
                    tool_names = [
                        escape_rich_markup(
                            t.__class__.__name__ if hasattr(t, "__class__") else str(t)
                        )
                        for t in task.tools
                    ]
                    table.add_row(
                        "[bold]Tools:[/bold]",
                        f"[green]{escape_rich_markup(', '.join(tool_names))}[/green]",
                    )
                panel = Panel(
                    table,
            ...
            if hasattr(runner, "do_async"):
                output = await runner.do_async(
                    task, state=state, graph_execution_id=graph_execution_id
                )
            else:
            ...
            if verbose:
                ...
                if task.total_cost:
                    table.add_row(
                        "[bold]Estimated Cost:[/bold]", f"${task.total_cost:.4f}"
                    )
                panel = Panel(
                    table,
            ...
            if verbose:
                console.print(
                    f"[bold red]Task '{escape_rich_markup(task.description)}' failed: {escape_rich_markup(str(e))}[/bold red]"
                )
            raise
    ...
    async def _evaluate_decision(
        ...
        if isinstance(decision_node, DecisionFunc):
            ...
            if agent is None:
                raise ValueError(
                    f"No agent available for LLM-based decision: '{decision_node.description}'"
                )
            ...
            if verbose:
                console.print(
                    f"[dim]LLM Decision Response: {escape_rich_markup(str(response))}[/dim]"
                )
                console.print(
                    f"[dim]Decision Result: {'Yes' if result else 'No'}[/dim]"
                )
        else:
        ...
        if verbose:
            ...
            table.add_row(
                "[bold]Decision:[/bold]",
                f"[cyan]{escape_rich_markup(decision_node.description)}[/cyan]",
            )
            table.add_row(
                "[bold]Result:[/bold]", f"[green]{'Yes' if result else 'No'}[/green]"
            )
            panel = Panel(
                table,
    ...
    def _get_predecessors(
        self, node: Union[TaskNode, DecisionFunc, DecisionLLM]
    ) -> List[Union[TaskNode, DecisionFunc, DecisionLLM]]:
        """
        ...
        """
        predecessor_ids = {
            n_id for n_id, next_ids in self.edges.items() if node.id in next_ids
        }
    ...
    def _get_start_nodes(self) -> List[Union[TaskNode, DecisionFunc, DecisionLLM]]:
        ...
        """
        all_target_ids = {
            target_id for targets in self.edges.values() for target_id in targets
        }
    ...
    def _get_all_branch_node_ids(
        self, branch: Union[TaskNode, TaskChain, DecisionFunc, DecisionLLM, None]
    ) -> Set[str]:
    ...
    async def _run_sequential(
        ...
        *,
        ...
        if progress_context:
            ...
            overall_task = progress_context.add_task(
                "[bold blue]Graph Execution", total=all_nodes_count
            )
        ...
        try:
            while execution_queue:
                ...
                if isinstance(node, TaskNode):
                    ...
                    if predecessors:
                        existing_source_ids = {
                            s.task_description_or_id
                            for s in node.task.context
                            if isinstance(s, TaskOutputSource)
                        }
                        for pred in predecessors:
                            if (
                                pred.id in executed_node_ids
                                and pred.id not in existing_source_ids
                            ):
                                source = TaskOutputSource(
                                    task_description_or_id=pred.id
                                )
                                node.task.context.append(source)
                                if verbose:
                                    console.print(
                                        f"[dim]Auto-injecting source for node '{pred.id}' into task {escape_rich_markup(node.task.description)}[/dim]"
                                    )
                    output = await self._execute_task(
                        node, self.state, verbose, graph_execution_id=graph_execution_id
                    )
                    ...
                    branch_to_follow = await self._evaluate_decision(
                        node, self.state, verbose
                    )
                    executed_node_ids.add(node.id)
                    pruned_branch = (
                        node.false_branch
                        if branch_to_follow == node.true_branch
                ...
                for next_node in successors:
                    if (
                        next_node.id in queued_node_ids
                        or next_node.id in executed_node_ids
                        or next_node.id in pruned_node_ids
                    ):
                        continue
                    if all(
                        (p.id in executed_node_ids or p.id in pruned_node_ids)
                        for p in self._get_predecessors(next_node)
                    ):
    ...
    async def run_async(
        self, verbose: bool = True, show_progress: bool = None
    ) -> State:
        """
        ...
        return await self._run_sequential(
            verbose, show_progress, graph_execution_id=graph_execution_id
        )
        ...
        if loop.is_running():
            with ThreadPoolExecutor() as executor:
                return executor.submit(
                    asyncio.run, self.run_async(verbose, show_progress)
                ).result()
        else:

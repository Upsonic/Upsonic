...
class DurableStateSerializer:
    ...
    def serialize_task(task: Any) -> Dict[str, Any]:
        ...
        serialized = {
            ...
            "_response": DurableStateSerializer._serialize_response(
                getattr(task, "_response", None)
            ),
            "context": DurableStateSerializer._serialize_context_data(
    ...
    def deserialize_task(data: Dict[str, Any]) -> Any:
        ...
        task = Task(
            ...
            response=DurableStateSerializer._deserialize_response(
                data.get("_response")
            ),
            context=DurableStateSerializer._deserialize_context_data(
                data.get("context")
            ),
    ...
    def _serialize_context_data(context: Any) -> Any:
        ...
        if isinstance(context, list):
            return [
                DurableStateSerializer._serialize_context_data(item) for item in context
            ]

        if isinstance(context, dict):
            return {
                k: DurableStateSerializer._serialize_context_data(v)
    ...
    def serialize_context(context: Any) -> Dict[str, Any]:
        ...
        return {
            ...
            "messages": DurableStateSerializer.serialize_messages(
                getattr(context, "messages", [])
            ),
            "response": DurableStateSerializer._serialize_model_response(
                getattr(context, "response", None)
            ),
            "final_output": DurableStateSerializer._serialize_response(
                getattr(context, "final_output", None)
            ),
    ...
    def deserialize_context(
        data: Dict[str, Any], task: Any, agent: Any, model: Any
    ) -> Any:
        """
        ...
        # Restore messages
        context.messages = DurableStateSerializer.deserialize_messages(
            data.get("messages", {"messages": []})
        )

        # Restore response if available
        if data.get("response"):
            context.response = DurableStateSerializer._deserialize_model_response(
                data["response"]
            )

        # Restore final output
        context.final_output = DurableStateSerializer._deserialize_response(
    ...
    def serialize_messages(messages: List[Any]) -> Dict[str, Any]:
        ...
        """
        if not messages:
            return {"type": "messages", "messages": [], "count": 0}
    ...
    def deserialize_messages(data: Dict[str, Any]) -> List[Any]:
        ...
        try:
            ...
            warning_log(
                f"Could not validate messages during deserialization: {e}",
                "DurableSerializer",
            )
            return []
    ...
    def _serialize_model_response(response: Any) -> Optional[Dict[str, Any]]:
        ...
        if hasattr(response, "model_dump"):
            try:
                ...
                return {"type": "ModelResponse", "data": to_jsonable_python(response)}
            except Exception:
    ...
    def _deserialize_model_response(data: Optional[Dict[str, Any]]) -> Any:
        ...
        try:
            ...
            warning_log(
                f"Could not deserialize ModelResponse: {e}", "DurableSerializer"
            )
            return None
    ...
    def reconstruct_context(
        context_data: Dict[str, Any], task: Any, agent: Any, model: Any
    ) -> Any:
        """
        ...
        Returns:
            Reconstructed StepContext object
        """
        return DurableStateSerializer.deserialize_context(
            context_data, task, agent, model
        )

...
class DurableExecution:
    ...
    async def mark_failed_async(self, error: str) -> None:
        ...
        if self.debug:
            ...
            error_log(
                f"Execution failed: {self.execution_id} - {error}", "DurableExecution"
            )
    ...
    async def load_by_id_async(
        execution_id: str, storage: DurableExecutionStorage
    ...
    def load_by_id(
        ...
        try:
            ...
            return asyncio.run(DurableExecution.load_by_id_async(execution_id, storage))

...
class FileDurableStorage(DurableExecutionStorage):
    ...
    def _get_file_path(self, execution_id: str) -> Path:
        ...
        safe_id = "".join(
            c if c.isalnum() or c in ("-", "_") else "_" for c in execution_id
        )
    ...
    def _read_file_sync(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            warning_log(
                f"Could not read state file {file_path}: {e}", "FileDurableStorage"
            )
            return None
    ...
    async def _cleanup_old_executions_internal_async(
        self, cutoff_date: datetime
    ) -> int:
        """Async internal cleanup method."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._cleanup_old_executions_internal_sync, cutoff_date
        )

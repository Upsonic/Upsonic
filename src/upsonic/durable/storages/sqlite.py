...
class SQLiteDurableStorage(DurableExecutionStorage):
    ...
    async def save_state_async(self, execution_id: str, state: ExecutionState) -> None:
        """
    ...
    async def _save_state_internal_async(
        self, execution_id: str, state: ExecutionState, state_json: str
    ):
        """Async internal save method."""
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None, self._save_state_internal_sync, execution_id, state, state_json
        )
    ...
    async def load_state_async(self, execution_id: str) -> Optional[ExecutionState]:
        """
    ...
    async def _load_state_internal_async(
        self, execution_id: str
    ) -> Optional[Dict[str, Any]]:
        """Async internal load method."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._load_state_internal_sync, execution_id
        )
    ...
    async def delete_state_async(self, execution_id: str) -> bool:
        """
    ...
    async def _delete_state_internal_async(self, execution_id: str) -> bool:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._delete_state_internal_sync, execution_id
        )
    ...
    async def _list_executions_internal_async(
        self, status: Optional[str], limit: Optional[int]
    ) -> List[Dict[str, Any]]:
        """Async internal list method."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._list_executions_internal_sync, status, limit
        )
    ...
    async def cleanup_old_executions_async(self, older_than_days: int = 7) -> int:
        """
    ...
    async def _cleanup_old_executions_internal_async(self, cutoff_str: str) -> int:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._cleanup_old_executions_internal_sync, cutoff_str
        )

...
class Team:
    ...
    def __init__(
        ...
        self.agents = agents
        self.tasks = (
            tasks if isinstance(tasks, list) else [tasks] if tasks is not None else []
        )
        self.model = model
        ...
        try:
            ...
            if loop.is_running():
                ...
                return asyncio.run_coroutine_threadsafe(
                    self.multi_agent_async(agent_configurations, tasks), loop
                ).result()
        except RuntimeError:
    ...
    async def multi_agent_async(self, agent_configurations: List[Agent], tasks: Any):
        ...
        if self.mode == "sequential":
            ...
            result_combiner = ResultCombiner(
                ...
                debug=self.agents[-1].debug if self.agents else False,
            )
            if not isinstance(tasks, list):
                tasks = [tasks]
            agents_registry, agent_names = task_assignment.prepare_agents_registry(
            ...
            for task_index, current_task in enumerate(tasks):
                ...
                if selected_agent_name:
                    ...
                    await agents_registry[selected_agent_name].do_async(current_task)
            ...
            if not self.model:
                raise ValueError(
                    f"A `model` must be set on the Team for '{self.mode}' mode."
                )
            tool_mapping = {}
            ...
            self.leader_agent = Agent(model=self.model, memory=self.memory)
            ...
            router_task = Task(
                description=router_task_description, tools=[routing_tool]
            )
            ...
            if not chosen_agent:
                raise ValueError(
                    "Routing failed: The router agent did not select a team member."
                )

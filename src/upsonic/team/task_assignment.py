...
class TaskAssignment:
    ...
    def prepare_agents_registry(
        self, agent_configurations: List[Any]
    ...
    async def select_agent_for_task(
        ...
        if current_task.agent is not None:
            ...
            predefined_agent_id = getattr(
                current_task.agent, "get_agent_id", lambda: None
            )()
        ...
        if not agent_configurations or not hasattr(agent_configurations[0], "model"):
            raise ValueError(
                "Cannot perform agent selection: The first agent in the team must have a valid model."
            )
        ...
        while attempts < max_attempts:
            ...
            for agent_name in agent_names:
                if (
                    agent_name.lower() in selected_name.lower()
                    or selected_name.lower() in agent_name.lower()
                ):
                    return agent_name

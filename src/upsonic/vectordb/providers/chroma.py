...
class ChromaProvider(BaseVectorDBProvider):
    ...
    def _validate_config(self) -> None:
        debug_log(
            "Performing Chroma-specific configuration validation...",
            context="ChromaVectorDB",
        )
        if self._config.core.provider_name is not ProviderName.CHROMA:
            raise ConfigurationError(
                f"Configuration Mismatch: ChromaProvider received a config for '{self._config.core.provider_name.value}'."
            )
        if self._config.indexing.index_config.index_type not in [
            IndexType.HNSW,
            IndexType.FLAT,
        ]:
            raise ConfigurationError(
                f"Unsupported Index: ChromaProvider does not support index type '{self._config.indexing.index_config.index_type.value}'. Please use HNSW or FLAT."
            )
        if (
            self._config.core.mode == Mode.EMBEDDED
            and self._config.core.db_path is None
        ):
            raise ConfigurationError(
                "Missing Path: 'db_path' must be set in CoreConfig for EMBEDDED (PersistentClient) mode."
            )
        info_log(
            "Chroma configuration validated successfully.", context="ChromaVectorDB"
        )
    ...
    def connect(self) -> None:
        if self._is_connected:
            return
        debug_log(
            f"Connecting to ChromaDB in '{self._config.core.mode.value}' mode...",
            context="ChromaVectorDB",
        )
        try:
            ...
            if self._config.core.mode == Mode.IN_MEMORY:
            ...
            elif self._config.core.mode == Mode.EMBEDDED:
                client_instance = chromadb.PersistentClient(
                    path=self._config.core.db_path
                )
            elif self._config.core.mode == Mode.LOCAL:
                if not self._config.core.host or not self._config.core.port:
                    raise ConfigurationError(
                        "Host and port must be specified for LOCAL mode."
                    )
                ...
                if not self._config.core.api_key:
                    raise ConfigurationError(
                        "api_key must be specified for CLOUD mode."
                    )

                # Prepare CloudClient kwargs
                cloud_kwargs = {"api_key": self._config.core.api_key.get_secret_value()}

                # Add tenant and database if provided in advanced config namespace
                if (
                    hasattr(self._config.advanced, "namespace")
                ...
                try:
                    ...
                    if not self._config.core.host:
                        raise ConfigurationError(
                            "CloudClient not available and no host specified for fallback HttpClient."
                        )

                    headers = {
                        "Authorization": f"Bearer {self._config.core.api_key.get_secret_value()}"
                    }
                    fallback_kwargs = {
                ...
                raise ConfigurationError(
                    f"Unsupported mode for ChromaProvider: {self._config.core.mode}"
                )
            ...
            info_log(
                "ChromaDB connection successful and verified.", context="ChromaVectorDB"
            )
    ...
    def disconnect(self) -> None:
        ...
        try:
            ...
            self._client, self._is_connected, self._collection_instance = (
                None,
                False,
                None,
            )
            info_log(
                "ChromaDB client session has been reset.", context="ChromaVectorDB"
            )
    ...
    def create_collection(self) -> None:
        if not self.is_ready():
            raise VectorDBConnectionError(
                "Cannot create collection: Provider is not connected or ready."
            )
        collection_name = self._config.core.collection_name
        try:
            if self._config.core.recreate_if_exists and self.collection_exists():
                info_log(
                    f"Configuration specifies 'recreate_if_exists'. Deleting existing collection '{collection_name}'...",
                    context="ChromaVectorDB",
                )
                self.delete_collection()
            chroma_metadata = self._translate_config_to_chroma_metadata()
            debug_log(
                f"Creating or retrieving collection '{collection_name}'...",
                context="ChromaVectorDB",
            )
            ...
            info_log(
                f"Successfully prepared collection '{collection_name}'.",
                context="ChromaVectorDB",
            )
        except Exception as e:
            raise VectorDBError(
    ...
    def _translate_config_to_chroma_metadata(self) -> dict:
        ...
        if isinstance(self._config.indexing.index_config, HNSWTuningConfig):
            ...
            metadata["hnsw:construction_ef"] = (
                self._config.indexing.index_config.ef_construction
            )
        return metadata
    ...
    def delete_collection(self) -> None:
        if not self.is_ready():
            raise VectorDBConnectionError(
                "Cannot delete collection: Provider is not connected or ready."
            )
        collection_name = self._config.core.collection_name
        debug_log(
    ...
    def collection_exists(self) -> bool:
        if not self.is_ready():
            raise VectorDBConnectionError(
                "Cannot check for collection: Provider is not connected or ready."
            )
        try:
    ...
    def upsert(
        ...
        **kwargs,
        ...
        debug_log(
            f"Upserting {len(ids)} records into collection '{collection.name}'...",
            context="ChromaVectorDB",
        )
        try:
            upsert_params = {
            ...
            if chunks is not None:
                if len(chunks) != len(ids):
                    raise UpsertError(
                        f"Number of documents ({len(chunks)}) must match number of IDs ({len(ids)})"
                    )
                upsert_params["documents"] = chunks
            collection.upsert(**upsert_params)
            info_log(
                f"Successfully upserted {len(ids)} records.", context="ChromaVectorDB"
            )
        except Exception as e:
            raise UpsertError(
    ...
    def delete(self, ids: List[Union[str, int]], **kwargs) -> None:
        ...
        collection = self._get_active_collection()
        debug_log(
            f"Deleting {len(ids)} records from collection '{collection.name}'...",
            context="ChromaVectorDB",
        )
        try:
            collection.delete(ids=[str(i) for i in ids])
            info_log(
                f"Successfully deleted {len(ids)} records.", context="ChromaVectorDB"
            )
        except Exception as e:
            raise VectorDBError(
    ...
    def fetch(self, ids: List[Union[str, int]], **kwargs) -> List[VectorSearchResult]:
        ...
        collection = self._get_active_collection()
        debug_log(
            f"Fetching {len(ids)} records from collection '{collection.name}'...",
            context="ChromaVectorDB",
        )
        try:
            ...
            info_log(
                f"Successfully fetched {len(final_results)} records.",
                context="ChromaVectorDB",
            )
            ...
            raise VectorDBError(
                f"Failed to fetch records from collection '{collection.name}': {e}"
            ) from e
        ...
        **kwargs,
        ...
        if is_hybrid:
            if not self._config.search.hybrid_search_enabled:
                raise ConfigurationError(
                    "Hybrid search is not enabled for this provider."
                )
            ...
            if not self._config.search.dense_search_enabled:
                raise ConfigurationError(
                    "Dense search is not enabled for this provider."
                )
            ...
            if not self._config.search.full_text_search_enabled:
                raise ConfigurationError(
                    "Full-text search is not enabled for this provider."
                )
            ...
            raise ValueError(
                "Invalid search arguments: Please provide 'query_vector', 'query_text', or both."
            )
    ...
    def dense_search(
        ...
        **kwargs,
        ...
        final_similarity_threshold = (
            ...
            else self._config.search.default_similarity_threshold or 0.5
        )
        ...
        try:
            ...
            vectors = (
                results["embeddings"][0] if results["embeddings"] else [None] * len(ids)
            )
            chunks = (
            ...
            for i in range(len(ids)):
                if self._config.core.distance_metric == DistanceMetric.COSINE:
                    ...
                    score = min(
                        1.0,
                        max(0.0, 1 - distances[i] / max_dist if max_dist > 0 else 1.0),
                    )
    ...
    def full_text_search(
        ...
        **kwargs,
        ...
        final_where = (
            {"$and": [filter, where_document_filter]}
            if filter
            else where_document_filter
        )
        try:
            results = collection.get(
                where=final_where,
    ...
    def _reciprocal_rank_fusion(
        self, results_lists: List[List[VectorSearchResult]], k: int = 60
    ) -> dict:
        fused_scores = {}
    ...
    def _weighted_fusion(
        ...
        ft_results: List[VectorSearchResult],
        alpha: float,
    ) -> dict:
        fused_scores = {}
        ...
        **kwargs,
        ...
        try:
            ...
            dense_results = self.dense_search(
                query_vector, candidate_k, filter, similarity_threshold, **kwargs
            )
            ft_results = self.full_text_search(
                query_text, candidate_k, filter, similarity_threshold, **kwargs
            )
            ...
            if fusion_method == "rrf":
                ...
                fused_scores = self._weighted_fusion(
                    dense_results, ft_results, final_alpha
                )
            else:

...
warnings.filterwarnings(
    "ignore", category=UserWarning, message=".*Cannot infer schema from data.*"
)
...
class MilvusProvider(BaseVectorDBProvider):
    ...
    def _validate_config(self) -> None:
        ...
        if (
            ...
            and self._config.core.vector_size is None
        ):
            raise ConfigurationError(
    ...
    def _get_connection_alias(self) -> str:
        ...
        db_name = (
            db_path.split("/")[-1].replace(".db", "") if "/" in db_path else db_path
        )
    ...
    def _build_connection_params(self) -> Dict[str, Any]:
        ...
        if mode in [Mode.EMBEDDED, Mode.IN_MEMORY]:
            ...
            if not self._config.core.host:
                raise ConfigurationError(
                    f"A `host` is required for '{mode.value}' mode."
                )
            ...
            if self._config.core.host.startswith("http"):
                ...
                port = self._config.core.port or (
                    443 if self._config.core.use_tls else 19530
                )
            ...
            if self._config.core.api_key:
                ...
                if ":" in api_key_value:
                    if (
                        api_key_value.startswith("db_")
                        or len(api_key_value.split(":")) == 2
                    ):
            ...
            if (
                hasattr(self._config, "advanced")
                and hasattr(self._config.advanced, "namespace")
                and self._config.advanced.namespace
            ):
            ...
            raise ConfigurationError(
                f"Unsupported mode '{mode.value}' for MilvusProvider."
            )
    ...
    def _build_field_schema(self) -> List[FieldSchema]:
        ...
        chunk_field = FieldSchema(
            name="chunk", dtype=DataType.VARCHAR, max_length=65535
        )
        ...
        if self._config.indexing.payload_indexes:
            ...
            for payload_index in self._config.indexing.payload_indexes:
                ...
                if field_name in reserved_names:
                    warnings.warn(
                        f"Payload field name '{field_name}' is reserved and will be ignored."
                    )
                    continue
                ...
                if field_type == DataType.UNKNOWN:
                    warnings.warn(
                        f"Payload field '{field_name}' has an unsupported schema type "
    ...
    def connect(self) -> None:
        ...
        try:
            debug_log(
                f"Attempting to connect with alias '{alias}' and params: {params}",
            ...
            try:
                if utility.has_collection(self._sanitized_collection_name, using=alias):
                    self._collection = Collection(
                        name=self._sanitized_collection_name, using=alias
                    )
                    info_log(
                        f"Attached to existing collection '{self._sanitized_collection_name}'.",
                        context="MilvusVectorDB",
                    )
    ...
    def disconnect(self) -> None:
        ...
        try:
            ...
            success_log(
                f"MilvusProvider: Disconnected from alias '{alias}'.", "MilvusProvider"
            )
        except MilvusException as e:
    ...
    def collection_exists(self) -> bool:
        ...
        try:
            return utility.has_collection(
                self._sanitized_collection_name, using=self._get_connection_alias()
            )
        except MilvusException as e:
    ...
    def create_collection(self) -> None:
        ...
        if self.collection_exists():
            if self._config.core.recreate_if_exists:
                info_log(
                    f"Collection '{collection_name}' exists and will be recreated.",
                    context="MilvusVectorDB",
                )
        ...
        try:
            ...
            schema = CollectionSchema(
                fields,
                description=f"Upsonic collection: {collection_name}",
                enable_dynamic_field=True,
            )
            consistency_level = self._CONSISTENCY_MAP[
            ...
            info_log(
                f"Collection '{collection_name}' created successfully with dynamic schema.",
                context="MilvusVectorDB",
            )
            ...
            if self._config.indexing.create_dense_index:
                ...
                debug_log(
                    f"Creating dense vector index on field 'dense_vector' with params: {index_params}",
                    context="MilvusVectorDB",
                )
                ...
                info_log(
                    "Dense vector index created successfully.", context="MilvusVectorDB"
                )
            ...
            if self._config.indexing.create_sparse_index:
                ...
                debug_log(
                    f"Creating sparse vector index on field 'sparse_vector' with params: {sparse_index_params}",
                    context="MilvusVectorDB",
                )
                self._collection.create_index(
                ...
                info_log(
                    "Sparse vector index created successfully.",
                    context="MilvusVectorDB",
                )
            ...
            if self._config.indexing.payload_indexes:
                for payload_config in self._config.indexing.payload_indexes:
                    if payload_config.enable_full_text_index:
                        ...
                        debug_log(
                            f"Creating full-text index on payload field '{field_name}'.",
                            context="MilvusVectorDB",
                        )
                        self._collection.create_index(
                            field_name=field_name,
                            index_params={"index_type": "FULL_TEXT"},
                        )
                        info_log(
                            f"Full-text index on '{field_name}' created successfully.",
                            context="MilvusVectorDB",
                        )
            ...
            info_log(
                "Collection loaded successfully and is ready for operations.",
                context="MilvusVectorDB",
            )
            ...
            self._collection = None
            raise VectorDBError(
                f"Failed to create and initialize collection '{collection_name}': {e}"
            )
    ...
    def _ensure_collection_handle(self) -> None:
        ...
        if self._collection is None:
            raise CollectionDoesNotExistError(
                f"Collection '{self._sanitized_collection_name}' handle is not loaded. Was it created?"
            )
    ...
    def upsert(
        ...
        **kwargs,
        ...
        if not ids:
            debug_log(
                "Upsert called with empty 'ids' list. No action taken.",
                context="MilvusVectorDB",
            )
            return
        ...
        if len(payloads) != num_ids:
            raise UpsertError(
                f"Length mismatch: `ids` has {num_ids} elements while `payloads` has {len(payloads)}."
            )

        if self._config.indexing.create_dense_index:
            if vectors is None:
                raise UpsertError(
                    "Collection requires dense vectors, but `vectors` argument was None."
                )
            if len(vectors) != num_ids:
                raise UpsertError(
                    f"Length mismatch: `ids` has {num_ids} elements while `vectors` has {len(vectors)}."
                )

        if self._config.indexing.create_sparse_index:
            if sparse_vectors is None:
                raise UpsertError(
                    "Collection requires sparse vectors, but `sparse_vectors` kwarg was not provided."
                )
            if len(sparse_vectors) != num_ids:
                raise UpsertError(
                    f"Length mismatch: `ids` has {num_ids} elements while `sparse_vectors` has {len(sparse_vectors)}."
                )
        ...
        try:
            ...
            info_log(
                f"Starting upsert of {total} entities in batches of {batch_size}.",
                context="MilvusVectorDB",
            )
            ...
            for i in range(0, total, batch_size):
                ...
                debug_log(
                    f"Upserting batch {i // batch_size + 1}/{-(-total // batch_size)}, size: {len(batch)}",
                    context="MilvusVectorDB",
                )
            ...
            info_log(
                "Upsert complete and data flushed successfully.",
                context="MilvusVectorDB",
            )
            ...
            raise UpsertError(
                f"An unexpected error occurred during data preparation for upsert: {e}"
            )
    ...
    def delete(self, ids: List[Union[str, int]], **kwargs) -> None:
        ...
        try:
            debug_log(
                f"Deleting {len(ids)} entities with expression: {expr}",
                context="MilvusVectorDB",
            )
        ...
        try:
            ...
            for res in results:
                payload = {
                    ...
                    if k not in ["id", "dense_vector", "sparse_vector", "chunk"]
                }
    ...
    def _build_filter_expression(
        ...
        for field, value in filter_dict.items():
            if isinstance(value, dict):
                if len(value) != 1:
                    raise SearchError(
                        f"Filter for field '{field}' must have exactly one operator."
                    )
                ...
                if op == "$in":
                    ...
                    raise SearchError(
                        f"Unsupported filter operator '{op}' for field '{field}'."
                    )
            else:
    ...
    def search(
        ...
        **kwargs,
        ...
        if k is None:
            raise ConfigurationError(
                "`top_k` must be provided for search or set as `default_top_k`."
            )
        ...
        if is_hybrid:
            ...
            return self.dense_search(
                query_vector, k, final_filter, similarity_threshold, **kwargs
            )
        elif is_full_text:
            ...
            return self.full_text_search(
                query_text, k, final_filter, similarity_threshold, **kwargs
            )
        else:
            raise SearchError(
    ...
    def dense_search(
        ...
        **kwargs,
        ...
        if not self._config.indexing.create_dense_index:
            raise ConfigurationError(
                "Dense search is not possible; no dense index was created for this collection."
            )
        self._ensure_collection_handle()
        ...
        index_cfg = self._config.indexing.index_config
        if isinstance(index_cfg, HNSWTuningConfig):
            params["params"]["ef"] = kwargs.get(
                "ef_search", self._config.search.default_ef_search or 128
            )
        elif isinstance(index_cfg, IVFTuningConfig):
            params["params"]["nprobe"] = kwargs.get(
                "nprobe", self._config.search.default_nprobe or 10
            )
        ...
        try:
            if self._config.indexing.create_sparse_index:
                ...
                if query_sparse_vector is None:
                    raise SearchError(
                        "`full_text_search` on a sparse-indexed collection requires `query_sparse_vector` in kwargs."
                    )
                ...
                if not search_field:
                    raise ConfigurationError(
                        "`full_text_search` on this collection requires `search_field` in kwargs."
                    )

                sanitized_query_text = query_text.replace("'", "''")
                text_expr = f"{search_field} like '%{sanitized_query_text}%'"
                final_expr = (
                    f"({text_expr}) and ({filter_expr})" if filter_expr else text_expr
                )

                hits = self._collection.query(
                    expr=final_expr, limit=top_k, output_fields=["*"]
                )
                filtered_results = []
                for hit in hits:
            ...
            for hit in results[0]:
                payload = {
                    ...
                    if k not in ["id", "dense_vector", "sparse_vector"]
                }
                ...
                if score >= final_similarity_threshold:
                    search_results.append(
                        VectorSearchResult(
                            ...
                            score=score,
                            payload=payload,
                            vector=hit.entity.get("sparse_vector"),
                        )
                    )
            return search_results
    ...
    def hybrid_search(
        ...
        **kwargs,
        ...
        if not (
            ...
            raise ConfigurationError(
                "Hybrid search is only possible on collections with both dense and sparse indexes."
            )
        ...
        fusion = (
            fusion_method or self._config.search.default_fusion_method or "weighted"
        )
        final_alpha = (
            alpha
        ...
        if isinstance(self._config.indexing.index_config, HNSWTuningConfig):
            dense_params["params"]["ef"] = kwargs.get(
                "ef_search", self._config.search.default_ef_search or 128
            )
        dense_req_params = {
            "data": [query_vector],
        ...
        try:
            ...
            for hit in results[0]:
                payload = {
                    k: v
                    for k, v in hit.entity["entity"].items()
                    if k not in ["id", "dense_vector", "sparse_vector", "chunk"]
                }

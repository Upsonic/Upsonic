...
class FaissProvider(BaseVectorDBProvider):
    ...
    **Concurrency Warning:** This implementation is NOT thread-safe or process-safe.
    ...
    def __init__(self, config: Config):
        ...
        if config.core.provider_name.value != "faiss":
        ...
        self._base_db_path: Optional[Path] = (
            Path(self._config.core.db_path) if self._config.core.db_path else None
        )
        self._normalize_vectors = (
            self._config.core.distance_metric == DistanceMetric.COSINE
        )
            ...
            if db_path:
                ...
                if id_map_file.exists():
                    with open(id_map_file, "r") as f:
                        ...
                        self._faiss_id_to_user_id = {
                            int(k): v for k, v in maps.get("faiss_to_user", {}).items()
                        }
            self._is_connected = True
        except Exception as e:
            raise VectorDBConnectionError(
                f"Failed to hydrate FAISS state from disk: {e}"
            )
    ...
    def disconnect(self) -> None:
        ...
        if not db_path:
            debug_log(
                "Running in 'in_memory' mode. Clearing state without persisting.",
            ...
            try:
                ...
                with open(db_path / "id_map.json", "w") as f:
                    json.dump(
                        {
                            "user_to_faiss": self._user_id_to_faiss_id,
                            "faiss_to_user": self._faiss_id_to_user_id,
                        },
                        f,
                    )
            except Exception as e:
                warning_log(
                    f"Failed to persist FAISS state to disk: {e}",
                    context="FaissVectorDB",
                )
        self._index = None
        self._metadata_store = {}
        self._user_id_to_faiss_id = {}
        self._faiss_id_to_user_id = {}
    ...
    def create_collection(self) -> None:
        ...
        if not self._is_connected:
            raise VectorDBConnectionError(
                "Must be connected before creating a collection."
            )
        if self._index is not None:
            info_log(
                "Collection (FAISS index) already exists in memory.",
                context="FaissVectorDB",
            )
            return
        ...
        if self.collection_exists():
            if self._config.core.recreate_if_exists:
                info_log(
                    "Deleting existing collection on disk to recreate.",
                    context="FaissVectorDB",
                )
                self.delete_collection()
            else:
                info_log(
                    "Collection path exists but index not loaded. Proceeding to create new index.",
                    context="FaissVectorDB",
                )
        ...
        try:
            debug_log(
                f"Creating FAISS index with factory string: '{factory_string}' and dimension: {d}",
                context="FaissVectorDB",
            )
            ...
            raise VectorDBError(
                f"Failed to create FAISS index with factory string '{factory_string}': {e}"
            )
    ...
    def delete_collection(self) -> None:
        ...
        if not self._active_db_path:
            debug_log(
                "Cannot delete collection in 'in_memory' mode.", context="FaissVectorDB"
            )
            return
        ...
        if self.collection_exists():
            try:
                ...
                info_log(
                    f"Successfully deleted collection directory: '{self._active_db_path}'",
                    context="FaissVectorDB",
                )
            except OSError as e:
                raise VectorDBError(
                    f"Error deleting collection directory '{self._active_db_path}': {e}"
                )
        else:
            debug_log(
                "Collection directory does not exist. No action taken.",
                context="FaissVectorDB",
            )
    ...
    def upsert(
        ...
        **kwargs,
        ...
        if not self.is_ready():
            raise VectorDBError(
                "FAISS index is not created. Please call 'create_collection' first."
            )
        ...
        ids_to_update = [
            user_id for user_id in ids if user_id in self._user_id_to_faiss_id
        ]
        if ids_to_update:
            debug_log(
                f"Updating {len(ids_to_update)} existing IDs by deleting old entries first.",
                context="FaissVectorDB",
            )
        ...
        if not self._index.is_trained:
            debug_log(
                f"FAISS index is not trained. Training on {len(vectors_np)} vectors...",
                context="FaissVectorDB",
            )
        ...
        try:
            ...
            info_log(
                f"Successfully upserted {len(vectors)} vectors. Index total: {self._index.ntotal}",
                context="FaissVectorDB",
            )
    ...
    def delete(self, ids: List[Union[str, int]], **kwargs) -> None:
        ...
        if deleted_count > 0:
            info_log(
                f"Successfully marked {deleted_count} IDs for deletion (tombstoned).",
                context="FaissVectorDB",
            )
        ...
        for user_id in ids:
            if user_id in self._user_id_to_faiss_id:
                try:
                    ...
                    debug_log(
                        f"Could not fetch data for ID '{user_id}': {e}",
                        context="FaissVectorDB",
                    )
        return results
    ...
    def _translate_filter(
        ...
        def build_checker(key, value):
            if isinstance(value, dict):
                ...
                if op == "$in":
                    return lambda payload: payload.get(key) in val
                if op == "$gte":
                    return lambda payload: payload.get(key, float("-inf")) >= val
                if op == "$lte":
                    return lambda payload: payload.get(key, float("inf")) <= val
                if op == "$gt":
                    return lambda payload: payload.get(key, float("-inf")) > val
                if op == "$lt":
                    return lambda payload: payload.get(key, float("inf")) < val
                if op == "$ne":
                    return lambda payload: payload.get(key) != val
            return lambda payload: payload.get(key) == value

        if "and" in filter_dict:
            checkers = [
                self._translate_filter(sub_filter) for sub_filter in filter_dict["and"]
            ]
            return lambda payload: all(checker(payload) for checker in checkers)
        if "or" in filter_dict:
            checkers = [
                self._translate_filter(sub_filter) for sub_filter in filter_dict["or"]
            ]
        ...
        **kwargs,
        ...
        if has_vector and has_text:
            ...
            return self.dense_search(
                query_vector, final_top_k, filter, similarity_threshold, **kwargs
            )
        elif has_text:
            return self.full_text_search(
                query_text, final_top_k, filter, similarity_threshold, **kwargs
            )
        else:
            raise ConfigurationError(
        ...
        **kwargs,
        ...
        for dist, faiss_id in zip(distances[0], faiss_ids[0]):
            ...
            if self._config.core.distance_metric == DistanceMetric.EUCLIDEAN:
                ...
                score = float(
                    dist
                )  # For dot product, distance is already similarity-like
            else:
            ...
            if score >= final_similarity_threshold:
                ...
                results.append(
                    VectorSearchResult(
                        id=user_id, score=score, payload=payload, text=text
                    )
                )
    ...
    def hybrid_search(
        ...
        **kwargs,
        ...
        warning_log(
            "FAISS provider received a hybrid search request. It will ignore the text query and alpha, performing a dense search instead.",
            context="FaissVectorDB",
        )
        return self.dense_search(

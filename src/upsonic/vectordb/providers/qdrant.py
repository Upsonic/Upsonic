...
class QdrantProvider(BaseVectorDBProvider):
    ...
    def connect(self) -> None:
        ...
        try:
            if core_cfg.mode == Mode.IN_MEMORY:
                ...
                self._client = QdrantClient(
                    url=url,
                    api_key=core_cfg.api_key.get_secret_value()
                    if core_cfg.api_key
                    else None,
                )
            else:
                raise ConfigurationError(
                    f"Unsupported mode for Qdrant: {core_cfg.mode.value}"
                )
    ...
    def create_collection(self) -> None:
        ...
        if not self._is_connected or not self._client:
            raise VectorDBConnectionError(
                "Must be connected to Qdrant to create a collection."
            )
        ...
        try:
            if self._config.core.recreate_if_exists and self.collection_exists():
                info_log(
                    f"Collection '{collection_name}' exists and `recreate_if_exists` is True. Deleting...",
            ...
            if isinstance(index_cfg, HNSWTuningConfig):
                hnsw_config = models.HnswConfigDiff(
                    m=index_cfg.m, ef_construct=index_cfg.ef_construction
                )
            ...
            info_log(
                f"Successfully created or configured collection '{collection_name}'.",
                context="QdrantVectorDB",
            )
    ...
    def delete_collection(self) -> None:
        ...
        if not self._is_connected or not self._client:
            raise VectorDBConnectionError(
                "Must be connected to Qdrant to delete a collection."
            )

        collection_name = self._config.core.collection_name
        try:
            operation_info = self._client.delete_collection(
                collection_name=collection_name
            )
            if type(operation_info) is not bool:
                if not operation_info.result:
                    if not self.collection_exists():
                        ...
                        raise VectorDBError(
                            f"Failed to delete collection '{collection_name}' but it still exists."
                        )
            else:
            ...
            raise VectorDBError(
                f"API error while deleting collection '{collection_name}': {e}"
            ) from e
        except Exception as e:
            raise VectorDBError(
                f"An unexpected error occurred while deleting collection: {e}"
            ) from e
    ...
    def collection_exists(self) -> bool:
        ...
        if not self._is_connected or not self._client:
            raise VectorDBConnectionError(
                "Must be connected to Qdrant to check for a collection."
            )
        ...
        try:
            ...
            if e.status_code == 404:
                return False
            raise VectorDBError(
                f"API error while checking for collection '{collection_name}': {e}"
            ) from e
        except Exception:
            return False
    ...
    def upsert(
        ...
        **kwargs,
        ...
        if chunks is not None and len(chunks) != len(vectors):
            raise ValueError(
                "The length of the chunks list must be identical to the other lists."
            )
        ...
        wait_for_result = (
            self._config.data_management.write_consistency == WriteConsistency.STRONG
        )
        ...
        try:
            ...
            raise UpsertError(
                f"Failed to upsert data into collection '{self._config.core.collection_name}': {e}"
            ) from e
    ...
    def delete(self, ids: List[Union[str, int]], **kwargs) -> None:
        ...
        try:
            ...
            raise VectorDBError(
                f"Failed to delete points from collection '{self._config.core.collection_name}': {e}"
            ) from e
    ...
    def fetch(self, ids: List[Union[str, int]], **kwargs) -> List[VectorSearchResult]:
        ...
        try:
            ...
            raise VectorDBError(
                f"Failed to fetch points from collection '{self._config.core.collection_name}': {e}"
            ) from e
        ...
        **kwargs,
        ...
        if is_dense:
            ...
            return self.dense_search(
                query_vector, effective_top_k, filter, similarity_threshold, **kwargs
            )
            ...
            return self.full_text_search(
                query_text, effective_top_k, filter, similarity_threshold, **kwargs
            )

        else:
            raise SearchError(
                "Invalid search query: You must provide a 'query_vector' and/or 'query_text'."
            )
        ...
        try:
            ...
            for point in query_response.points:
                ...
                if self._config.core.distance_metric == DistanceMetric.COSINE:
                    ...
                    max_distance = (
                        1.0 / final_similarity_threshold
                        if final_similarity_threshold > 0
                        else float("inf")
                    )
    ...
    def full_text_search(
        ...
        **kwargs,
        ...
        if not self._is_connected or not self._client:
            raise VectorDBConnectionError(
                "Must be connected to perform a full-text search."
            )
        ...
        if self._config.core.mode == Mode.IN_MEMORY:
            try:
                ...
                raise SearchError(
                    f"An error occurred during full-text search: {e}"
                ) from e
        else:
            try:
            ...
            text_condition = models.FieldCondition(
                key=target_text_field, match=models.MatchText(text=query_text)
            )
    ...
    def hybrid_search(
        ...
        **kwargs,
        ...
        dense_results = self.dense_search(
            query_vector, top_k, filter, similarity_threshold, **kwargs
        )
        ft_results = self.full_text_search(
            query_text, top_k, filter, similarity_threshold, **kwargs
        )

        if effective_fusion == "weighted":
            fused_results = self._fuse_weighted(
                dense_results, ft_results, effective_alpha
            )
                    ...
                    if op == "$gte":
                        conditions.append(
                            models.FieldCondition(
                                key=key, range=models.Range(gte=op_value)
                            )
                        )
                    elif op == "$lte":
                        conditions.append(
                            models.FieldCondition(
                                key=key, range=models.Range(lte=op_value)
                            )
                        )
                    elif op == "$gt":
                        conditions.append(
                            models.FieldCondition(
                                key=key, range=models.Range(gt=op_value)
                            )
                        )
                    elif op == "$lt":
                        conditions.append(
                            models.FieldCondition(
                                key=key, range=models.Range(lt=op_value)
                            )
                        )
                    elif op == "$in":
                        conditions.append(
    ...
    def _create_payload_indexes(self, collection_name: str) -> None:
        ...
        for index_config in self._config.indexing.payload_indexes:
            ...
            if schema_type not in schema_map:
                warning_log(
                    f"Unknown payload index schema type '{schema_type}'. Skipping field '{field_name}'.",
                    context="QdrantVectorDB",
                )
                continue
            ...
            try:
                ...
                debug_log(
                    f"Creating index for field '{field_name}' of type '{schema_type}'...",
                    context="QdrantVectorDB",
                )
                self._client.create_payload_index(
                    collection_name=collection_name,

...
class PgvectorProvider(BaseVectorDBProvider):
    ...
    def __init__(self, config: Config):
        ...
        info_log(
            f"PgvectorProvider initialized for table '{self._config.core.collection_name}'.",
            context="PgVectorDB",
        )
    ...
    def connect(self) -> None:
        ...
        debug_log(
            f"Attempting to connect to PostgreSQL at {self._config.core.host}:{self._config.core.port}...",
            context="PgVectorDB",
        )
        try:
            conninfo = (
            ...
            with self._connection.cursor() as cursor:
                debug_log(
                    "Verifying 'vector' extension exists in PostgreSQL...",
                    context="PgVectorDB",
                )
    ...
    def disconnect(self) -> None:
        if self._is_connected and self._connection:
            try:
                ...
                self._connection = None
                info_log(
                    "Successfully disconnected from PostgreSQL.", context="PgVectorDB"
                )
            except Exception as e:
                self._is_connected = False
                self._connection = None
    ...
    def create_collection(self) -> None:
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL before creating a collection."
            )
        ...
        try:
            if self.collection_exists():
                if self._config.core.recreate_if_exists:
                    info_log(
                        f"Table '{table_name}' already exists. Deleting and recreating as requested.",
                        context="PgVectorDB",
                    )
                    self.delete_collection()
                else:
                    info_log(
                        f"Table '{table_name}' already exists and 'recreate_if_exists' is False. No action taken.",
                        context="PgVectorDB",
                    )
                    return
            ...
            with self._connection.cursor() as cursor:
                ...
                tenant_index_sql = SQL("CREATE INDEX ON {table} (tenant_id);").format(
                    table=Identifier(table_name)
                )
                cursor.execute(tenant_index_sql)

                payload_index_sql = SQL(
                    "CREATE INDEX ON {table} USING gin (payload);"
                ...
                if isinstance(index_conf, (HNSWTuningConfig, IVFTuningConfig)):
                    ...
                    if not operator_class:
                        raise ConfigurationError(
                            f"Unsupported distance metric '{self._config.core.distance_metric}' for pgvector."
                        )

                    if isinstance(index_conf, HNSWTuningConfig):
                        index_type, with_opts = (
                            "hnsw",
                            f"m = {index_conf.m}, ef_construction = {index_conf.ef_construction}",
                        )
                    else:
                    ...
                    """).format(
                        table=Identifier(table_name),
                        index_type=SQL(index_type),
                        operator_class=SQL(operator_class),
                        with_opts=SQL(with_opts),
                    )
                    debug_log(
                        f"Creating {index_type.upper()} index on 'vector' column...",
                        context="PgVectorDB",
                    )
                    ...
                    debug_log(
                        "Index type is FLAT. No vector index created.",
                        context="PgVectorDB",
                    )

            self._connection.commit()
            info_log(
                f"Successfully created table '{table_name}' and its indexes.",
                context="PgVectorDB",
            )
    ...
    def delete_collection(self) -> None:
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL before deleting a collection."
            )
        ...
        try:
            with self._connection.cursor() as cursor:
                delete_sql = SQL("DROP TABLE IF EXISTS {table} CASCADE;").format(
                    table=Identifier(table_name)
                )
            ...
            self._connection.commit()
            info_log(
                f"Successfully deleted table '{table_name}'.", context="PgVectorDB"
            )
        except (Exception, psycopg.Error) as e:
            if self._connection:
                self._connection.rollback()
    ...
    def collection_exists(self) -> bool:
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL to check for a collection's existence."
            )
        ...
        try:
            ...
            if isinstance(e, InFailedSqlTransaction) and self._connection:
                self._connection.rollback()
            debug_log(
                f"Could not check for collection existence: {e}", context="PgVectorDB"
            )
            return False
    ...
    def upsert(
        ...
        **kwargs,
        ...
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL before upserting data."
            )
        ...
        if not vectors:
            debug_log(
                "Upsert called with empty lists. No action taken.", context="PgVectorDB"
            )
            return
        ...
        if not tenant_id:
            raise ConfigurationError(
                "A `namespace` must be provided in the AdvancedConfig for pgvector multi-tenancy."
            )
        ...
        try:
            ...
            with self._connection.cursor() as cursor:
                debug_log(
                    f"Upserting {len(data_to_insert)} records into '{table_name}'...",
                    context="PgVectorDB",
                )
    ...
    def delete(self, ids: List[Union[str, int]], **kwargs) -> None:
        ...
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL before deleting data."
            )
        if not ids:
            debug_log(
        ...
        try:
            delete_sql = SQL(
                "DELETE FROM {table} WHERE tenant_id = %s AND id = ANY(%s);"
            ).format(table=Identifier(table_name))

            with self._connection.cursor() as cursor:
                cursor.execute(delete_sql, (tenant_id, [str(id_val) for id_val in ids]))
                debug_log(
                    f"Delete command affected {cursor.rowcount} rows in '{table_name}'.",
                    context="PgVectorDB",
                )
    ...
    def fetch(self, ids: List[Union[str, int]], **kwargs) -> List[VectorSearchResult]:
        ...
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL before fetching data."
            )
        if not ids:
            return []
        ...
        try:
            fetch_sql = SQL(
                "SELECT id, payload, vector FROM {table} WHERE tenant_id = %s AND id = ANY(%s);"
            ).format(table=Identifier(table_name))
    ...
    def search(
        ...
        fusion_method: Optional[TypingLiteral["rrf", "weighted"]] = None,
    ...
    def dense_search(
        ...
        **kwargs,
        ...
        if not self._is_connected or not self._connection:
            raise VectorDBConnectionError(
                "Must be connected to PostgreSQL before searching."
            )

        tenant_id = self._config.advanced.namespace
        if not tenant_id:
            raise ConfigurationError(
                "A `namespace` must be provided for pgvector multi-tenancy."
            )
        ...
        metric_map = {
            DistanceMetric.COSINE: (
                SQL("<=>"),
                SQL("1 - (vector <=> %s::vector) AS score"),
                "ASC",
            ),
            DistanceMetric.EUCLIDEAN: (
                SQL("<->"),
                SQL("vector <-> %s::vector AS score"),
                "ASC",
            ),
            DistanceMetric.DOT_PRODUCT: (
                SQL("<#>"),
                SQL("(vector <#> %s::vector) * -1 AS score"),
                "DESC",
            ),
        }
        ...
        if metric not in metric_map:
            raise ConfigurationError(
                f"Unsupported distance metric for dense search: {metric}"
            )
    ...
    def full_text_search(
        ...
        **kwargs,
        ...
        if not fts_field:
            raise ConfigurationError(
                "`fts_field` must be provided in kwargs for full_text_search."
            )
        ...
        query_parts = [
            SQL("SELECT id, payload, vector,"),
            SQL(
                "ts_rank(to_tsvector('english', payload->>%s), plainto_tsquery('english', %s)) AS score"
            ),
            SQL(
                "FROM {table} WHERE to_tsvector('english', payload->>%s) @@ plainto_tsquery('english', %s) AND tenant_id = %s"
            ).format(table=Identifier(table_name)),
        ]
    ...
    def hybrid_search(
        ...
        **kwargs,
        ...
        dense_results = self.dense_search(
            query_vector, candidate_pool_size, filter, similarity_threshold, **kwargs
        )

        keyword_results = self.full_text_search(
            query_text, candidate_pool_size, filter, similarity_threshold, **kwargs
        )
        ...
        for field, value in filter_dict.items():
            ...
            if isinstance(value, dict):
                ...
                if op == "$gte":
                    ...
                    sql_parts.append(
                        field_sql + SQL("::text = ANY(") + Placeholder() + SQL(")")
                    )
                else:

...
try:
    ...
    from pinecone.exceptions import (
        PineconeApiException as ApiException,
        NotFoundException,
    )
...
class PineconeProvider(BaseVectorDBProvider):
    ...
    def _validate_configuration(self) -> None:
        ...
        """
        logger.debug(
            "Executing Phase 1: Performing deep Pinecone configuration validation..."
        )

        if self._config.core.provider_name != ProviderName.PINECONE:
            raise ConfigurationError(
                f"Fatal: Attempted to initialize PineconeProvider with a configuration for '{self._config.core.provider_name.value}'."
            )

        if not self._config.core.api_key:
            raise ConfigurationError(
                "Configuration Error: 'api_key' is mandatory for connecting to the Pinecone service."
            )

        if isinstance(self._config.indexing.index_config, IVFTuningConfig):
            raise ConfigurationError(
                "Configuration Error: Pinecone does not support IVF-based indexes. Please use 'HNSW' or 'FLAT' index types."
            )

        if self._config.indexing.quantization is not None:
            raise ConfigurationError(
                "Configuration Error: Pinecone manages quantization internally and does not support user-configurable quantization."
            )
        ...
        if search_cfg.hybrid_search_enabled:
            is_hybrid_ready = (
                indexing_cfg.create_dense_index and indexing_cfg.create_sparse_index
            )
        ...
        logger.debug(
            "Layer 2 validation passed: Search and Indexing configurations are consistent."
        )

        logger.info(
            "Phase 1 Succeeded: Pinecone configuration has been successfully and comprehensively validated."
        )
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            if self._config.indexing.create_dense_index:
                safe_collection_name = self._sanitize_collection_name(
        ...
        try:
            if self._config.indexing.create_dense_index:
                ...
                try:
                    ...
                    logger.debug(
                        f"Dense index '{dense_name}' does not exist or is not accessible."
                    )
                    return False
            ...
            if self._config.indexing.create_sparse_index:
                ...
                try:
                    ...
                    logger.debug(
                        f"Sparse index '{sparse_name}' does not exist or is not accessible."
                    )
                    return False
    ...
    def collection_exists(self) -> bool:
        ...
        if not self._is_connected or not self._client:
            raise Exception(
                "Must be connected to Pinecone to check if a collection exists."
            )
        try:
            existing_indexes = self._client.list_indexes().names()
            safe_collection_name = self._sanitize_collection_name(
            ...
            if (
                self._config.indexing.create_dense_index
                and self._config.indexing.create_sparse_index
            ):
    ...
    def delete_collection(self) -> None:
        ...
        if not self._is_connected or not self._client:
            raise VectorDBConnectionError(
                "Must be connected to Pinecone to delete collections."
            )

        collection_name = self._config.core.collection_name
        safe_collection_name = self._sanitize_collection_name(collection_name)
        logger.warning(
            f"Attempting to permanently delete collections for: '{collection_name}' (sanitized: '{safe_collection_name}')"
        )
        ...
        try:
            if self._config.indexing.create_dense_index:
                ...
                if dense_name in self._client.list_indexes().names():
                    ...
                    logger.info(
                        f"Dense index '{dense_name}' did not exist, so no deletion was necessary."
                    )
            ...
            if self._config.indexing.create_sparse_index:
                ...
                if sparse_name in self._client.list_indexes().names():
                    ...
                    logger.info(
                        f"Sparse index '{sparse_name}' did not exist, so no deletion was necessary."
                    )

        except ApiException as e:
            raise VectorDBError(
                f"Failed to delete Pinecone collections for '{collection_name}': {e}"
            ) from e
    ...
    def _create_dense_index(self) -> None:
        ...
        if dense_name in self._client.list_indexes().names():
            if self._config.core.recreate_if_exists:
                ...
                logger.info(
                    f"Dense index '{dense_name}' already exists. Skipping creation."
                )
                return
        ...
        try:
            create_params = {
                ...
                "spec": ServerlessSpec(
                    cloud=self._config.core.cloud, region=self._config.core.region
                ),
    ...
    def _create_sparse_index(self) -> None:
        ...
        if sparse_name in self._client.list_indexes().names():
            if self._config.core.recreate_if_exists:
                logger.info(
                    f"Sparse index '{sparse_name}' exists and 'recreate_if_exists' is True. Deleting it first."
                )
                self._client.delete_index(sparse_name)
                self._wait_for_deletion(sparse_name)
            else:
                logger.info(
                    f"Sparse index '{sparse_name}' already exists. Skipping creation."
                )
                self._sparse_index = self._client.Index(sparse_name)
                return
        ...
        try:
            ...
            raise Exception(
                f"Failed to create sparse index '{sparse_name}': {e}"
            ) from e
    ...
    def _wait_for_index_ready(self, index_name: str) -> None:
        ...
        while True:
            ...
            if time.time() - start_time > wait_timeout:
                raise Exception(
                    f"Timeout waiting for index '{index_name}' to become ready."
                )
    ...
    def _wait_for_deletion(self, index_name: str) -> None:
        ...
        while index_name in self._client.list_indexes().names():
            if time.time() - start_time > wait_timeout:
                raise VectorDBError(
                    f"Timeout waiting for index '{index_name}' to be deleted."
                )
    ...
    def _generate_batches(
        ...
        for i in range(len(ids)):
            ...
            record_dict = {"id": str(ids[i]), "metadata": metadata}
            if vectors:
    ...
    def upsert(
        ...
        **kwargs,
        ...
        if not self._is_connected:
            raise VectorDBConnectionError(
                "Not connected to Pinecone. Please call connect() first."
            )

        logger.debug(
            "Initiating upsert. Validating data against collection configuration..."
        )
        ...
        if cfg.create_dense_index and cfg.create_sparse_index:
            ...
            if not is_dense_provided and not is_sparse_provided:
                raise UpsertError(
                    "In Hybrid mode, at least one of 'vectors' or 'sparse_vectors' must be provided."
                )
            if is_dense_provided and is_sparse_provided:
                # Both provided - validate lengths match
                if not (
                    len(ids) == len(payloads) == len(vectors) == len(sparse_vectors)
                ):
                    raise UpsertError(
                        "In Hybrid mode, all lists must have the same length."
                    )
            elif is_dense_provided:
                # Only dense provided - this is allowed for dense-only upserts
                if not (len(ids) == len(payloads) == len(vectors)):
                    raise UpsertError(
                        "In Hybrid mode with dense-only data, ids, payloads, and vectors must have the same length."
                    )
                ...
                if not (len(ids) == len(payloads) == len(sparse_vectors)):
                    raise UpsertError(
                        "In Hybrid mode with sparse-only data, ids, payloads, and sparse_vectors must have the same length."
                    )
            ...
            if is_sparse_provided:
                raise UpsertError(
                    "Configuration mismatch: sparse_vectors provided but collection is dense-only."
                )
        elif cfg.create_sparse_index:
            if not is_sparse_provided:
                raise UpsertError(
                    "In Sparse-Only mode, 'sparse_vectors' must be provided."
                )
            if is_dense_provided:
                raise UpsertError(
                    "Configuration mismatch: vectors provided but collection is sparse-only."
                )
        ...
        try:
            if cfg.create_dense_index and self._dense_index and vectors:
                dense_batches = self._generate_batches(
                    ids, payloads, vectors, None, chunks
                )
                for batch in dense_batches:
                    self._dense_index.upsert(
            ...
            if cfg.create_sparse_index and self._sparse_index and sparse_vectors:
                sparse_batches = self._generate_batches(
                    ids, payloads, None, sparse_vectors, chunks
                )
                for batch in sparse_batches:
                    self._sparse_index.upsert(
                        vectors=batch, namespace=namespace, **kwargs
                    )
                logger.info(
                    f"Successfully upserted {len(ids)} records to sparse index."
                )

        except ApiException as e:
            raise UpsertError(
                f"An error occurred during Pinecone upsert operation: {e}"
            ) from e
        except Exception as e:
            raise UpsertError(
                f"A general error occurred during the upsert process: {e}"
            ) from e
    ...
    def fetch(self, ids: List[Union[str, int]], **kwargs) -> List[VectorSearchResult]:
        ...
        if self._dense_index:
            try:
                dense_response = self._dense_index.fetch(
                    ids=[str(i) for i in ids], namespace=namespace, **kwargs
                )
        ...
        if self._sparse_index and not results:
            try:
                sparse_response = self._sparse_index.fetch(
                    ids=[str(i) for i in ids], namespace=namespace, **kwargs
                )
    ...
    def _parse_fetch_response(self, response) -> List[VectorSearchResult]:
        ...
        for record_id, vector_data in fetched_vectors.items():
            metadata = (
                ...
                else vector_data.get("metadata", {})
            )
            values = (
                vector_data.values
                if hasattr(vector_data, "values")
                else vector_data.get("values")
            )
    ...
    def dense_search(
        ...
        **kwargs,
        ...
        if not self._dense_index:
            raise VectorDBConnectionError("Not connected to a Pinecone index.")
        ...
        try:
            ...
            logger.debug(
                f"Dense search found {len(results)} results, {len(filtered_results)} after similarity threshold {final_similarity_threshold}"
            )
        ...
        **kwargs,
        ...
        try:
            ...
            filtered_results = [
                result
                for result in results
                if result.score >= final_similarity_threshold
            ]
            return filtered_results
        except ApiException as e:
            raise SearchError(
                f"Full-text (sparse) search operation failed in Pinecone: {e}"
            ) from e
    ...
    def hybrid_search(
        ...
        **kwargs,
        ...
        if not self._dense_index or not self._sparse_index:
            raise VectorDBConnectionError(
                "Both dense and sparse indexes required for hybrid search."
            )
    ...
    def _reciprocal_rank_fusion(
        ...
        if dense_result:
            dense_rank = next(
                (i for i, r in enumerate(dense_results) if r.id == dense_result.id),
                len(dense_results),
            )
        ...
        if sparse_result:
            sparse_rank = next(
                (i for i, r in enumerate(sparse_results) if r.id == sparse_result.id),
                len(sparse_results),
            )
    ...
    def search(
        ...
        **kwargs,
        ...
        if final_top_k is None:
            raise ConfigurationError(
                "A 'top_k' value must be provided either in the search call or as 'default_top_k' in the configuration."
            )
        ...
        if is_dense_intent and is_sparse_intent:
            ...
            if not self._config.search.hybrid_search_enabled:
                raise ConfigurationError(
                    "Hybrid search was requested, but it is disabled in the provider's SearchConfig."
                )
            if not (self._dense_index and self._sparse_index):
                raise VectorDBConnectionError(
                    "Hybrid search requires both dense and sparse indexes to be available."
                )
            ...
            if not self._config.search.dense_search_enabled:
                raise ConfigurationError(
                    "Dense search was requested, but it is disabled in the provider's SearchConfig."
                )
            if not self._dense_index:
                raise VectorDBConnectionError(
                    "Dense search requires a dense index to be available."
                )
            return self.dense_search(
                query_vector, final_top_k, final_filter, similarity_threshold, **kwargs
            )
            ...
            if not self._config.search.full_text_search_enabled:
                raise ConfigurationError(
                    "Full-text search was requested, but it is disabled in the provider's SearchConfig."
                )
            if not self._sparse_index:
                raise VectorDBConnectionError(
                    "Full-text search requires a sparse index to be available."
                )
            return self.full_text_search(
                query_text, final_top_k, final_filter, similarity_threshold, **kwargs
            )

        else:
            raise SearchError(
                "Search requires at least one of 'query_vector' or 'query_text' to be provided."
            )

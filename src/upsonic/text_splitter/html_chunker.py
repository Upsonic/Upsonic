...
class HTMLChunker(BaseChunker[HTMLChunkingConfig]):
    ...
    def _chunk_document(self, document: Document) -> List[Chunk]:
        ...
        for block in semantic_blocks:
            ...
            if block.tag_name in self.config.preserve_whole_tags:
                ...
                all_chunks.append(
                    self._create_chunk(
                        ...
                        text_content=original_html_content,
            ...
            if (
                ...
                temp_doc = Document(
                    content=original_html_content, document_id=document.document_id
                )
                ...
                for sub_chunk in sub_chunks:
                    ...
                    all_chunks.append(
                        self._create_chunk(
                            parent_document=document,
                            text_content=sub_chunk.text_content,
    ...
    def _segment_dom(self, soup: BeautifulSoup, raw_html: str) -> List[_SemanticBlock]:
        ...
        for i, start_tag in enumerate(all_split_tags):
            ...
            if self.config.preserve_html_content:
                block_contents = [
                    self._get_html_from_node(node) for node in all_nodes_in_block
                ]
                full_text = "".join(filter(None, block_contents))
            else:
                block_texts = [
                    self._get_text_from_node(node) for node in all_nodes_in_block
                ]
            ...
            start_index, end_index = self._calculate_tag_indices(
                start_tag, all_nodes_in_block, raw_html
            )
    ...
    def _calculate_tag_indices(
        self, start_tag, all_nodes_in_block, raw_html: str
    ) -> tuple[int, int]:
        import re
    ...
    def _merge_small_chunks(
        ...
        while i < len(chunks):
            ...
            while j < len(chunks):
                next_chunk = chunks[j]
                self.config.length_function(next_chunk.text_content)

                if self.config.preserve_html_content:
                    separator = ""
                    combined_size = self.config.length_function(
                        merged_text + separator + next_chunk.text_content
                    )
                else:
                    separator = " "
                    combined_size = self.config.length_function(
                ...
                if self.config.preserve_html_content:
                    ...
                    merged_text = (
                        merged_text.rstrip() + " " + next_chunk.text_content.lstrip()
                    )

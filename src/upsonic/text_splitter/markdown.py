    ...
    def _chunk_document(self, document: Document) -> List[Chunk]:
        ...
        for block in semantic_blocks:
            ...
            if (is_boundary or is_preserved) and current_chunk_blocks:
                all_chunks.extend(
                    self._finalize_chunk(current_chunk_blocks, header_stack, document)
                )
            ...
            if block.type.startswith("h") and block.type[1:].isdigit():
                ...
                header_stack = [
                    h for h in header_stack if int(list(h.keys())[0][1:]) < level
                ]
        ...
        for match in master_pattern.finditer(text):
            if match.start() > last_end:
                blocks.append(
                    _SemanticBlock(
                        type="paragraph",
                        raw_content=text[last_end : match.start()],
    ...
    def _finalize_chunk(
        ...
        if self.config.length_function(full_text) <= self.config.chunk_size:
            if full_text.strip():
                chunk_content = (
                    original_text
                    if self.config.preserve_original_content
                    else full_text
                )
                return [
                    self._create_chunk(
            ...
            content_to_split = (
                original_text if self.config.preserve_original_content else full_text
            )
            temp_doc = Document(
                content=content_to_split, document_id=document.document_id
            )

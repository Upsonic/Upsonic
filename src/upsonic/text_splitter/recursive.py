...
class RecursiveChunker(BaseChunker[RecursiveChunkingConfig]):
    ...
    def _chunk_document(self, document: Document) -> List[Chunk]:
        ...
        atomic_splits = self._recursive_split(
            text=content, separators=self.config.separators, offset=0
        )
    ...
    def _recursive_split(
        ...
        for match in re.finditer(pattern, text):
            ...
            if self.config.length_function(segment_text) > self.config.chunk_size:
                if next_separators:
                    final_splits.extend(
                        self._recursive_split(segment_text, next_separators, start_idx)
                    )
                else:
                ...
                if next_separators:
                    final_splits.extend(
                        self._recursive_split(
                            remaining_text, next_separators, start_idx
                        )
                    )
                else:
    ...
    @classmethod
    def from_language(
        cls, language: Language, config: Optional[RecursiveChunkingConfig] = None
    ) -> "RecursiveChunker":
        ...
        logger.info(
            f"Created a RecursiveChunker instance configured for '{language.value}'."
        )

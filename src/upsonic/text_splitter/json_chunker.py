...
class JSONChunker(BaseChunker[JSONChunkingConfig]):
    ...
    def _chunk_document(self, document: Document) -> List[Chunk]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse JSON for document {document.document_id}: {e}. Falling back to text chunking."
            )
        ...
        for builder in chunk_builders:
            ...
            text_content = json.dumps(
                builder["content"], **self.config.json_encoder_options
            )
    ...
    def _recursive_walk(
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                self._recursive_walk(
                    value, new_path, chunk_builders, depth + 1, min_chunk_size
                )
        else:
            self._add_to_chunk(data, current_path, chunk_builders, min_chunk_size)
    ...
    def _fallback_to_text_chunking(self, document: Document) -> List[Chunk]:
        from upsonic.text_splitter.recursive import (
            RecursiveChunker,

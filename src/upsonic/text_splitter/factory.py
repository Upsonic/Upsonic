...
def register_chunking_strategy(
    name: str,
    strategy_class: Type[BaseChunker],
    config_class: Type[BaseChunkingConfig] = None,
):
    """
    ...
    if not issubclass(strategy_class, BaseChunker):
        raise TypeError(
            f"Strategy class must inherit from BaseChunker, got {strategy_class}"
        )

    if config_class and not issubclass(config_class, BaseChunkingConfig):
        raise TypeError(
            f"Config class must inherit from BaseChunkingConfig, got {config_class}"
        )
...
def _lazy_import_strategies():
    ...
    try:
        ...
        register_chunking_strategy(
            "character", CharacterChunker, CharacterChunkingConfig
        )
    except ImportError:
        pass
    ...
    try:
        ...
        register_chunking_strategy(
            "recursive", RecursiveChunker, RecursiveChunkingConfig
        )
    except ImportError:
        pass
    ...
    try:
        if "recursive" in _STRATEGY_REGISTRY:
            register_chunking_strategy(
                "recursive_character",
                _STRATEGY_REGISTRY["recursive"],
                _CONFIG_REGISTRY.get("recursive"),
            )
    except Exception:
        pass
    ...
    try:
        if "markdown" in _STRATEGY_REGISTRY:
            register_chunking_strategy(
                "markdown_header",
                _STRATEGY_REGISTRY["markdown"],
                _CONFIG_REGISTRY.get("markdown"),
            )
            register_chunking_strategy(
                "markdown_recursive",
                _STRATEGY_REGISTRY["markdown"],
    ...
    try:
        if "python" in _STRATEGY_REGISTRY:
            register_chunking_strategy(
                "code", _STRATEGY_REGISTRY["python"], _CONFIG_REGISTRY.get("python")
            )
            register_chunking_strategy(
                "py", _STRATEGY_REGISTRY["python"], _CONFIG_REGISTRY.get("python")
            )
            register_chunking_strategy(
                "python_code",
                _STRATEGY_REGISTRY["python"],
                _CONFIG_REGISTRY.get("python"),
            )
    except Exception:
        pass
    ...
    try:
        if "agentic" in _STRATEGY_REGISTRY:
            register_chunking_strategy(
                "ai", _STRATEGY_REGISTRY["agentic"], _CONFIG_REGISTRY.get("agentic")
            )
    except Exception:
        pass
...
def detect_content_type(
    ...
    if metadata:
        ...
        if any(ext in source or ext in file_name for ext in [".md", ".markdown"]):
            ...
            ext in source or ext in file_name for ext in [".js", ".jsx", ".ts", ".tsx"]
        ):
            return ContentType.JAVASCRIPT

    content_sample = content[:2000]

    if re.search(r"<[^>]+>", content_sample) and any(
        tag in content_sample.lower() for tag in ["<html", "<div", "<p>", "<span"]
    ):
        return ContentType.HTML
    ...
    if any(
        re.search(pattern, content_sample, re.MULTILINE)
    ...
    if content_sample.strip().startswith("<?xml") or re.search(
        r"<\w+[^>]*>.*?</\w+>", content_sample
    ):
        return ContentType.XML
    ...
    if (
        sum(
            content_sample.lower().count(indicator.lower())
...
def recommend_strategy_for_content(
    ...
    if content_type == ContentType.MARKDOWN and "markdown" in available:
        ...
        content_type in [ContentType.PYTHON, ContentType.JAVASCRIPT]
        and "python" in available
    ):
        return "python"
    ...
    if use_case in [ChunkingUseCase.RAG_RETRIEVAL, ChunkingUseCase.QUESTION_ANSWERING]:
        if (
            ...
            and "agentic" in available
            and content_length < 50000
        ):
            try:
                return "agentic"
            except:
...
def create_adaptive_strategy(
    ...
    **kwargs,
    ...
    optimized_config = _create_optimized_config(
        content, content_type, strategy_name, **kwargs
    )

    info_log(
        f"Auto-selected {strategy_name} strategy for {content_type.value} content",
        context="TextSplitterFactory",
    )
...
def _create_optimized_config(
    content: str, content_type: ContentType, strategy_name: str, **kwargs
) -> Dict[str, Any]:
    ...
    **global_config_kwargs,
    ...
    for i, source in enumerate(sources):
        try:
            ...
            info_log(
                f"Created {splitter.__class__.__name__} for {source}",
                context="TextSplitterFactory",
            )

        except Exception as e:
            warning_log(
                f"Failed to create intelligent splitter for {source}: {e}",
                context="TextSplitterFactory",
            )
            try:
                ...
                fallback_splitter = create_chunking_strategy(
                    "recursive", **fallback_config
                )
                splitters.append(fallback_splitter)
                info_log(
                    f"Using recursive strategy fallback for {source}",
                    context="TextSplitterFactory",
                )
            except Exception as fallback_error:
                warning_log(
                    f"Fallback splitter also failed for {source}: {fallback_error}",
                    context="TextSplitterFactory",
                )
                raise
    ...
    **global_config_kwargs,
    ...
    if source_path.exists() and source_path.is_file():
        ...
        if extension in [".md", ".markdown"]:
            config.setdefault(
                "chunk_overlap", int(config.get("chunk_size", 1000) * 0.2)
            )
        elif extension in [".py", ".js", ".ts", ".java", ".cpp", ".c"]:
            config.setdefault(
                "chunk_overlap", int(config.get("chunk_size", 1000) * 0.15)
            )
        elif extension in [".json", ".xml", ".yaml", ".yml"]:
            config.setdefault(
                "chunk_overlap", int(config.get("chunk_size", 1000) * 0.1)
            )
        elif extension in [".html", ".htm"]:
            config.setdefault(
                "chunk_overlap", int(config.get("chunk_size", 1000) * 0.25)
            )
        elif extension in [".pdf", ".docx"]:
            config.setdefault(
                "chunk_overlap", int(config.get("chunk_size", 1000) * 0.3)
            )
        else:  # .txt, .csv, etc.
            config.setdefault(
                "chunk_overlap", int(config.get("chunk_size", 1000) * 0.2)
    ...
    if content:
        ...
        try:
            ...
            if (
                ...
                or "OpenAI API key not found" in str(e)
                or "embedding_provider" in str(e)
            ):
                warning_log(
                    "No embedding provider available, falling back to recursive strategy",
                    context="TextSplitterFactory",
                )
                return create_chunking_strategy("recursive", **kwargs)
            else:
...
def create_quality_strategy(content: str = "", **kwargs) -> BaseChunker:
    ...
    if content:
        return create_adaptive_strategy(content, quality_preference="quality", **kwargs)
    else:
        ...
        if "agentic" in available:
            try:
                ...
                if error_code == "MISSING_AGENT" or "agent" in str(e).lower():
                    warning_log(
                        "No agent available, trying semantic strategy",
                        context="TextSplitterFactory",
                    )
                else:
                    raise

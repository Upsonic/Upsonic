...
class AgenticChunkingConfig(BaseChunkingConfig):
    ...
    max_agent_retries: int = Field(
        default=3, description="Maximum retries for agent calls"
    )

    min_proposition_length: int = Field(
        default=20, description="Minimum length for valid propositions"
    )
    max_propositions_per_chunk: int = Field(
        default=15, description="Maximum propositions in a single chunk"
    )
    min_propositions_per_chunk: int = Field(
        default=3, description="Minimum propositions to form a chunk"
    )

    enable_proposition_caching: bool = Field(
        default=True, description="Cache proposition extraction results"
    )
    enable_topic_caching: bool = Field(
        default=True, description="Cache topic assignment results"
    )
    enable_refinement_caching: bool = Field(
        default=True, description="Cache topic refinement results"
    )

    enable_proposition_validation: bool = Field(
        default=True, description="Validate proposition quality"
    )
    enable_topic_optimization: bool = Field(
        default=True, description="Optimize topic assignments"
    )
    enable_coherence_scoring: bool = Field(
        default=True, description="Score chunk coherence"
    )

    fallback_to_recursive: bool = Field(
        default=True, description="Fallback to recursive chunking on agent failure"
    )

    include_proposition_metadata: bool = Field(
        default=True, description="Include proposition-level metadata"
    )
    include_topic_scores: bool = Field(
        default=True, description="Include topic coherence scores"
    )
    include_agent_metadata: bool = Field(
        default=True, description="Include agent processing metadata"
    )
...
class AgenticChunker(BaseChunker[AgenticChunkingConfig]):
    ...
    async def _achunk_document(self, document: Document) -> List[Chunk]:
        ...
        try:
            ...
            logger.error(
                f"Agentic chunking failed for document {document.document_id}: {e}"
            )
            return await self._fallback_chunking(document)
    ...
    async def _extract_propositions(self, document: Document) -> List[str]:
        cache_key = (
            self._get_cache_key(document.content)
            if self.config.enable_proposition_caching
            else None
        )
        ...
        for attempt in range(self.config.max_agent_retries):
            try:
                ...
                logger.warning(
                    f"Proposition extraction attempt {attempt + 1} failed: {e}"
                )
    ...
    def _validate_propositions(self, propositions: List[str]) -> List[str]:
        ...
        for prop in propositions:
            ...
            if (
                len(prop) >= self.config.min_proposition_length
                and prop
                and not prop.isspace()
                and prop not in validated
            ):
    ...
    @upsonic_error_handler(max_retries=1, show_error_details=True)
    async def _group_propositions_into_topics(
        self, propositions: List[str], document: Document
    ) -> List[Topic]:
        cache_key = (
            self._get_cache_key(str(propositions))
            if self.config.enable_topic_caching
            else None
        )
    ...
    def _validate_topic_sizes(self, topics: List[Topic]) -> List[Topic]:
        ...
        for topic in topics:
            if (
                self.config.min_propositions_per_chunk
                <= len(topic.propositions)
                <= self.config.max_propositions_per_chunk
            ):
                validated_topics.append(topic)
            elif len(topic.propositions) > self.config.max_propositions_per_chunk:
                for i in range(
                    0, len(topic.propositions), self.config.max_propositions_per_chunk
                ):
                    chunk_props = topic.propositions[
                        i : i + self.config.max_propositions_per_chunk
                    ]
                    if len(chunk_props) >= self.config.min_propositions_per_chunk:
                        split_topic = Topic(
                            topic_id=len(validated_topics) + 1, propositions=chunk_props
                        )
    ...
    def _optimize_topics(self, topics: List[Topic]) -> List[Topic]:
        ...
        if small_propositions:
            for i in range(
                0, len(small_propositions), self.config.max_propositions_per_chunk
            ):
                chunk_props = small_propositions[
                    i : i + self.config.max_propositions_per_chunk
                ]
                if len(chunk_props) >= self.config.min_propositions_per_chunk:
                    merged_topic = Topic(
                        topic_id=len(optimized_topics) + 1, propositions=chunk_props
                    )
                    optimized_topics.append(merged_topic)

        return optimized_topics

    def _find_chunk_indices_in_document(
        self, chunk_text: str, document_content: str
    ) -> Tuple[int, int]:
    ...
    async def _create_chunks_from_topics(
        self, topics: List[Topic], document: Document
    ) -> List[Chunk]:
        chunks = []
        ...
        for i, topic in enumerate(topics):
            ...
            if (
                current_length + topic_length > self.config.chunk_size
                and current_chunk_topics
            ):
                if current_length >= min_chunk_size:
                    final_chunk = await self._finalize_topic_chunk(
                        current_chunk_topics, document
                    )
    ...
    async def _finalize_topic_chunk(
        self, topics: List[Topic], document: Document
    ) -> Chunk:
        all_propositions = []
        for topic in topics:
        ...
        start_index, end_index = self._find_chunk_indices_in_document(
    ...
    @upsonic_error_handler(max_retries=1, show_error_details=True)
    async def _refine_topic_metadata(
        self, chunk_text: str, topic: Topic
    ) -> RefinedTopic:
        cache_key = (
            self._get_cache_key(chunk_text)
            if self.config.enable_refinement_caching
            else None
        )
    ...
    def _score_chunk_coherence(self, chunks: List[Chunk]) -> List[Chunk]:
        ...
        for chunk in chunks:
            ...
            if self.config.include_topic_scores:
                ...
                chunk.metadata["quality_assessment"] = self._assess_chunk_quality(
                    coherence_score
                )
    ...
    async def _fallback_chunking(self, document: Document) -> List[Chunk]:
        ...
        logger.warning(
            f"Falling back to recursive chunking for document {document.document_id}"
        )

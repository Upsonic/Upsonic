...
from upsonic.profiles._json_schema import JsonSchema, JsonSchemaTransformer
...
class OpenAIJsonSchemaTransformer(JsonSchemaTransformer):
    ...
    * all fields in properties must be marked as required
    ...
    def walk(self) -> JsonSchema:
        ...
        # However, we don't need to check for it here because we ensure in upsonic._utils.check_object_json_schema
        ...
        if incompatible_values:
            if self.strict is True:
                ...
                schema["description"] = (
                    notes_string
                    if not description
                    else f"{description} ({notes_string})"
                )
            elif self.strict is None:  # pragma: no branch
                self.is_strict_compatible = False

...
class AttachmentFileNotFoundError(Exception):
    ...
    def __init__(self, attachment_path: str, task_description: str = None):
        ...
        if not os.path.isabs(attachment_path):
            suggestions.append(
                f"• Check if the file path is correct relative to the current working directory: {os.getcwd()}"
            )
            suggestions.append(
        ...
        if (
            ...
            and not os.access(parent_dir, os.R_OK)
        ):
            suggestions.append(
                f"• Check if you have read permissions for the directory: {parent_dir}"
            )
...
def validate_attachments_exist(task: "Task") -> None:
    ...
    if missing_files:
        ...
        raise AttachmentFileNotFoundError(
            attachment_path=missing_files[0], task_description=task.description
        )
...
def validate_attachments_readable(task: "Task") -> None:
    ...
    if unreadable_files:
        raise AttachmentFileNotFoundError(
            attachment_path=unreadable_files[0], task_description=task.description
        )

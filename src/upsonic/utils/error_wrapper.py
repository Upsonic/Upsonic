...
def map_pydantic_error_to_upsonic(error: Exception) -> UupsonicError:
    ...
    if any(
        ...
        for keyword in ["api key", "apikey", "authentication", "unauthorized", "401"]
    ):
        return NoAPIKeyException(f"API key error: {str(error)}")

    # Connection and network errors
    if any(
        keyword in error_str
        for keyword in ["connection", "network", "timeout", "refused", "unreachable"]
    ):
    ...
    if any(
        keyword in error_str
        for keyword in ["rate limit", "quota", "billing", "usage limit"]
    ):
        return ModelConnectionError(
    ...
    if any(
        keyword in error_str
        for keyword in ["validation", "invalid input", "bad request", "400"]
    ):
    ...
    if any(
        keyword in error_str
        for keyword in ["configuration", "config", "setup", "missing"]
    ):
    ...
    if any(
        keyword in error_str
        for keyword in ["500", "server error", "internal error", "service unavailable"]
    ):
        return ModelConnectionError(
            ...
            async def async_wrapper(*args, **kwargs) -> Any:
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if attempt == max_retries or not _is_retryable_error(
                            upsonic_error
                        ):
...
def _is_retryable_error(error: UupsonicError) -> bool:
    ...
    """
    retryable_codes = {"CONNECTION_ERROR", "SERVER_ERROR", "TIMEOUT_ERROR"}

    return (
        isinstance(error, ModelConnectionError) and error.error_code in retryable_codes
    )
...
def _display_error(error: Union[UupsonicError, Exception]) -> None:
    ...
    error_message(error_type=error_type_name, detail=detail, error_code=status_code)

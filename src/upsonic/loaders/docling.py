...
class DoclingLoader(BaseLoader):
    ...
    def __init__(self, config: DoclingLoaderConfig):
        ...
        self._chunker = (
            self._create_chunker() if config.extraction_mode == "chunks" else None
        )
    ...
    def _create_pdf_pipeline_options(self) -> "PdfPipelineOptions":
        ...
        if self.config.enable_table_structure:
            ...
            pipeline_options.table_structure_options.do_cell_matching = (
                self.config.table_structure_cell_matching
            )
    ...
    def _convert_document(
        ...
        try:
            ...
            conversion_result = self._converter.convert(
                source=str(validated_source), **convert_kwargs
            )
    ...
    def _handle_conversion_error(self, source: Union[str, Path], error: Exception):
        ...
        self._handle_loading_error(str(source), error)
        return None  # Return None for document conversion failures

    def _extract_markdown(
        self, dl_doc: "DoclingDocument", source: Union[str, Path], document_id: str
    ) -> List[Document]:
        """
        ...
        try:
            ...
            doc = Document(
                content=markdown_content, document_id=document_id, metadata=metadata
            )
    ...
    def _extract_chunks(
        ...
        if not self._chunker:
            ...
            self._logger.warning(
                f"Chunker not available, extracting as markdown: {source}"
            )
        ...
        try:
            ...
            for idx, chunk in enumerate(chunk_iter):
                ...
                doc = Document(
                    content=chunk_text, document_id=chunk_doc_id, metadata=metadata
                )

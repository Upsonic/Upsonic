...
class LoaderFactory:
    ...
    def _get_default_loader_configs() -> List[Tuple[Type[BaseLoader], List[str]]]:
        ...
        loaders = [
            ...
            (
                PdfPlumberLoader,
                [".pdf"],
            ),  # Alternative PDF loader with pdfplumber (superior tables)
            (DOCXLoader, [".docx"]),
    ...
    def register_loader(self, loader_class: Type[BaseLoader], extensions: List[str]):
        ...
        if not issubclass(loader_class, BaseLoader):
            raise ValueError(
                f"Loader class {loader_class.__name__} must be a subclass of BaseLoader"
            )
        ...
        for ext in extensions:
            if not ext.startswith(".") or len(ext) < 2:
                raise ValueError(
                    f"Invalid extension '{ext}'. Extensions must start with '.' and be at least 2 characters"
                )
        ...
        for ext in extensions:
            if (
                ext.lower() in self._extensions
                and self._extensions[ext.lower()] != loader_name
            ):
                conflicting_extensions.append(ext)

        if conflicting_extensions:
            logger.warning(
                f"Extension conflicts detected for {loader_name}: {conflicting_extensions}"
            )
            for ext in conflicting_extensions:
                old_loader = self._extensions[ext.lower()]
                logger.info(
                    f"Replacing {old_loader} with {loader_name} for extension {ext}"
                )
        ...
        for loader_name, loader_class in self._loaders.items():
            for method in self._REQUIRED_METHODS:
                if not hasattr(loader_class, method):
                    logger.warning(
                        f"Loader {loader_name} missing required method: {method}"
                    )
    ...
    def _check_duplicate_extensions(self):
        ...
        duplicates = {
            ext: count for ext, count in extension_counts.items() if count > 1
        }
        if duplicates:
            logger.warning(f"Duplicate extensions found: {duplicates}")

    def get_loader(
        self, source: str, loader_type: Optional[str] = None, **config_kwargs
    ) -> BaseLoader:
        """
        ...
        try:
            ...
            if (
                not source.startswith(("http://", "https://", "ftp://"))
            ...
            if loader_type:
                ...
                if loader_name not in self._loaders:
                    raise ValueError(
                        f"No loader found for type '{loader_type}'. Available types: {list(self._loaders.keys())}"
                    )
            else:
                loader_name = self._detect_loader_type(source)
                if loader_name not in self._loaders:
                    raise ValueError(
                        f"No loader found for source '{source}'. Detected type: '{loader_name}'. Available types: {list(self._loaders.keys())}"
                    )
    ...
    def create_intelligent_loaders(
        self, sources: List[Union[str, Path]], **global_config_kwargs
    ) -> List[BaseLoader]:
        """
        ...
        for source in sources:
            if isinstance(source, str):
                logger.debug(
                    f"Skipping loader creation for string content: {source[:50]}..."
                )
                continue
            ...
            try:
                ...
                source_config = self._create_optimized_config(
                    source_str, loader_type, **global_config_kwargs
                )
                ...
                try:
                    fallback_config = self._create_optimized_config(
                        source_str, "text", **global_config_kwargs
                    )
                    fallback_loader = self.get_loader(
                        source_str, "text", **fallback_config
                    )
                    loaders.append(fallback_loader)
                    logger.info(f"Using text loader fallback for {source}")
                except Exception as fallback_error:
                    logger.error(
                        f"Fallback loader also failed for {source}: {fallback_error}"
                    )
                    raise
    ...
    def _create_optimized_config(
        self, source: str, loader_type: str, **global_config_kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def _apply_loader_optimizations(
        self, config: Dict[str, Any], loader_type: str, source: str, file_size: int
    ) -> None:
    ...
    def _optimize_pymupdf_config(self, config: Dict[str, Any], file_size: int) -> None:
        ...
        if file_size > self._PDF_LARGE_THRESHOLD:
            ...
            )  # Better structure for smaller files

    def _optimize_docling_config(
        self, config: Dict[str, Any], file_size: int, source: str
    ) -> None:
        ...
        if file_size > self._PDF_LARGE_THRESHOLD:
            ...
            )  # Disable cell matching for speed
        else:
    ...
    def _is_json_content(content: str) -> bool:
        ...
        if not (
            ...
            and (content.endswith("}") or content.endswith("]"))
        ):
            return False
        try:
            json.loads(content)
    ...
    def _create_specific_config(
        self, loader_name: str, config_kwargs: Dict[str, Any]
    ) -> LoaderConfig:
    ...
    def _create_base_config(self, config_kwargs: Dict[str, Any]) -> LoaderConfig:
        ...
        valid_kwargs = {
            k: v for k, v in config_kwargs.items() if k in LoaderConfig.model_fields
        }
        return LoaderConfig(**valid_kwargs) if valid_kwargs else LoaderConfig()

    def _validate_config_params(
        self, config_class: Type[LoaderConfig], config_kwargs: Dict[str, Any]
    ) -> None:
        ...
        if invalid_params:
            logger.warning(
                f"Invalid config parameters for {config_class.__name__}: {invalid_params}"
            )
    ...
    def get_loader_for_content(
        self, content: str, content_type: str, **config_kwargs
    ) -> BaseLoader:
    ...
    def get_loaders_for_directory(
        ...
        **config_kwargs,
        ...
        filtered_files = self._filter_files_by_patterns(
            all_files, file_patterns, exclude_patterns
        )
    ...
    def get_loader_info(self, loader_type: str) -> Dict[str, Any]:
        ...
        info = {
            ...
            "extensions": [
                ext for ext, name in self._extensions.items() if name == loader_type
            ],
    ...
    def list_loaders(self) -> List[Dict[str, Any]]:
        return [
            self.get_loader_info(loader_type) for loader_type in self._loaders.keys()
        ]
    ...
    def _find_extension_conflicts(self) -> Dict[str, List[str]]:
        ...
        return {
            ext: loaders
            for ext, loaders in extension_to_loaders.items()
            if len(loaders) > 1
        }
    ...
    def validate_source(self, source: str) -> Dict[str, Any]:
        ...
        try:
            if result["is_url"]:
                ...
                result["recommended_loader"] = (
                    detected_type if result["can_handle"] else None
                )
            elif result["is_file"]:
...
def create_loader(
    source: str, loader_type: Optional[str] = None, **config_kwargs
) -> BaseLoader:
...
def create_loader_for_content(
    content: str, content_type: str, **config_kwargs
) -> BaseLoader:
...
def create_intelligent_loaders(
    sources: List[Union[str, Path]], **config_kwargs
) -> List[BaseLoader]:
...
def load_documents_batch(
    sources: List[str], **config_kwargs

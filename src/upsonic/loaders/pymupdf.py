...
class PyMuPDFLoader(BaseLoader):
        ...
        try:
            ...
            if doc.needs_pass:
                ...
                auth_result = await asyncio.to_thread(
                    doc.authenticate, self.config.pdf_password
                )
                if not auth_result:
                    doc.close()
                    raise PermissionError(
            ...
            if self.config.include_metadata:
                ...
                if self.config.extract_annotations:
                    annotations = await self._extract_annotations(
    ...
    async def _perform_ocr(self, page: "pymupdf.Page") -> str:
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            ocr_result = await loop.run_in_executor(
                pool, self._run_single_ocr, img_data
            )
    ...
    async def _extract_images_info(
        self, doc: "pymupdf.Document"
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _extract_annotations(
        self, doc: "pymupdf.Document", start_idx: int, end_idx: int
    ) -> List[Dict[str, Any]]:
        """

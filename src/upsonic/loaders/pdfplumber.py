...
class PdfPlumberLoader(BaseLoader):
    ...
    async def _process_single_pdf(self, path: Path) -> List[Document]:
        ...
        try:
            ...
            pdf_handle = await asyncio.to_thread(
                pdfplumber.open, str(path), password=self.config.pdf_password
            )
            ...
            try:
                ...
                start_idx = (
                    (self.config.start_page - 1) if self.config.start_page else 0
                )
    ...
    async def _extract_page_content(self, page, page_num: int) -> Tuple[str, int]:
        ...
        if self.config.crop_box:
            working_page = await asyncio.to_thread(page.crop, self.config.crop_box)
        ...
        if self.config.extract_tables and self.config.extraction_mode in (
            "text_only",
            "hybrid",
        ):
    ...
    async def _extract_text_from_page(self, page) -> str:
        ...
        text = await asyncio.to_thread(page.extract_text, **extract_text_kwargs)
    ...
    async def _extract_tables_from_page(self, page) -> str:
        ...
        tables = await asyncio.to_thread(
            page.extract_tables, table_settings=self.config.table_settings
        )
    ...
    def _format_table_grid(self, table: List[List[str]]) -> str:
        ...
        for col_idx in range(max_cols):
            max_width = max(
                len(row[col_idx]) if col_idx < len(row) else 0 for row in table
            )
        ...
        for row in table:
            ...
            line = (
                "|"
                + "|".join(
                    [f" {cell:<{col_widths[i]}} " for i, cell in enumerate(padded_row)]
                )
                + "|"
            )
    ...
    def _format_table_text(self, table: List[List[str]]) -> str:
        ...
        for row in table:
            padded_row = row + [""] * (max_cols - len(row))
            line = "  ".join(
                [cell.ljust(col_widths[i]) for i, cell in enumerate(padded_row)]
            )
            lines.append(line)
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                ocr_result = await loop.run_in_executor(
                    pool, self._run_single_ocr, img_bytes
                )

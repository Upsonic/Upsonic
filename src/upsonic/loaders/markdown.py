...
class MarkdownLoader(BaseLoader):
    ...
    async def aload(
        ...
        tasks = [
            asyncio.to_thread(self._load_single_file, file) for file in files_to_process
        ]
    ...
    def _chunk_tokens(self, tokens: List[Token]) -> List[List[Token]]:
        ...
        for token in tokens:
            is_split_heading = (
                ...
                and token.tag == self.config.split_by_heading
            )
    ...
    def _process_chunk(
        self, tokens: List[Token], document_id: str, metadata: Dict[str, Any]
    ) -> Optional[Document]:
        ...
        for token in tokens:
            if not self.config.include_code_blocks and token.type in (
                "fence",
                "code_block",
            ):
                continue
            ...
            if self.config.include_metadata:
                if self.config.heading_metadata and token.type == "heading_open":
                    ...
                    if (
                        ...
                        extracted_headings.setdefault(token.tag, []).append(
                            heading_text
                        )

                if (
                    self.config.code_block_language_metadata
        ...
        return Document(
            document_id=document_id, content=final_content, metadata=metadata
        )

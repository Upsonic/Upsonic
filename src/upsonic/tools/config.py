...
class ToolConfig(BaseModel):
    ...
    cache_dir: Optional[str] = Field(
        default=None, description="Directory to store cache files."
    )

    cache_ttl: Optional[int] = Field(
        default=None, description="Time-to-live for cache entries in seconds."
    )

    tool_hooks: Optional[ToolHooks] = Field(
        default=None, description="Custom functions to run before/after tool execution."
    )

    max_retries: Optional[int] = Field(
        default=5, description="Maximum number of retries allowed for this tool."
    )

    timeout: Optional[float] = Field(
        default=30.0, description="Timeout for tool execution in seconds."
    )

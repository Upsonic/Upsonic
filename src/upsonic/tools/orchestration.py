...
class PlanStep(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Optional description of what this step accomplishes."
    )
...
class AnalysisResult(BaseModel):
    ...
    reasoning: Optional[str] = Field(
        None, description="Additional reasoning for the chosen next action."
    )
...
class ExecutionResult(BaseModel):
    ...
    success: bool = Field(..., description="Whether the plan execution was successful.")
    final_result: Any = Field(..., description="The final synthesized result.")
    execution_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="History of all tool executions."
    )
    total_steps: int = Field(..., description="Total number of steps executed.")
    revisions: int = Field(0, description="Number of plan revisions made.")
...
class Orchestrator:
    ...
    def __init__(
        self, agent_instance: Any, task: "Task", wrapped_tools: Dict[str, Callable]
    ):
        """Initialize the orchestrator."""
        self.agent_instance = agent_instance
        self.task = task
        self.wrapped_tools = wrapped_tools
        self.is_reasoning_enabled = (
            agent_instance.enable_reasoning_tool if agent_instance else False
        )
        self.original_user_request = task.description

        self.execution_history = (
            "Orchestrator's execution history for the user's request:\n"
        )
    ...
    async def execute(self, thought: Thought) -> Any:
        ...
        console.print(
            "[bold magenta]Orchestrator Activated:[/bold magenta] Received initial plan."
        )
        spacing()
        ...
        while self.program_counter < len(self.pending_plan):
            ...
            if self.is_reasoning_enabled:
                should_continue = await self._handle_reasoning_step(
    ...
    async def _inject_analysis(self) -> AnalysisResult:
        ...
        analysis_result: AnalysisResult = (
            ...
            if hasattr(analysis_run_result, "output")
            else analysis_run_result
        )
        self.execution_history += (
    ...
    async def _handle_reasoning_step(self, tool_name: str, result: Any) -> bool:
        ...
        console.print(
            f"[bold yellow]Injecting Mandatory Analysis Step after Tool '{tool_name}'...[/bold yellow]"
        )
        ...
        if analysis_result.next_action == "continue_plan":
            console.print(
                "[bold green]Analysis complete. Continuing with the original plan.[/bold green]"
            )
            self.program_counter += 1
            return True
            ...
            console.print(
                "[bold green]Analysis concluded that the task is complete. Proceeding to final synthesis.[/bold green]"
            )
            return False

        elif analysis_result.next_action == "revise_plan":
            console.print(
                "[bold red]Analysis concluded that the plan is flawed. Requesting a new plan.[/bold red]"
            )
    ...
    async def _request_plan_revision(self) -> None:
        ...
        revision_task = Task(
            description=revision_prompt, not_main_task=True, response_format=Thought
        )
        ...
        new_thought: Thought = (
            revision_run_result.output
            if hasattr(revision_run_result, "output")
            else revision_run_result
        )

        console.print(
            "[bold magenta]Orchestrator:[/bold magenta] Received revised plan. Restarting execution."
        )
    ...
    async def _synthesize_final_answer(self) -> Any:
        ...
        console.print(
            "[bold magenta]Orchestrator:[/bold magenta] Plan complete. Preparing for final synthesis."
        )
        spacing()

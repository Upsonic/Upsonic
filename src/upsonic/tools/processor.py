...
from upsonic.tools.base import Tool, ToolDefinition, ToolKit
from upsonic.tools.config import ToolConfig
from upsonic.tools.context import ToolContext
from upsonic.tools.schema import generate_function_schema, validate_tool_function
...
class ToolProcessor:
    ...
    def process_tools(
        self, tools: List[Any], context: Optional[ToolContext] = None
    ) -> Dict[str, Tool]:
    ...
    def _process_function_tool(self, func: Callable) -> Tool:
        ...
        # Create wrapped tool
        return FunctionTool(function=func, schema=schema, config=config)
    ...
    def _is_agent_instance(self, obj: Any) -> bool:
        ...
        return hasattr(obj, "name") and (
            hasattr(obj, "do_async") or hasattr(obj, "do") or hasattr(obj, "agent_id")
        )
        ...
        async def wrapper(**kwargs: Any) -> Any:
            ...
            if config.requires_user_input and config.user_input_fields:
                kwargs = self._get_user_input(
                    tool.name, kwargs, config.user_input_fields
                )
                ...
                try:
                    ...
                    if config.timeout:
                        result = await asyncio.wait_for(
                            tool.execute(**kwargs), timeout=config.timeout
                        )
                    else:
                    ...
                    if attempt < max_retries:
                        ...
                        console.print(
                            f"[yellow]Tool '{tool.name}' timed out, retrying in {wait_time}s... (attempt {attempt + 1}/{max_retries + 1})[/yellow]"
                        )
                        await asyncio.sleep(wait_time)
                    else:
                        raise TimeoutError(
                            f"Tool '{tool.name}' timed out after {config.timeout}s and {max_retries} retries"
                        )
                    ...
                    if attempt < max_retries:
                        wait_time = 2**attempt  # Exponential backoff
                        console.print(
                            f"[yellow]Tool '{tool.name}' failed, retrying in {wait_time}s... (attempt {attempt + 1}/{max_retries + 1})[/yellow]"
                        )
                        await asyncio.sleep(wait_time)
                    else:
                        console.print(
    ...
    def _get_user_input(
        self, tool_name: str, args: Dict[str, Any], fields: List[str]
    ) -> Dict[str, Any]:

...
class StateGraph(Runnable[Dict[str, Any], Dict[str, Any]]):
    ...
    def add_node(
        ...
        *,
        ...
        self.nodes[name] = NodeConfig(
            func=func, retry_policy=retry_policy, cache_policy=cache_policy
        )
    ...
    def _validate_graph(self):
        ...
        for cond_edge in self.conditional_edges:
            if cond_edge.from_node not in all_nodes:
                raise ValueError(
                    f"Conditional edge references unknown node: {cond_edge.from_node}"
                )
    ...
    def invoke(
        ...
        """
        raise RuntimeError(
            "Graph must be compiled before execution. Use graph.compile().invoke()"
        )
    ...
    async def ainvoke(
        ...
        raise RuntimeError(
            "Graph must be compiled before execution. Use graph.compile().ainvoke()"
        )
...
class CompiledStateGraph(Runnable[Dict[str, Any], Dict[str, Any]]):
    ...
    async def ainvoke(
        ...
        if isinstance(input, Command):
            try:
                final_state = await self._resume_from_interrupt(
                    input, thread_id, checkpoint_id, config, recursion_limit
                )
                ...
                return {**state, "__interrupt__": [{"value": e.value}]}
        ...
        if input is None and self.checkpointer:
            ...
            if existing_checkpoint:
                ...
                raise ValueError(
                    f"Cannot resume: no checkpoint found for thread_id={thread_id}"
                )
        # Load existing state from checkpoint or start fresh
        elif self.checkpointer and (
            existing_checkpoint := self.checkpointer.get(thread_id, checkpoint_id)
        ):
            state = deepcopy(existing_checkpoint.state)
            next_nodes = existing_checkpoint.next_nodes
    ...
    async def _execute_graph(
        ...
        while current_nodes:
            ...
            if START in current_nodes:
                ...
                if (
                    next_result
                    and len(next_result) > 0
                    and isinstance(next_result[0], Send)
                ):
                    # Execute Send objects from START
                    send_results = await self._execute_send_objects(
                        next_result, state, config
                    )
            ...
            if len(nodes_to_execute) == 1:
                ...
                try:
                    ...
                    if self.checkpointer:
                        await self._save_checkpoint(
                            ...
                            next_nodes=[
                                node_name
                            ],  # Will re-execute this node on resume
                            thread_id=thread_id,
                            parent_checkpoint_id=current_parent,
                ...
                if isinstance(updates, Command):
                    ...
                    if isinstance(updates, Send) or (
                        isinstance(updates, list)
                        and any(isinstance(u, Send) for u in updates)
                    ):
                        raise ValueError(
                ...
                if node_name in self.interrupt_after:
                    ...
                    if next_goto:
                        ...
                        if resolved_goto and isinstance(resolved_goto[0], Send):
                            next_nodes_after = [
                                s.node for s in resolved_goto if isinstance(s, Send)
                            ]
                        else:
                        ...
                        if next_nodes_result and isinstance(next_nodes_result[0], Send):
                            next_nodes_after = [
                                s.node for s in next_nodes_result if isinstance(s, Send)
                            ]
                        else:
                ...
                if next_goto:
                    ...
                    if resolved_goto and isinstance(resolved_goto[0], Send):
                        ...
                        send_results = await self._execute_send_objects(
                            send_objects=resolved_goto, state=state, config=config
                        )
                        # Merge send results
                        for send_updates in send_results:
                            state = self._merge_state(state, send_updates)
                        ...
                        for send_obj in resolved_goto:
                            ...
                            if next_from_send and not isinstance(
                                next_from_send[0], Send
                            ):
                                all_next.update(next_from_send)
                        current_nodes = list(all_next) if all_next else [END]
                    else:
                ...
                if self.checkpointer:
                    ...
                    for node_name, _ in parallel_results:
                        ...
                        if next_from_node and isinstance(next_from_node[0], Send):
                            all_next_nodes.update(
                                s.node for s in next_from_node if isinstance(s, Send)
                            )
                        else:
                ...
                for node_name, _ in parallel_results:
                    ...
                    if (
                        next_from_node
                        and len(next_from_node) > 0
                        and isinstance(next_from_node[0], Send)
                    ):
                ...
                if send_objects_to_execute:
                    send_results = await self._execute_send_objects(
                        send_objects_to_execute, state, config
                    )
                    for send_updates in send_results:
                        state = self._merge_state(state, send_updates)
    ...
    async def _execute_nodes_parallel(
        ...
        for node_name in node_names:
            ...
            task = self._execute_node_with_policies(
                node_name, node_config, state, config or {}
            )
        ...
        for i, (node_name, _) in enumerate(tasks):
            ...
            if isinstance(result, InterruptException):
                ...
                raise RuntimeError(
                    f"Error in parallel node '{node_name}': {str(result)}"
                ) from result
            else:
    ...
    async def _execute_node_with_policies(
        ...
        if node_config.retry_policy:
            result = await self._execute_with_retry(
                node_config.func, state, node_config.retry_policy, config
            )
        else:
        ...
        if node_config.cache_policy and self.cache:
            self.cache.put(
                (node_name,), cache_key, result, ttl=node_config.cache_policy.ttl
            )
    ...
    async def _execute_send_objects(
        self, send_objects: List[Send], state: Dict[str, Any], config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for i, result in enumerate(task_results):
            if isinstance(result, Exception):
                ...
                raise RuntimeError(
                    f"Error in Send task for node '{node_name}': {str(result)}"
                ) from result
            else:
    ...
    def _resolve_goto(
        self, goto: Union[str, Send, List[Send], List[str]]
    ) -> Union[List[str], List[Send]]:
    ...
    async def _save_checkpoint_with_durability(
        ...
        if self.durability == "sync":
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, self.checkpointer.put, checkpoint
            )
    ...
    async def _resume_from_interrupt(
        ...
        """
        from upsonic.graphv2.primitives import (
            _interrupt_resume_values,
            _interrupt_counter,
        )

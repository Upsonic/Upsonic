...
class ReliabilityEvaluator:
    ...
    def __init__(
        ...
        if not isinstance(expected_tool_calls, list) or not all(
            isinstance(i, str) for i in expected_tool_calls
        ):
    ...
    def run(
        self, run_result: Union[Task, List[Task], Graph], print_results: bool = True
        ...
        for expected_tool in self.expected_tool_calls:
            ...
            checks.append(
                ToolCallCheck(
                    tool_name=expected_tool, was_called=was_called, times_called=count
                )
            )
            if not was_called:
                passed = False
        ...
        if missing_tool_calls:
            summary_messages.append(
                f"Missing expected tool calls: {', '.join(missing_tool_calls)}."
            )
        ...
        if self.exact_match:
            ...
            if unexpected_set:
                ...
                summary_messages.append(
                    f"Unexpected tools were called: {', '.join(unexpected_tool_calls)}."
                )
    ...
    def _normalize_tool_call_history(
        self, run_result: Union[Task, List[Task], Graph]
    ) -> List[str]:
        """Extracts a single, flat list of tool call names from the run result."""
        actual_tool_calls = []

        if isinstance(run_result, Task):
            actual_tool_calls.extend(
                call["tool_name"] for call in run_result.tool_calls
            )

        elif isinstance(run_result, list) and all(
            isinstance(t, Task) for t in run_result
        ):
            ...
            for node in run_result.nodes:
                if isinstance(node, TaskNode) and node.id in executed_node_ids:
                    actual_tool_calls.extend(
                        call["tool_name"] for call in node.task.tool_calls
                    )
        else:
            raise TypeError(

...
class ChatMessage:
    ...
    def from_model_message(cls, message: "ModelMessage") -> "ChatMessage":
        ...
        if isinstance(message, ModelRequest):
                        ...
                        for item in part.content:
                            if isinstance(item, str):
                                ...
                                if hasattr(item, "url"):
                                    ...
                                    attachments.append(
                                        f"{type(item).__name__}: {item.url}"
                                    )
                                elif hasattr(item, "data"):
                                    # Handle BinaryContent
                                    attachments.append(
                                        f"{type(item).__name__}: {item.identifier or 'binary_data'}"
                                    )
                                else:
                                    # Fallback for unknown content types
                                    attachments.append(
                                        f"{type(item).__name__}: {str(item)}"
                                    )
            ...
            return cls(content=content, role="assistant", timestamp=time.time())

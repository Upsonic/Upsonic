...
class Chat:
    ...
    def __init__(
        ...
        *,
        ...
        if self.debug:
            ...
            debug_log(
                f"Chat initialized: session_id={session_id}, user_id={user_id}", "Chat"
            )
    ...
    def _normalize_input(
        self, input_data: Union[str, Task], attachments: Optional[List[str]] = None
    ) -> Task:
        ...
        if isinstance(input_data, str):
            ...
            return Task(description=input_data.strip(), attachments=attachments)
            ...
            raise TypeError(
                f"Unsupported input type: {type(input_data)}. Expected str or Task, got {type(input_data)}"
            )
            ...
            try:
                ...
                if attempt < self._retry_attempts:
                    ...
                    if self.debug:
                        ...
                        debug_log(
                            f"All {self._retry_attempts + 1} attempts failed. Last error: {e}",
                            "Chat",
                        )
        ...
        **kwargs,
        ...
        if not self._session_manager.can_accept_invocation():
            if self._session_manager.state == SessionState.ERROR:
                ...
                raise RuntimeError(
                    f"Maximum concurrent invocations exceeded. Current: {current}, Max allowed: {max_allowed}. Wait for current operations to complete or increase max_concurrent_invocations."
                )
        ...
        self._transition_state(
            SessionState.STREAMING if stream else SessionState.AWAITING_RESPONSE
        )
        ...
        if stream:
            ...
            return await self._invoke_blocking_async(
                task, response_start_time, **kwargs
            )

    async def _invoke_blocking_async(
        self, task: Task, response_start_time: float, **kwargs
    ) -> str:
    ...
    def _invoke_streaming(
        ...
        async def _execute_streaming():
            ...
            stream_result = await self.agent.stream_async(
                task, debug=self.debug, **kwargs
            )
            ...
            stream_result.get_final_output()
                    ...
                    try:
                        ...
                        if "context manager is already active" in str(e):
                            if self.debug:
                                ...
                                debug_log(
                                    f"Streaming context manager conflict on attempt {attempt + 1}. Waiting longer...",
                                    "Chat",
                                )
                            await asyncio.sleep(
                                self._retry_delay * (3**attempt)
                            )  # Longer wait for context conflicts
                        elif attempt < self._retry_attempts:
                            if self.debug:
                                from upsonic.utils.printing import debug_log

                                debug_log(
                                    f"Streaming attempt {attempt + 1} failed: {e}. Retrying in {self._retry_delay}s...",
                                    "Chat",
                                )
                            ...
                            if self.debug:
                                ...
                                debug_log(
                                    f"All {self._retry_attempts + 1} streaming attempts failed. Last error: {e}",
                                    "Chat",
                                )
                            raise last_exception
            finally:

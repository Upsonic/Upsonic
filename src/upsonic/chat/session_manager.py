...
class SessionManager:
    ...
    def __init__(
        ...
        self._metrics = SessionMetrics(
            session_id=session_id, user_id=user_id, start_time=self._session_start_time
        )
    ...
    def can_accept_invocation(self) -> bool:
        ...
        return (
            ...
            and self._concurrent_invocations < self._max_concurrent_invocations
        )
    ...
    def add_message(self, message: ChatMessage) -> None:
        ...
        if self.debug:
            ...
            debug_log(
                f"Added {message.role} message to session history", "SessionManager"
            )
    ...
    def add_usage(self, usage: RequestUsage, model: Optional[Model] = None) -> None:
        ...
        if self.debug:
            ...
            individual_cost = (
                self._cost_tracker._cost_history[-1].estimated_cost
                if self._cost_tracker._cost_history
                else 0.0
            )
            from upsonic.utils.printing import info_log

            info_log(
                f"Added usage: {input_tokens} in, {output_tokens} out, ${individual_cost:.4f} (cumulative: {self._cost_tracker.input_tokens} in, {self._cost_tracker.output_tokens} out, ${self._cost_tracker.total_cost:.4f})",
                "SessionManager",
            )

...
class ChatPromptTemplate(Runnable[dict[str, Any], Union[str, ModelRequest]]):
    ...
    def __init__(
        ...
        messages: Optional[
            List[Union[Tuple[str, str], Tuple[Literal["placeholder"], Dict[str, Any]]]]
        ] = None,
    ):
    ...
    def from_template(cls, template: str) -> "ChatPromptTemplate":
        ...
        return cls(
            template=template, input_variables=input_variables, messages=messages
        )
    ...
    def from_messages(
        cls,
        messages: List[
            Union[Tuple[str, str], Tuple[Literal["placeholder"], Dict[str, Any]]]
        ],
    ) -> "ChatPromptTemplate":
    ...
    def invoke(
    ...
    ) -> Union[str, ModelRequest, List[Union[ModelRequest, ModelResponse]]]:
        ...
        if not isinstance(input, dict):
            raise TypeError(
                f"ChatPromptTemplate expects a dict input, got {type(input)}"
            )
        ...
        if self.is_message_template:
                ...
                if isinstance(msg, tuple) and len(msg) == 2:
                    ...
                    if role == "placeholder" and isinstance(content, dict):
                        ...
                        if var_name and var_name in input:
                            ...
                            if isinstance(placeholder_value, list):
                                ...
                                for hist_item in placeholder_value:
                                    if (
                                        ...
                                        if current_request_parts:
                                            ...
                                            if (
                                                system_prompt
                                                and not has_system_in_first
                                            ):
                                                current_request_parts.insert(
                                                    0, system_prompt
                                                )
                                        ...
                                        if hist_role == "human" or hist_role == "user":
                                            ...
                                            hist_role == "ai"
                                            or hist_role == "assistant"
                                        ):
                                            # Create a ModelResponse for AI message
                                            conversation_messages.append(
                                                ModelResponse(
                                                    parts=[
                                                        TextPart(content=hist_content)
                                                    ],
                                                    usage=RequestUsage(
                                ...
                                if current_request_parts and isinstance(
                                    current_request_parts[-1], UserPromptPart
                                ):
                                    # Combine with existing UserPromptPart
                                    current_request_parts[-1] = UserPromptPart(
                                        content=current_request_parts[-1].content
                                        + " "
                                        + placeholder_value,
                                        timestamp=now_utc(),
                                    )
                    ...
                    if isinstance(content, str):
                        try:
                            ...
                            raise KeyError(
                                f"Template variable {e} not found in input: {input}"
                            )
                        ...
                        if role == "system":
                            ...
                            if not system_prompt:
                                system_prompt = SystemPromptPart(
                                    content=formatted_content, timestamp=now_utc()
                                )
                            ...
                            conversation_messages.append(
                                ModelResponse(
                                    parts=[TextPart(content=formatted_content)],
            ...
            if current_request_parts:
                ...
                conversation_messages.append(ModelRequest(parts=current_request_parts))

            # Handle case where we only have a system prompt and no other parts
            if not conversation_messages and system_prompt:
                conversation_messages.append(ModelRequest(parts=[system_prompt]))

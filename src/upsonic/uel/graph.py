...
class RunnableNode:
    ...
    def __init__(self, runnable: Runnable, node_id: int):
        ...
        self.parallel_branches: List[
            Tuple[str, int]
        ] = []  # (key_name, node_id) for parallel branches
...
class RunnableGraph:
            ...
            if node.parallel_branches:
                for i, (key_name, branch_id) in enumerate(node.parallel_branches):
                    ...
                    if branch_node:
                        ...
                        if is_last:
                            lines.append(
                                f"{branch_indent}└─> [{key_name}] {branch_node.name}"
                            )
                        else:
                            lines.append(
                                f"{branch_indent}├─> [{key_name}] {branch_node.name}"
    ...
    def get_structure_details(self) -> str:
        ...
        for node_id, node in sorted(self.nodes.items()):
            ...
            if node.parallel_branches:
                branch_info = [
                    f"'{key}' -> Node {bid}" for key, bid in node.parallel_branches
                ]
                ...
                if node.edges_to:
                    ...
                    lines.append(
                        f"  Merge: {', '.join(merge_info)} --> Node {next_node}"
                    )
    ...
    def __repr__(self) -> str:
        ...
        num_parallel_branches = sum(
            len(node.parallel_branches) for node in self.nodes.values()
        )

"""Synchronous SQLite storage implementation for Upsonic agent framework."""
...
class SqliteStorage(Storage):
    ...
    This storage backend uses SQLAlchemy for synchronous SQLite operations.
    ...
    def __init__(
        ...
        """
        Initialize the synchronous SQLite storage.
    ...
    def table_exists(self, table_name: str) -> bool:
        ...
        with self.Session() as sess:
            return is_table_available_sync(sess, table_name)
            ...
            for idx in table.indexes:
                try:
                    with self.Session() as sess:
                        ...
                        result = sess.execute(exists_query, {"index_name": idx.name})
    ...
    def _get_or_create_table(
        ...
        with self.Session() as sess:
            table_available = is_table_available_sync(sess, table_name)
    ...
    def _get_generic_model_table(self, collection: str, create_if_not_found: bool = True) -> Table:
        ...
        with self.Session() as sess:
            table_exists = is_table_available_sync(sess, table_name)
        if table_exists:

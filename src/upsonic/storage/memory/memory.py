...
class Memory:
    ...
    def __init__(
        ...
        if self.full_session_memory_enabled or self.summary_memory_enabled:
            if not session_id:
                raise ValueError(
                    "`session_id` is required when full_session_memory or summary_memory is enabled."
                )
        ...
        if self.user_analysis_memory_enabled:
            if not user_id:
                raise ValueError(
                    "`user_id` is required when user_analysis_memory is enabled."
                )
    ...
    async def prepare_inputs_for_task(self) -> Dict[str, Any]:
        ...
        if self.user_analysis_memory_enabled and self.user_id:
            ...
            if profile and profile.profile_data:
                profile_str = "\n".join(
                    f"- {key}: {value}" for key, value in profile.profile_data.items()
                )
                prepared_data["system_prompt_injection"] = (
                    f"<UserProfile>\n{profile_str}\n</UserProfile>"
                )
        ...
        if self.session_id:
            ...
            if session:
                if self.summary_memory_enabled and session.summary:
                    prepared_data["context_injection"] = (
                        f"<SessionSummary>\n{session.summary}\n</SessionSummary>"
                    )
                if self.full_session_memory_enabled and session.chat_history:
                    try:
                        ...
                        validated_history = ModelMessagesTypeAdapter.validate_python(
                            raw_messages
                        )
                        ...
                        warning_log(
                            f"Could not validate or process stored chat history. Starting fresh. Error: {e}",
                            "MemoryStorage",
                        )
    ...
    async def _update_interaction_session(self, model_response):
        ...
        if (
            not (self.full_session_memory_enabled or self.summary_memory_enabled)
        ...
        if self.full_session_memory_enabled:
            ...
            session.chat_history.extend(
                all_messages_as_dicts
            )  # Store as a list of messages

        if self.summary_memory_enabled:
            try:
                session.summary = await self._generate_new_summary(
                    session.summary, model_response
                )
            except Exception as e:
                from upsonic.utils.printing import warning_log
    ...
    async def _update_user_profile(self, model_response):
        ...
        if self.user_analysis_memory_enabled:
            try:
                updated_traits = await self._analyze_interaction_for_traits(
                    profile.profile_data, model_response
                )
    ...
    def _limit_message_history(self, message_history: List) -> List:
        ...
        for i in range(0, len(message_history) - 1, 2):
            ...
            if isinstance(request, ModelRequest) and isinstance(
        ...
        if not original_system_prompt:
            ...
            warning_log(
                "Could not find original SystemPromptPart. History might be malformed.",
                "MemoryStorage",
            )
        ...
        if not new_user_prompt:
            ...
            warning_log(
                "Could not find UserPromptPart in the first message of the limited window.",
                "MemoryStorage",
            )
            return [message for run in kept_runs for message in run]
        ...
        info_log(
            f"Original history had {len(all_runs)} runs. "
...
"""
    ...
    async def _analyze_interaction_for_traits(
        self, current_profile: dict, model_response
    ) -> dict:
        """
        ...
        if not self.model:
            raise ValueError(
                "A model must be configured on the Memory object to analyze user traits."
            )
        ...
        if session and session.chat_history:
            try:
                ...
                historical_prompts_content = self._extract_user_prompt_content(
                    validated_history
                )
            except Exception as e:
                from upsonic.utils.printing import warning_log
                ...
                warning_log(
                    f"Could not validate session history. It will be skipped for analysis. Error: {e}",
                    "MemoryStorage",
                )
        ...
        info_log(
            f"Analyzing traits using context from: {', '.join(source_log)}.",
            context="Memory",
        )
...
"""
            schema_task = Task(
                description=schema_generator_prompt, response_format=ProposedSchema
            )

            try:
                proposed_schema_response = await analyzer.do_async(schema_task)
                field_count = (
                    len(proposed_schema_response.fields)
                    if proposed_schema_response
                    and hasattr(proposed_schema_response, "fields")
                    else 0
                )
                info_log(f"LLM generated schema with {field_count} fields", "Memory")
                if field_count > 0:
                    info_log(
                        f"Generated field names: {[f.name for f in proposed_schema_response.fields]}",
                        "Memory",
                    )
            except Exception as e:
                warning_log(
                    f"Dynamic schema generation failed with error: {e}. No user traits extracted.",
                    "Memory",
                )
                return {}

            if (
                not proposed_schema_response
                or not hasattr(proposed_schema_response, "fields")
                or not proposed_schema_response.fields
            ):
                field_count = (
                    len(proposed_schema_response.fields)
                    if proposed_schema_response
                ...
                info_log(
                    f"Schema generation result: {field_count} fields generated",
                    "Memory",
                )
                warning_log(
                    f"Dynamic schema generation returned {field_count} fields (expected at least 2). No user traits extracted.",
                    "Memory",
                )
                return {}
            ...
            dynamic_fields = {
                field_def.name: (
                    Optional[str],
                    Field(None, description=field_def.description),
                )
                for field_def in proposed_schema_response.fields
            }
            DynamicUserTraitModel = create_model(
                "DynamicUserTraitModel", **dynamic_fields
            )
...
"""
            trait_task = Task(
                description=trait_extractor_prompt,
                response_format=DynamicUserTraitModel,
            )

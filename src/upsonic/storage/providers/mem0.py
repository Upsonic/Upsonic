...
class Mem0Storage(Storage):
    ...
    async def _initialize_client(self) -> Union[Memory, AsyncMemoryClient]:
        ...
        try:
            if self._api_key:
                ...
                if self._all_categories:
                    try:
                        await client.update_project(
                            custom_categories=self._all_categories
                        )
                    except Exception as e:
                        from upsonic.utils.printing import warning_log

                        warning_log(
                            f"Could not update project categories: {e}", "Mem0Storage"
                        )
                ...
                info_log(
                    "Initializing Mem0 Open Source client with config", "Mem0Storage"
                )
                ...
                info_log(
                    "Initializing Mem0 Open Source client with defaults", "Mem0Storage"
                )
                return Memory()
    ...
    def _deserialize_session(self, memory_dict: Dict[str, Any]) -> InteractionSession:
        ...
        try:
            memory_data = (
                json.loads(memory_text) if isinstance(memory_text, str) else {}
            )
        ...
        return InteractionSession(
            session_id=SessionId(
                metadata.get("session_id", memory_data.get("session_id", ""))
            ),
            ...
            session_data=json.loads(metadata["session_data"])
            if metadata.get("session_data")
            else {},
    ...
    async def _resolve_memory_id(
        self, object_id: str, model_type: Type[BaseModel]
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            warning_log(
                f"Failed to resolve memory ID for {object_id}: {e}", "Mem0Storage"
            )
            return None
    ...
    async def read_async(self, object_id: str, model_type: Type[T]) -> Optional[T]:
        ...
        try:
            ...
            warning_log(
                f"Failed to read from Mem0 (id={object_id}): {e}", "Mem0Storage"
            )
            return None
    ...
    async def delete_async(self, object_id: str, model_type: Type[BaseModel]) -> None:
        ...
        try:
            ...
            if memory_id:
                ...
                warning_log(
                    f"Memory not found for deletion: {object_id}", "Mem0Storage"
                )
            ...
            error_log(
                f"Failed to delete from Mem0 (id={object_id}): {e}", "Mem0Storage"
            )
            raise
    ...
    async def drop_async(self) -> None:
        ...
        try:
            ...
            if self._is_platform_client:
                ...
                all_memories = [
                    m
                    for m in all_memories_raw
                    if m.get("metadata", {}).get("namespace") == self.namespace
                    and m.get("metadata", {}).get("category")
                    in [self.CATEGORY_SESSION, self.CATEGORY_PROFILE]
                ]
                ...
                if hasattr(client, "reset"):
                    ...
                    warning_log(
                        "Reset not supported in this Mem0 version", "Mem0Storage"
                    )

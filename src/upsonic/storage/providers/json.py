...
class JSONStorage(Storage):
    ...
    def _get_path(self, object_id: str, model_type: Type[BaseModel]) -> Path:
        ...
        raise TypeError(
            f"Unsupported model type for path generation: {model_type.__name__}"
        )
    ...
    async def read_async(self, object_id: str, model_type: Type[T]) -> Optional[T]:
        ...
        async with self.lock:
            ...
            try:
                ...
                warning_log(
                    f"Could not parse file {file_path}. Error: {e}", "JSONStorage"
                )
                return None
        ...
        async with self.lock:
            if await asyncio.to_thread(file_path.exists):
                try:
                    ...
                    error_log(
                        f"Could not delete file {file_path}. Reason: {e}", "JSONStorage"
                    )

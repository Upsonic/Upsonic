...
class KnowledgeBase:
    ...
    def __init__(
        ...
        **config_kwargs,
        ...
        if not sources:
            raise ValueError(
                "KnowledgeBase must be initialized with at least one source."
            )
        ...
        if loaders is None:
            ...
            try:
                ...
                info_log(
                    f"Created {len(self.loaders)} intelligent loaders",
                    context="KnowledgeBase",
                )
        ...
        if splitters is None:
            ...
            try:
                ...
                info_log(
                    f"Created {len(self.splitters)} intelligent splitters",
                    context="KnowledgeBase",
                )
            except Exception as e:
                warning_log(
    ...
    def _resolve_sources(
        self, sources: Union[str, Path, List[Union[str, Path]]]
    ) -> List[Union[str, Path]]:
        """
        ...
        for item in source_list:
            ...
            try:
                ...
                if path_item.is_file():
                    ...
                    supported_files = self._get_supported_files_from_directory(
                        path_item
                    )
    ...
    def _normalize_splitters(
        self, splitters: Union[BaseChunker, List[BaseChunker]]
    ) -> List[BaseChunker]:
        ...
        if isinstance(splitters, list):
            ...
            raise ValueError(
                "Splitters must be a BaseChunker or list of BaseChunker instances"
            )

    def _normalize_loaders(
        self, loaders: Optional[Union[BaseLoader, List[BaseLoader]]]
    ) -> List[BaseLoader]:
        ...
        if loaders is None:
            ...
            raise ValueError(
                "Loaders must be a BaseLoader or list of BaseLoader instances"
            )
    ...
    def _validate_component_counts(self):
        ...
        file_source_count = sum(
            1 for source in self.sources if isinstance(source, Path)
        )
    ...
    def _is_direct_content(self, source: str) -> bool:
        ...
        if len(words) > 5 and not any(
            word.startswith("/") or word.startswith(".") for word in words
        ):
            return True
    ...
    def _create_document_from_content(
        self, content: str, source_index: int
    ) -> Document:
        """
        ...
        return Document(content=content, metadata=metadata, document_id=content_hash)

    def _get_component_for_source(
        self, source_index: int, component_list: List, component_name: str
    ):
        """
        ...
        if len(component_list) == 1:
            ...
            warning_log(
                f"{component_name} index {source_index} out of range, using first {component_name}",
                "KnowledgeBase",
            )
            return component_list[0]
    ...
    async def setup_async(self) -> None:
        ...
        async with self._setup_lock:
            ...
            if self.vectordb.collection_exists():
                info_log(
                    f"KnowledgeBase '{self.name}' is already indexed. Setup is complete.",
                    context="KnowledgeBase",
                )
                self._is_ready = True
                return
            ...
            info_log(
                f"KnowledgeBase '{self.name}' not found in vector store. Starting indexed indexing process...",
                context="KnowledgeBase",
            )
            ...
            for source_index, source in enumerate(self.sources):
                ...
                info_log(
                    f"Processing source {source_index}: {source_str[:100]}{'...' if len(source_str) > 100 else ''}",
                    context="KnowledgeBase",
                )

                if isinstance(source, str):
                    info_log(
                        "Detected direct content, creating document directly...",
                        context="KnowledgeBase",
                    )
                    try:
                        document = self._create_document_from_content(
                            source, source_index
                        )
                        ...
                        success_log(
                            f"Created document from direct content (length: {len(source)})",
                            "KnowledgeBase",
                        )
                        ...
                        error_log(
                            f"Error creating document from direct content: {e}",
                            "KnowledgeBase",
                        )
                        continue
                else:
                    if self.loaders:
                        loader = self._get_component_for_source(
                            source_index, self.loaders, "loader"
                        )
                        info_log(
                            f"Using loader: {loader.__class__.__name__}",
                            context="KnowledgeBase",
                        )

                        info_log(
                            f"Checking if {loader.__class__.__name__} can load {source}...",
                            context="KnowledgeBase",
                        )
                        can_load_result = loader.can_load(source)
                        info_log(
                            f"can_load result: {can_load_result}",
                            context="KnowledgeBase",
                        )
                        ...
                        if can_load_result:
                            try:
                                ...
                                success_log(
                                    f"Loaded {len(source_documents)} documents from {source}",
                                    "KnowledgeBase",
                                )
                                all_documents.extend(source_documents)
                                source_to_documents[source_index] = source_documents
                                ...
                                error_log(
                                    f"Error loading {source}: {e}", "KnowledgeBase"
                                )
                                continue
                        else:
                            from upsonic.utils.printing import warning_log
                        ...
                        warning_log(
                            f"No loaders provided for {source}", "KnowledgeBase"
                        )
                        continue
            ...
            info_log(
                f"[Step 2/4] Chunking {len(all_documents)} documents with indexed splitters...",
                context="KnowledgeBase",
            )
            all_chunks = []
            chunks_per_source = {}
            ...
            for source_index in sorted(source_to_documents.keys()):
                ...
                splitter = self._get_component_for_source(
                    source_index, self.splitters, "splitter"
                )
                ...
                for doc in documents:
                    ...
                    debug_log(
                        f"Document '{doc.document_id}' split into {len(doc_chunks)} chunks",
                        context="KnowledgeBase",
                    )
                ...
                debug_log(
                    f"Source {source_index} total chunks: {len(source_chunks)}",
                    context="KnowledgeBase",
                )

            info_log(
                f"Summary: Total chunks created: {len(all_chunks)}",
                context="KnowledgeBase",
            )
            for source_index, chunks in chunks_per_source.items():
                debug_log(
                    f"Source {source_index}: {len(chunks)} chunks",
                    context="KnowledgeBase",
                )

            info_log(
                f"[Step 3/4] Creating embeddings for {len(all_chunks)} chunks...",
                context="KnowledgeBase",
            )
            vectors = await self.embedding_provider.embed_documents(all_chunks)
            info_log(
                f"Created embeddings for {len(vectors)} chunks", context="KnowledgeBase"
            )

            info_log(
                f"[Step 4/4] Storing {len(all_chunks)} chunks in vector database...",
                context="KnowledgeBase",
            )
            ...
            self._is_ready = True
            info_log(
                f"KnowledgeBase '{self.name}' indexing completed successfully!",
                context="KnowledgeBase",
            )
    ...
    async def query_async(self, query: str) -> List[RAGSearchResult]:
        ...
        info_log(
            f"Querying KnowledgeBase '{self.name}' with: '{query}'",
            context="KnowledgeBase",
        )

        query_vector = await self.embedding_provider.embed_query(query)

        if hasattr(self.vectordb, "_config") and hasattr(
            self.vectordb._config, "search"
        ):
            if self.vectordb._config.search.hybrid_search_enabled:
                ...
                search_results = self.vectordb.search(query_vector=query_vector)
        else:
            search_results = self.vectordb.search(
                query_vector=query_vector, query_text=query
            )
        ...
        if len(rag_results) == 0:
            ...
            warning_log(
                f"No results found for KnowledgeBase '{self.name}' with query: '{query}'",
                "KnowledgeBase",
            )
        ...
        success_log(
            f"Successfully processed {len(rag_results)} results for RAG",
            "KnowledgeBase",
        )
        return rag_results
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "embedding_provider": {"class": self.embedding_provider.__class__.__name__},
            "vectordb": self.vectordb.get_config_summary()
            if hasattr(self.vectordb, "get_config_summary")
            else {"class": self.vectordb.__class__.__name__},
        }
        ...
        try:
            try:
                if hasattr(self.embedding_provider, "validate_connection"):
                    embedding_health = (
                        await self.embedding_provider.validate_connection()
                    )
    ...
    def __del__(self):
        ...
        try:
            if (
                hasattr(self, "_is_ready")
                and self._is_ready
                and not getattr(self, "_is_closed", False)
            ):
                try:
                            ...
                            try:
                                ...
                                if loop.is_running():
                                    ...
                                    loop.run_until_complete(
                                        self.vectordb.disconnect_async()
                                    )
                            except RuntimeError:

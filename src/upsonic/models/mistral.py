...
try:
    ...
    raise ImportError("Please install `mistral` to use the Mistral model.") from e
...
class MistralModel(Model):
    ...
    async def request(
        ...
        response = await self._completions_create(
            messages,
            cast(MistralModelSettings, model_settings or {}),
    ...
    async def _completions_create(
        ...
        try:
            response = await self.client.chat.complete_async(
                ...
                n=1,
                tools=self._map_function_and_output_tools_definition(
                    model_request_parameters
                )
                or UNSET,
    ...
    async def _stream_completions_create(
        ...
        if model_request_parameters.function_tools:
            ...
            user_output_format_message = self._generate_user_output_format(
                parameters_json_schemas
            )
    ...
    def _get_tool_choice(
        ...
        """
        if (
            not model_request_parameters.function_tools
    ...
    def _process_response(
        ...
        if isinstance(tool_calls, list):
            for tool_call in tool_calls:
                tool = self._map_mistral_to_upsonic_tool_call(tool_call=tool_call)
                parts.append(tool)
    ...
    @staticmethod
    def _map_mistral_to_upsonic_tool_call(tool_call: MistralToolCall) -> ToolCallPart:
    ...
    def _generate_user_output_format(
        ...
        return MistralUserMessage(
            content=self.json_mode_schema_prompt.format(schema=example_schema)
        )
    ...
    def _get_python_type(cls, value: dict[str, Any]) -> str:
        ...
        if (
            value_type in SIMPLE_JSON_TYPE_MAPPING
        ...
        if value_type == "object":
            ...
            if (
                ...
                return (
                    f"dict[str, {SIMPLE_JSON_TYPE_MAPPING[additional_properties_type]}]"
                )
    ...
    def _map_messages(self, messages: list[ModelMessage]) -> list[MistralMessages]:
            ...
            if isinstance(message, ModelRequest):
                ...
                if thinking_chunks:
                    content_chunks.insert(
                        0, MistralThinkChunk(thinking=thinking_chunks)
                    )
                mistral_messages.append(
                    MistralAssistantMessage(
                        content=content_chunks, tool_calls=tool_calls
                    )
                )
            else:
        ...
        for i, current_message in enumerate(mistral_messages):
            ...
            if isinstance(current_message, MistralToolMessage) and i + 1 < len(
                mistral_messages
            ):
        ...
        if isinstance(part.content, str):
            ...
            for item in part.content:
                if isinstance(item, str):
                    ...
                    content.append(
                        MistralImageURLChunk(image_url=MistralImageURL(url=item.url))
                    )
                elif isinstance(item, BinaryContent):
                    base64_encoded = base64.b64encode(item.data).decode("utf-8")
                    if item.is_image:
                        image_url = MistralImageURL(
                            url=f"data:{item.media_type};base64,{base64_encoded}"
                        )
                        content.append(
                            MistralImageURLChunk(image_url=image_url, type="image_url")
                        )
                        ...
                        raise RuntimeError(
                            "BinaryContent other than image or PDF is not supported in Mistral."
                        )
                elif isinstance(item, DocumentUrl):
                    if item.media_type == "application/pdf":
                        content.append(
                            MistralDocumentURLChunk(
                                document_url=item.url, type="document_url"
                            )
                        )
                    else:
                        raise RuntimeError(
                            "DocumentUrl other than PDF is not supported in Mistral."
                        )
...
class MistralStreamedResponse(StreamedResponse):
        ...
        async for chunk in self._response:
            ...
            for thought in thinking:
                self._parts_manager.handle_thinking_delta(
                    vendor_part_id="thinking", content=thought
                )
            if text:
                # Attempt to produce an output tool call from the received text
                output_tools = {
                    c.name: c for c in self.model_request_parameters.output_tools
                }
                if output_tools:
                    self._delta_content += text
                    # TODO: Port to native "manual JSON" mode
                    maybe_tool_call_part = self._try_get_output_tool_from_text(
                        self._delta_content, output_tools
                    )
                    ...
                    maybe_event = self._parts_manager.handle_text_delta(
                        vendor_part_id="content", content=text
                    )
    ...
    @staticmethod
    def _try_get_output_tool_from_text(
        text: str, output_tools: dict[str, ToolDefinition]
    ) -> ToolCallPart | None:
        output_json: dict[str, Any] | None = pydantic_core.from_json(
            text, allow_partial="trailing-strings"
        )
        if output_json:
            ...
            if param_type == "array" and param_items_type:
                ...
                for item in json_dict[param]:
                    if not isinstance(item, VALID_JSON_TYPE_MAPPING[param_items_type]):
                        return False
            elif param_type and not isinstance(
                json_dict[param], VALID_JSON_TYPE_MAPPING[param_type]
            ):
                return False
...
def _map_content(
    content: MistralOptionalNullable[MistralContent],

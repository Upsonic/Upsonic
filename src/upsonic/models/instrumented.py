...
from upsonic.models import KnownModelName, Model, ModelRequestParameters, StreamedResponse
from upsonic.models.wrapper import WrapperModel

__all__ = 'instrument_model', 'InstrumentationSettings', 'InstrumentedModel'

DEFAULT_INSTRUMENTATION_VERSION = 4
...
def instrument_model(model: Model, instrument: InstrumentationSettings | bool) -> Model:
    """Instrument a model with OpenTelemetry."""
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            version: Version of the data format. This is unrelated to the Upsonic package version.
                ...
                Version 2 uses the newer OpenTelemetry GenAI spec and stores messages in the following attributes:
                    ...
                    - `upsonic.all_messages` on agent run spans.
        ...
        if self.version == 1:
            ...
            if self.include_content:
                attributes['gen_ai.input.messages'] = json.dumps(
                    self.messages_to_otel_messages(input_messages)
                )
                attributes['gen_ai.output.messages'] = json.dumps([output_message])
                attributes.update(system_instructions_attributes)
                schema_properties['gen_ai.input.messages'] = {'type': 'array'}
                schema_properties['gen_ai.output.messages'] = {'type': 'array'}

...
class ModelRecommendation(BaseModel):
    ...
    alternative_models: List[str] = Field(
        default_factory=list, description="Alternative models that could also work"
    )
    ...
    def _score_model(
        ...
        if criteria:
            ...
            if criteria.required_capabilities:
                if not all(
                    cap in model.capabilities for cap in criteria.required_capabilities
                ):
    ...
    def select_model(
        ...
        if capability_scores:
            top_capabilities = sorted(
                capability_scores.items(), key=lambda x: x[1], reverse=True
            )[:3]
...
class LLMBasedSelector:
    ...
    async def select_model_async(
        ...
        # Build the prompt
        prompt = self._build_selection_prompt(task_description, model_info, criteria)
        ...
        selection_task = Task(
            description=prompt, response_format=ModelRecommendation, not_main_task=True
        )
        ...
        try:
            ...
            recommendation = rule_selector.select_model(
                task_description, criteria, default_model
            )
    ...
    def _prepare_model_info_for_llm(self) -> str:
        ...
        for provider, models in sorted(providers.items()):
            ...
            for name, meta in unique_models:
                caps = ", ".join(
                    [c.value.replace("_", " ") for c in meta.capabilities[:5]]
                )
                bench_summary = ""
                if meta.benchmarks:
                    bench_summary = (
                        f" (MMLU: {meta.benchmarks.mmlu:.1f}, Code: {meta.benchmarks.humaneval:.1f})"
                        if meta.benchmarks.mmlu and meta.benchmarks.humaneval
                        else ""
                    )
        ...
        if criteria:
            ...
            if criteria.max_cost_tier:
                criteria_parts.append(
                    f"- Maximum cost tier: {criteria.max_cost_tier}/10"
                )
            if criteria.min_context_window:
                criteria_parts.append(
                    f"- Minimum context window: {criteria.min_context_window} tokens"
                )
            if criteria.preferred_provider:
                criteria_parts.append(
                    f"- Preferred provider: {criteria.preferred_provider}"
                )
            if criteria.require_open_source:
            ...
            if criteria.requires_vision:
                criteria_parts.append(
                    "- REQUIRED: Vision/image processing capabilities"
                )
            if criteria.requires_audio:
...
async def select_model_async(
    ...
    if use_llm:
        selector = LLMBasedSelector(agent)
        return await selector.select_model_async(
            task_description, criteria, default_model
        )
    else:
        selector = RuleBasedSelector()

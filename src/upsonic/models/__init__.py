...
class Model(Runnable[Any, Any]):
    ...
    def add_memory(self, history: bool = False, memory: Any = None) -> "Model":
        ...
        if memory is not None:
            ...
            self._memory = Memory(
                storage=storage, session_id=session_id, full_session_memory=True
            )

        return self

    def bind_tools(
        self, tools: list[Any], *, tool_call_limit: int | None = None
    ) -> "Model":
    ...
    async def ainvoke(
        ...
        if self._tools:
            response = await self._handle_response_with_tools(
                response, messages, model_params
            )
    ...
    def _prepare_messages_for_invoke(
        self, input: str | "ModelRequest" | list["ModelMessage"]
    ) -> list["ModelMessage"]:
    ...
    def _setup_tools_for_invoke(self) -> None:
        ...
        if not self._tool_context:
            self._tool_context = ToolContext(
                deps=None, agent_id="uel_model", max_retries=1, tool_call_limit=5
            )
    ...
    def _build_parameters_for_invoke(self) -> ModelRequestParameters:
        ...
        if self._tools and self._tool_manager:
            ...
            builtin_tools = self._tool_manager.processor.extract_builtin_tools(
                self._tools
            )
        ...
        if (
            ...
            and self._response_format != str
            and self._response_format is not str
        ):
            if isinstance(self._response_format, type) and issubclass(
                self._response_format, BaseModel
            ):
                output_mode = "native"
    ...
    async def _handle_response_with_tools(
        ...
        return await self._handle_response_with_tools(
            follow_up_response, messages, model_params
        )
    ...
    async def _save_to_memory(
        self, messages: list["ModelMessage"], response: "ModelResponse"
    ) -> None:
    ...
    def __aiter__(self) -> AsyncIterator[ModelResponseStreamEvent]:
        ...
        if self._event_iterator is None:
            ...
            async def iterator_with_final_event(
                ...
                final_result_info: tuple[str | None, str | None] | None = (
                    None  # (tool_name, tool_call_id)
                )
                ...
                async for event in iterator:
                    ...
                    if isinstance(event, PartStartEvent):
                        ...
                        if (
                            result_event := _get_final_result_event(
                ...
                if (
                    final_result_part_index is not None
                    and final_result_info is not None
                ):
...
def _customize_output_object(
    transformer: type[JsonSchemaTransformer], o: OutputObjectDefinition
):
...
def _get_final_result_event(
    ...
    if isinstance(e, PartStartEvent):
        new_part = e.part
        if (
            isinstance(new_part, TextPart) and params.allow_text_output
        ):  # pragma: no branch

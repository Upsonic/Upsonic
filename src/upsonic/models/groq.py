...
class GroqModel(Model):
    ...
    async def request(
        ...
        try:
            response = await self._completions_create(
                messages,
                False,
                cast(GroqModelSettings, model_settings or {}),
    ...
    async def _process_streamed_response(
        self,
        response: AsyncStream[chat.ChatCompletionChunk],
        model_request_parameters: ModelRequestParameters,
    ...
    def _get_builtin_tools(
        ...
        for tool in model_request_parameters.builtin_tools:
            if isinstance(tool, WebSearchTool):
                if not GroqModelProfile.from_profile(
                    self.profile
                ).groq_always_has_web_search_builtin_tool:
                    raise UserError(
                        "`WebSearchTool` is not supported by Groq"
                    )  # pragma: no cover
            else:
                raise UserError(
        ...
        if isinstance(part.content, str):
            ...
            for item in part.content:
                if isinstance(item, str):
                    content.append(
                        chat.ChatCompletionContentPartTextParam(text=item, type="text")
                    )
...
def _map_executed_tool(
    ...
    if tool.type == "search":
        if tool.search_results and (
            tool.search_results.images or tool.search_results.results
        ):

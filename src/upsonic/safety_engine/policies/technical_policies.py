    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            if "custom_patterns" in options:
                self.api_key_patterns.extend(
                    options["custom_patterns"].get("api_key", [])
                )
                self.password_patterns.extend(
                    options["custom_patterns"].get("password", [])
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.certificate_patterns:
            ...
            triggered_items.extend(
                [f"CERTIFICATE:{cert[:50]}..." for cert in cert_matches]
            )
        ...
        for pattern in self.sensitive_technical_patterns:
            ...
            triggered_items.extend(
                [f"SENSITIVE_TECHNICAL:{sensitive}" for sensitive in sensitive_matches]
            )
        ...
        high_risk_count = len(
            [
                item
                for item in triggered_items
                if any(x in item for x in ["TOKEN:", "DATABASE:", "CLOUD:"])
            ]
        )
        medium_risk_count = len([item for item in triggered_items if "CONFIG:" in item])
        low_risk_count = len(
            [item for item in triggered_items if "TECHNICAL_KEYWORD:" in item]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "TECHNICAL_SECURITY_INFORMATION", policy_input
            )
...
class TechnicalSecurityBlockAction_LLM(ActionBase):
    ...
    def action(self, rule_result: RuleOutput) -> PolicyOutput:
        ...
        reason = (
            f"Content contains technical security information: {rule_result.details}"
        )
...
class TechnicalSecurityAnonymizeAction(ActionBase):
    ...
    description = (
        "Anonymizes technical security information while preserving content structure"
    )
    language = "en"

...
class CybersecurityRule(RuleBase):
    ...
    description = (
        "Detects malware, malicious code, cyber attacks, and security vulnerabilities"
    )
    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            ...
            if "custom_vulnerability_patterns" in options:
                self.vulnerability_patterns.extend(
                    options["custom_vulnerability_patterns"]
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.malware_patterns:
            ...
            triggered_items.extend(
                [f"MALWARE:{malware}" for malware in malware_matches]
            )
        ...
        for pattern in self.network_threat_patterns:
            ...
            triggered_items.extend(
                [f"NETWORK_THREAT:{network}" for network in network_matches]
            )
        ...
        for pattern in self.social_engineering_patterns:
            ...
            triggered_items.extend(
                [f"SOCIAL_ENGINEERING:{social}" for social in social_matches]
            )
        ...
        for pattern in self.crypto_threat_patterns:
            ...
            triggered_items.extend(
                [f"CRYPTO_THREAT:{crypto}" for crypto in crypto_matches]
            )
        ...
        for pattern in self.compromise_patterns:
            ...
            triggered_items.extend(
                [f"COMPROMISE:{compromise}" for compromise in compromise_matches]
            )
        ...
        critical_count = len(
            [
                ...
                if any(x in item for x in ["MALWARE:", "COMPROMISE:", "VULNERABILITY:"])
            ]
        )
        high_risk_count = len(
            [
        ...
        medium_risk_count = len(
            [item for item in triggered_items if "SUSPICIOUS_FILE:" in item]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "CYBERSECURITY_THREAT", policy_input
            )
...
class CybersecurityBlockAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate block messages for cybersecurity threats"
    )
    language = "en"
...
class CybersecurityRaiseExceptionAction(ActionBase):
    ...
    description = (
        "Raises DisallowedOperation exception for cybersecurity threat content"
    )
    language = "en"
...
class CybersecurityRaiseExceptionAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate exception messages for cybersecurity threats"
    )
    language = "en"

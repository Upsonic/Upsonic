    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            if "custom_patterns" in options:
                ...
                self.credit_card_patterns.extend(
                    options["custom_patterns"].get("credit_card", [])
                )
                self.address_patterns.extend(
                    options["custom_patterns"].get("address", [])
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.passport_patterns:
            ...
            triggered_items.extend(
                [f"PASSPORT:{passport}" for passport in passport_matches]
            )
        ...
        if not triggered_items:
            return RuleOutput(
                confidence=0.0, content_type="NO_PII", details="No PII detected"
            )
        ...
        high_risk_count = len(
            [
                ...
                if any(x in item for x in ["SSN:", "CREDIT_CARD:", "PASSPORT:"])
            ]
        )
        medium_risk_count = len(
            [
                item
                for item in triggered_items
                if any(x in item for x in ["EMAIL:", "PHONE:", "ADDRESS:", "DOB:"])
            ]
        )
        low_risk_count = len(
            [item for item in triggered_items if "PII_KEYWORD:" in item]
        )

        # Calculate confidence with weighted scoring
        confidence = min(
            1.0,
            (high_risk_count * 0.9 + medium_risk_count * 0.6 + low_risk_count * 0.3),
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "PERSONAL_IDENTIFIABLE_INFORMATION", policy_input
            )
...
class PIIBlockAction_LLM(ActionBase):
    ...
    def action(self, rule_result: RuleOutput) -> PolicyOutput:
        ...
        reason = (
            f"Content contains personal identifiable information: {rule_result.details}"
        )
...
class PIIAnonymizeAction(ActionBase):
    ...
    def _anonymize_pii_with_specific_format(self) -> PolicyOutput:
        ...
        translated_message = self._translate(
            "PII anonymized with specific format", self.detected_language
        )

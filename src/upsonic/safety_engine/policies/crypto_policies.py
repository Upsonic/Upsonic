...
class CryptoBlockAction_LLM(ActionBase):
    ...
    description = (
        "Handles crypto content with blocking, flagging or transformation using LLM"
    )
    language = "en"  # Default language for this action
    ...
    def action(self, rule_result: RuleOutput) -> PolicyOutput:
        ...
        if rule_result.confidence >= 0.8:
            return self.llm_raise_block_error(
                reason="Providing investment advice is not a legally permitted operation."
            )
        else:
            return self.allow_content()
...
class CryptoReplaceAction(ActionBase):
    ...
    def __init__(self):
        ...
        self.replace_message = (
            "Cryptocurrency related content detected and transformed."
        )
...
class CryptoRaiseExceptionAction(ActionBase):
    ...
    def __init__(self):
        ...
        self.exception_message = (
            "Cryptocurrency related content detected and operation stopped."
        )
...
class CryptoRaiseExceptionAction_LLM(ActionBase):
    ...
    def action(self, rule_result: RuleOutput) -> PolicyOutput:
        """Execute crypto-specific action based on rule confidence"""
        if rule_result.confidence >= 0.8:
            return self.llm_raise_exception(
                reason="Providing investment advice is not a legally permitted operation."
            )
        else:
            return self.allow_content()

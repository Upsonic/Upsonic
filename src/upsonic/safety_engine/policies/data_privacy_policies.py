    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.data_collection_patterns:
            ...
            triggered_items.extend(
                [f"DATA_COLLECTION:{collection}" for collection in collection_matches]
            )
        ...
        for pattern in self.consent_patterns:
            ...
            triggered_items.extend(
                [f"CONSENT_VIOLATION:{consent}" for consent in consent_matches]
            )
        ...
        for pattern in self.data_breach_patterns:
            ...
            triggered_items.extend(
                [f"DATA_BREACH:{breach}" for breach in breach_matches]
            )
        ...
        for pattern in self.data_transfer_patterns:
            ...
            triggered_items.extend(
                [f"DATA_TRANSFER:{transfer}" for transfer in transfer_matches]
            )
        ...
        for pattern in self.automated_decision_patterns:
            ...
            triggered_items.extend(
                [f"AUTOMATED_DECISION:{decision}" for decision in decision_matches]
            )
        ...
        for pattern in self.data_rights_patterns:
            ...
            triggered_items.extend(
                [f"DATA_RIGHTS:{rights}" for rights in rights_matches]
            )
        ...
        for pattern in self.transparency_patterns:
            ...
            triggered_items.extend(
                [
                    ...
                    for transparency in transparency_matches
                ]
            )
        ...
        for pattern in self.children_data_patterns:
            ...
            triggered_items.extend(
                [f"CHILDREN_DATA:{children}" for children in children_matches]
            )
        ...
        critical_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["DATA_BREACH:", "CONSENT_VIOLATION:", "CHILDREN_DATA:"]
                )
            ]
        )
        high_risk_count = len(
        ...
        medium_risk_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["DATA_RIGHTS:", "TRANSPARENCY:", "GDPR_KEYWORD:"]
                )
            ]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "DATA_PRIVACY_VIOLATION", policy_input
            )
...
class DataPrivacyBlockAction(ActionBase):
    ...
    description = (
        "Blocks content containing data privacy violations and GDPR compliance issues"
    )
    language = "en"
...
class DataPrivacyBlockAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate block messages for data privacy violations"
    )
    language = "en"
...
class DataPrivacyAnonymizeAction(ActionBase):
    ...
    description = (
        "Anonymizes data privacy violations while preserving content structure"
    )
    language = "en"
...
class DataPrivacyRaiseExceptionAction(ActionBase):
    ...
    description = (
        "Raises DisallowedOperation exception for data privacy violation content"
    )
    language = "en"

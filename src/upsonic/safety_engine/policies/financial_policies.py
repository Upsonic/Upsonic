    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            if "custom_patterns" in options:
                ...
                self.bank_account_patterns.extend(
                    options["custom_patterns"].get("bank_account", [])
                )
                self.crypto_patterns.extend(
                    options["custom_patterns"].get("crypto", [])
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.routing_patterns:
            ...
            triggered_items.extend(
                [f"ROUTING_NUMBER:{routing}" for routing in routing_matches]
            )
        ...
        for pattern in self.financial_statement_patterns:
            ...
            triggered_items.extend(
                [f"FINANCIAL_STATEMENT:{stmt}" for stmt in stmt_matches]
            )
        ...
        high_risk_count = len(
            [
                item
                for item in triggered_items
                if any(x in item for x in ["CVV:", "TAX_ID:", "CRYPTO:"])
            ]
        )
        medium_risk_count = len(
            [
                ...
                for item in triggered_items
                if any(
                    x in item for x in ["FINANCIAL_STATEMENT:", "INVESTMENT_ACCOUNT:"]
                )
            ]
        )
        low_risk_count = len(
            [item for item in triggered_items if "FINANCIAL_KEYWORD:" in item]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "FINANCIAL_INFORMATION", policy_input
            )
...
class FinancialInfoBlockAction_LLM(ActionBase):
    ...
    def action(self, rule_result: RuleOutput) -> PolicyOutput:
        ...
        reason = (
            f"Content contains sensitive financial information: {rule_result.details}"
        )
...
class FinancialInfoRaiseExceptionAction(ActionBase):
    ...
    description = (
        "Raises DisallowedOperation exception for financial information content"
    )
    language = "en"

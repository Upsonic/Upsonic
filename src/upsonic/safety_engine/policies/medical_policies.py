    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            if "custom_patterns" in options:
                self.medical_record_patterns.extend(
                    options["custom_patterns"].get("medical_record", [])
                )
                self.insurance_patterns.extend(
                    options["custom_patterns"].get("insurance", [])
                )
                self.prescription_patterns.extend(
                    options["custom_patterns"].get("prescription", [])
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.medical_record_patterns:
            ...
            triggered_items.extend(
                [f"MEDICAL_RECORD:{record}" for record in record_matches]
            )
        ...
        for pattern in self.insurance_patterns:
            ...
            triggered_items.extend(
                [f"INSURANCE:{insurance}" for insurance in insurance_matches]
            )
        ...
        for pattern in self.device_patterns:
            ...
            triggered_items.extend(
                [f"MEDICAL_DEVICE:{device}" for device in device_matches]
            )
        ...
        for pattern in self.condition_patterns:
            ...
            triggered_items.extend(
                [f"MEDICAL_CODE:{condition}" for condition in condition_matches]
            )
        ...
        for pattern in self.sensitive_medical_patterns:
            ...
            triggered_items.extend(
                [f"SENSITIVE_MEDICAL:{sensitive}" for sensitive in sensitive_matches]
            )
        ...
        critical_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["MEDICAL_RECORD:", "INSURANCE:", "SENSITIVE_MEDICAL:"]
                )
            ]
        )
        high_risk_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["PRESCRIPTION:", "LAB_RESULT:", "MEDICAL_DEVICE:"]
                )
            ]
        )
        medium_risk_count = len(
            [item for item in triggered_items if "MEDICAL_CODE:" in item]
        )
        low_risk_count = len(
            [item for item in triggered_items if "MEDICAL_KEYWORD:" in item]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "MEDICAL_INFORMATION", policy_input
            )

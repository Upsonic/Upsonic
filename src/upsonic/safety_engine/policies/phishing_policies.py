...
class PhishingRule(RuleBase):
    ...
    description = (
        "Detects phishing attacks, social engineering, and deceptive communications"
    )
        ...
        if options:
            ...
            if "custom_impersonation_patterns" in options:
                self.impersonation_patterns.extend(
                    options["custom_impersonation_patterns"]
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.urgent_action_patterns:
            ...
            triggered_items.extend(
                [f"URGENT_ACTION:{urgent}" for urgent in urgent_matches]
            )
        ...
        for pattern in self.credential_harvesting_patterns:
            ...
            triggered_items.extend(
                [
                    f"CREDENTIAL_HARVESTING:{credential}"
        ...
        for pattern in self.impersonation_patterns:
            ...
            triggered_items.extend(
                [
                    ...
                    for impersonation in impersonation_matches
                ]
            )
        ...
        for pattern in self.account_security_patterns:
            ...
            triggered_items.extend(
                [f"ACCOUNT_SECURITY:{security}" for security in security_matches]
            )
        ...
        for pattern in self.payment_financial_patterns:
            ...
            triggered_items.extend(
                [f"PAYMENT_FINANCIAL:{payment}" for payment in payment_matches]
            )
        ...
        for pattern in self.social_media_patterns:
            ...
            triggered_items.extend(
                [f"SOCIAL_MEDIA:{social}" for social in social_matches]
            )
        ...
        for pattern in self.technical_patterns:
            ...
            triggered_items.extend(
                [f"TECHNICAL:{technical}" for technical in technical_matches]
            )
        ...
        high_risk_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["URGENT_ACTION:", "SUSPICIOUS_LINK:", "ACCOUNT_SECURITY:"]
                )
            ]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "PHISHING_SOCIAL_ENGINEERING", policy_input
            )
...
class PhishingBlockAction(ActionBase):
    ...
    description = (
        "Blocks content containing phishing attacks and social engineering attempts"
    )
    language = "en"
...
class PhishingReplaceAction(ActionBase):
    ...
    description = (
        "Replaces phishing and social engineering indicators with safe placeholders"
    )
    language = "en"

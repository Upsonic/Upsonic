    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            if "custom_patterns" in options:
                self.legal_document_patterns.extend(
                    options["custom_patterns"].get("legal_document", [])
                )
                self.confidential_patterns.extend(
                    options["custom_patterns"].get("confidential", [])
                )
                self.trade_secret_patterns.extend(
                    options["custom_patterns"].get("trade_secret", [])
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.legal_document_patterns:
            ...
            triggered_items.extend(
                [f"LEGAL_DOCUMENT:{legal}" for legal in legal_matches]
            )
        ...
        for pattern in self.business_sensitive_patterns:
            ...
            triggered_items.extend(
                [f"BUSINESS_SENSITIVE:{business}" for business in business_matches]
            )
        ...
        for pattern in self.legal_proceeding_patterns:
            ...
            triggered_items.extend(
                [f"LEGAL_PROCEEDING:{proceeding}" for proceeding in proceeding_matches]
            )
        ...
        for pattern in self.sensitive_legal_patterns:
            ...
            triggered_items.extend(
                [f"SENSITIVE_LEGAL:{sensitive}" for sensitive in sensitive_matches]
            )
        ...
        critical_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["CONFIDENTIAL:", "TRADE_SECRET:", "SENSITIVE_LEGAL:"]
                )
            ]
        )
        high_risk_count = len(
        ...
        medium_risk_count = len(
            [item for item in triggered_items if "INTELLECTUAL_PROPERTY:" in item]
        )
        low_risk_count = len(
            [item for item in triggered_items if "LEGAL_KEYWORD:" in item]
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "LEGAL_INFORMATION", policy_input
            )
...
class LegalInfoBlockAction(ActionBase):
    ...
    description = (
        "Blocks content containing legal documents and confidential information"
    )
    language = "en"
...
class LegalInfoBlockAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate block messages for legal information content"
    )
    language = "en"

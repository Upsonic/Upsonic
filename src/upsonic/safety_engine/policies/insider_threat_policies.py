...
class InsiderThreatRule(RuleBase):
    ...
    description = (
        "Detects insider threats, data exfiltration, and internal security violations"
    )
    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            ...
            if "custom_exfiltration_patterns" in options:
                self.data_exfiltration_patterns.extend(
                    options["custom_exfiltration_patterns"]
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.data_exfiltration_patterns:
            ...
            triggered_items.extend(
                [
                    f"DATA_EXFILTRATION:{exfiltration}"
                    for exfiltration in exfiltration_matches
                ]
            )
        ...
        for pattern in self.unauthorized_access_patterns:
            ...
            triggered_items.extend(
                [f"UNAUTHORIZED_ACCESS:{access}" for access in access_matches]
            )
        ...
        for pattern in self.suspicious_behavior_patterns:
            ...
            triggered_items.extend(
                [f"SUSPICIOUS_BEHAVIOR:{behavior}" for behavior in behavior_matches]
            )
        ...
        for pattern in self.disgruntled_employee_patterns:
            ...
            triggered_items.extend(
                [
                    f"DISGRUNTLED_EMPLOYEE:{disgruntled}"
                    for disgruntled in disgruntled_matches
                ]
            )
        ...
        for pattern in self.sabotage_patterns:
            ...
            triggered_items.extend(
                [f"SABOTAGE:{sabotage}" for sabotage in sabotage_matches]
            )
        ...
        for pattern in self.data_hoarding_patterns:
            ...
            triggered_items.extend(
                [f"DATA_HOARDING:{hoarding}" for hoarding in hoarding_matches]
            )
        ...
        for pattern in self.external_communication_patterns:
            ...
            triggered_items.extend(
                [
                    f"EXTERNAL_COMMUNICATION:{communication}"
                    for communication in communication_matches
                ]
            )
        ...
        critical_count = len(
            [
                ...
                if any(
                    x in item for x in ["DATA_EXFILTRATION:", "IP_THEFT:", "SABOTAGE:"]
                )
            ]
        )
        high_risk_count = len(
        ...
        confidence = min(
            1.0,
            (critical_count * 1.0 + high_risk_count * 0.8 + medium_risk_count * 0.6),
        )
...
class InsiderThreatRule_LLM_Finder(RuleBase):
    ...
    description = (
        "Uses LLM to detect insider threats with context awareness and better accuracy"
    )
    language = "en"
...
class InsiderThreatBlockAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate block messages for insider threat content"
    )
    language = "en"
...
class InsiderThreatAnonymizeAction(ActionBase):
    ...
    description = (
        "Anonymizes insider threat indicators while preserving content structure"
    )
    language = "en"
...
class InsiderThreatRaiseExceptionAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate exception messages for insider threat content"
    )
    language = "en"

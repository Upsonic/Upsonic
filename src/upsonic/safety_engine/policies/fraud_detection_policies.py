    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for pattern in self.identity_theft_patterns:
            ...
            triggered_items.extend(
                [f"IDENTITY_THEFT:{identity}" for identity in identity_matches]
            )
        ...
        for pattern in self.romance_scam_patterns:
            ...
            triggered_items.extend(
                [f"ROMANCE_SCAM:{romance}" for romance in romance_matches]
            )
        ...
        for pattern in self.investment_scam_patterns:
            ...
            triggered_items.extend(
                [f"INVESTMENT_SCAM:{investment}" for investment in investment_matches]
            )
        ...
        for pattern in self.tech_support_scam_patterns:
            ...
            triggered_items.extend(
                [f"TECH_SUPPORT_SCAM:{tech}" for tech in tech_matches]
            )
        ...
        for pattern in self.charity_scam_patterns:
            ...
            triggered_items.extend(
                [f"CHARITY_SCAM:{charity}" for charity in charity_matches]
            )
        ...
        for pattern in self.urgency_patterns:
            ...
            triggered_items.extend(
                [f"URGENCY_TACTIC:{urgency}" for urgency in urgency_matches]
            )
        ...
        for pattern in self.payment_patterns:
            ...
            triggered_items.extend(
                [f"SUSPICIOUS_PAYMENT:{payment}" for payment in payment_matches]
            )
        ...
        critical_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["FRAUD_KEYWORD:", "IDENTITY_THEFT:", "INVESTMENT_SCAM:"]
                )
            ]
        )
        high_risk_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["FINANCIAL_SCAM:", "ROMANCE_SCAM:", "TECH_SUPPORT_SCAM:"]
                )
            ]
        )
        medium_risk_count = len(
            [
                ...
                if any(
                    x in item
                    for x in ["CHARITY_SCAM:", "URGENCY_TACTIC:", "SUSPICIOUS_PAYMENT:"]
                )
            ]
        )
...
class FraudDetectionRule_LLM_Finder(RuleBase):
    ...
    description = (
        "Uses LLM to detect fraud and scams with context awareness and better accuracy"
    )
    language = "en"
...
class FraudDetectionBlockAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate block messages for fraud and scam content"
    )
    language = "en"
...
class FraudDetectionAnonymizeAction(ActionBase):
    ...
    description = (
        "Anonymizes fraud and scam indicators while preserving content structure"
    )
    language = "en"
...
class FraudDetectionRaiseExceptionAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate exception messages for fraud and scam content"
    )
    language = "en"

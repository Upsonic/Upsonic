...
class SensitiveSocialRule(RuleBase):
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        for keyword in self.hate_speech_keywords:
            ...
            pattern = (
                r"\b"
                + re.escape(keyword.lower()).replace(r"\*", r"[a-z*]")
                + r"(?:s|es|ies|ed|ing)?\b"
            )
        ...
        all_triggered = (
            triggered_hate_speech + triggered_patterns + triggered_discriminatory
        )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        try:
            ...
            triggered_keywords = self._llm_find_keywords_with_input(
                "SENSITIVE_SOCIAL_CONTENT", policy_input
            )
...
class SensitiveSocialBlockAction(ActionBase):
    ...
    description = (
        "Blocks content containing sensitive social issues with educational message"
    )
    language = "en"
...
class SensitiveSocialBlockAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate block messages for sensitive social content"
    )
    language = "en"

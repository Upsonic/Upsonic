...
class AdultContentRule(RuleBase):
    ...
    description = (
        "Detects explicit sexual content, adult themes, and inappropriate material"
    )
    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None):
        ...
        if options:
            ...
            if "age_verification_keywords" in options:
                self.age_verification_keywords.extend(
                    options["age_verification_keywords"]
                )
    ...
    def process(self, policy_input: PolicyInput) -> RuleOutput:
        ...
        explicit_weight = (
            len(triggered_explicit) * 0.9
        )  # High weight for explicit content
        pattern_weight = (
            len(triggered_patterns) * 0.85
        )  # High weight for adult patterns
        suggestive_weight = (
            len(triggered_suggestive) * 0.4
        )  # Moderate weight for suggestive content
        age_verification_weight = (
            len(triggered_age_verification) * 0.7
...
class AdultContentRule_LLM_Finder(RuleBase):
    ...
    description = (
        "Uses LLM to detect adult content and explicit material with context awareness"
    )
    language = "en"
...
class AdultContentRaiseExceptionAction_LLM(ActionBase):
    ...
    description = (
        "Uses LLM to generate appropriate exception messages for adult content"
    )
    language = "en"

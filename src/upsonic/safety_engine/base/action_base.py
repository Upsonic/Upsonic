...
class ActionBase(ABC):
    ...
    async def execute_action_async(
        ...
        if language and language != "auto":
            ...
            self.detected_language = await self._detect_content_language_async(
                original_content
            )
        else:
    ...
    def _detect_content_language(self, content: List[str]) -> str:
        ...
        if isinstance(self.language_identify_llm, UpsonicLLMProvider):
            ...
            llm = UpsonicLLMProvider(
                agent_name="Language Detection Agent", model=self.language_identify_llm
            )
        try:
    ...
    def _translate(self, text: str, target_language: str) -> str:
        ...
        if self.__class__.language != target_language:
            ...
            if isinstance(self.base_llm, UpsonicLLMProvider):
                ...
                llm = UpsonicLLMProvider(
                    agent_name="Translation Agent", model=self.base_llm
                )
    ...
    async def allow_content_async(self) -> PolicyOutput:
        return PolicyOutput(
            ...
            action_output={
                "action_taken": "ALLOW",
                "success": True,
                "message": await self._translate_async(
                    "Content allowed", self.detected_language
                ),
            },
        )
    ...
    async def raise_block_error_async(self, message: str) -> PolicyOutput:
        translated_message = await self._translate_async(
            message, self.detected_language
        )
        return PolicyOutput(
            output_texts=[translated_message],
            action_output={
    ...
    def replace_triggered_keywords(self, replacement: str) -> PolicyOutput:
        ...
        triggered_keywords = (
            self.rule_result.triggered_keywords if self.rule_result else []
        )
        ...
        translated_message = self._translate(
            f"Keywords replaced with: {replacement}", self.detected_language
        )
    ...
    def anonymize_triggered_keywords(self) -> PolicyOutput:
        ...
        translated_message = self._translate(
            "Keywords anonymized with unique replacements", self.detected_language
        )
    ...
    def llm_raise_block_error(self, reason: str) -> PolicyOutput:
        ...
        if isinstance(self.base_llm, UpsonicLLMProvider):
            ...
            llm = UpsonicLLMProvider(
                agent_name="Block Error Message Agent", model=self.base_llm
            )
        llm_message = llm.generate_block_message(
            reason, language=self.detected_language
        )
        return PolicyOutput(
    ...
    async def llm_raise_block_error_async(self, reason: str) -> PolicyOutput:
        ...
        llm_message = await llm.generate_block_message_async(
            reason, language=self.detected_language
        )
        return PolicyOutput(
            output_texts=[llm_message],
    ...
    def llm_raise_exception(self, reason: str) -> PolicyOutput:
        ...
        if isinstance(self.base_llm, UpsonicLLMProvider):
            ...
            llm = UpsonicLLMProvider(
                agent_name="Exception Message Agent", model=self.base_llm
            )

...
class RuleBase(ABC):
    ...
    def _llm_find_keywords_with_input(
        self, content_type: str, policy_input: PolicyInput
    ) -> List[str]:
        ...
        llm = UpsonicLLMProvider(
            agent_name="Text Finder Agent", model=self.text_finder_llm
        )
        ...
        return llm.find_keywords(
            content_type, combined_text, language=detected_language
        )
    ...
    async def _llm_find_keywords_with_input_async(
        self, content_type: str, policy_input: PolicyInput
    ) -> List[str]:
        ...
        try:
            ...
            detected_language = self.language
        return await llm.find_keywords_async(
            content_type, combined_text, language=detected_language
        )

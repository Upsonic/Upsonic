...
class Policy:
    ...
    def __init__(
        ...
        if language_identify_llm is None and language_identify_model:
            language_identify_llm = UpsonicLLMProvider(
                agent_name="Language Detection Agent", model=language_identify_model
            )
        if base_llm is None and base_model:
            base_llm = UpsonicLLMProvider(
                agent_name="Base Operations Agent", model=base_model
            )
        if text_finder_llm is None and text_finder_model:
            text_finder_llm = UpsonicLLMProvider(
                agent_name="Text Finder Agent", model=text_finder_model
            )
    ...
    def execute(
    ...
    ) -> tuple[RuleOutput, ActionOutput, PolicyOutput]:
        ...
        action_result = self.action.execute_action(
            rule_result,
            policy_input.input_texts or [],
            self.language,
            self.language_identify_llm,
            self.base_llm,
            self.text_finder_llm,
        )

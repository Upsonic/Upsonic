...
class PolicyResult:
    ...
    def __init__(self):
        self.action_taken: str = (
            "ALLOW"  # ALLOW, BLOCK, REPLACE, ANONYMIZE, DISALLOWED_EXCEPTION
        )
...
class PolicyManager:
    ...
    async def execute_policies_async(
        self, policy_input: PolicyInput, check_type: str = "Policy Check"
    ) -> PolicyResult:
        """
        ...
        for policy in self.policies:
            try:
                ...
                # Execute policy
                rule_output, action_output, policy_output = await policy.execute_async(
                    current_input
                )
                action_taken = policy_output.action_output.get(
                ...
                if action_taken == "BLOCK":
                    ...
                    result.final_output = (
                        policy_output.output_texts[0]
                        if policy_output.output_texts
                        else f"Content blocked by policy: {policy.name}"
                    )
                ...
                result.message = (
                    f"Operation disallowed by policy '{policy.name}': {str(e)}"
                )
                result.triggered_policies.append(policy.name)
                ...
                if self.debug:
                    ...
                    warning_log(
                        f"Policy '{policy.name}' execution failed: {str(e)}",
                        "PolicyManager",
                    )
                continue

        # Set final output if not blocked and transformations were applied
        if not result.should_block() and result.action_taken in [
            "REPLACE",
            "ANONYMIZE",
        ]:
        ...
        for policy in self.policies:
            if hasattr(policy, "base_llm") and policy.base_llm is None:
                ...
                policy.base_llm = UpsonicLLMProvider(
                    agent_name=f"Policy Base Agent ({policy.name})", model=model
                )

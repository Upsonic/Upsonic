...
class PipelineManager:
    ...
    def __init__(self, steps: Optional[List[Step]] = None, debug: bool = False):
        """
    ...
    async def execute(self, context: StepContext) -> StepContext:
        ...
        with sentry_sdk.start_transaction(
            ...
            transaction.set_tag(
                "pipeline.streaming",
                context.is_streaming if hasattr(context, "is_streaming") else False,
            )
            ...
            if hasattr(context, "task") and context.task:
                ...
                if hasattr(context.task, "description"):
                    transaction.set_data(
                        "task.description", str(context.task.description)[:200]
                    )
            ...
            _sentry_logger.info(
                ...
                extra={
                    "total_steps": len(self.steps),
                    "debug": self.debug,
                    "streaming": context.is_streaming
                    if hasattr(context, "is_streaming")
                    else False,
                },
            )
            ...
            try:
                for step_index, step in enumerate[Step](self.steps):
                    ...
                    with sentry_sdk.start_span(
                        op=f"pipeline.step.{step.name}", name=step.description
                    ) as span:
                        # Add step metadata
                ...
                total_time = sum(
                    r["execution_time"]
                    for r in self._execution_stats["step_results"].values()
                )
                ...
                transaction.set_data(
                    "pipeline.executed_steps", self._execution_stats["executed_steps"]
                )
                ...
                if self.debug:
                    ...
                    pipeline_timeline(self._execution_stats["step_results"], total_time)
                ...
                failed_step_index = getattr(
                    self,
                    "_current_step_index",
                    max(0, self._execution_stats["executed_steps"] - 1),
                )
                ...
                await self._save_durable_checkpoint(
                    context, failed_step, failed_step_index, status="failed"
                )
    ...
    async def execute_stream(self, context: StepContext) -> AsyncIterator[Any]:
        ...
        try:
            ...
            if self.debug:
                ...
                pipeline_completed(
                    self._execution_stats["executed_steps"], len(self.steps), total_time
                )
    ...
    async def _save_durable_checkpoint(
        self, context: StepContext, step: Step, step_index: int, status: str = "running"
    ) -> None:
        """
        ...
        if (
            not hasattr(context.task, "durable_execution")
            or not context.task.durable_execution
        ):
            return
        ...
        try:
            ...
            if self.debug:
                ...
                warning_log(
                    f"Failed to save durable checkpoint: {str(e)}", "PipelineManager"
                )

...
class InitializationStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Agent initialized", execution_time=0.0
        )
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        if context.agent.debug:
            ...
            cache_configuration(
                ...
                cache_threshold=context.task.cache_threshold
                if context.task.cache_method == "vector_search"
                else None,
        ...
        cached_response = await context.task.get_cached_response(
            input_text, context.model
        )

        if cached_response is not None:
            similarity = None
            if (
                hasattr(context.task, "_last_cache_entry")
                and "similarity" in context.task._last_cache_entry
            ):
            ...
            cache_hit(
                ...
                input_preview=(
                    context.task._original_input or context.task.description
                )[:100]
                if (context.task._original_input or context.task.description)
                else None,
            )
...
class UserPolicyStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        if (
            not context.agent.user_policy_manager.has_policies()
            or not context.task.description
            or context.task.is_paused
        ):
            return StepResult(
                status=StepStatus.SUCCESS,
        ...
        processed_task, should_continue = await context.agent._apply_user_policy(
            context.task
        )
...
class ValidationStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Validation passed", execution_time=0.0
        )
...
class ToolSetupStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Tools configured", execution_time=0.0
        )
...
class LLMManagerStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        llm_manager = LLMManager(
            default_model=context.agent.model, requested_model=context.model
        )
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        async with memory_manager.manage_memory() as memory_handler:
            messages = await context.agent._build_model_request(
                context.task, memory_handler, context.state
            )
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        try:
            if context.is_continuation and context.continuation_tool_results:
                ...
                tool_results_message = ModelRequest(
                    parts=context.continuation_tool_results
                )
            ...
            async with memory_manager.manage_memory() as memory_handler:
                if context.task.guardrail:
                    ...
                    model_params = context.agent._build_model_request_parameters(
                        context.task
                    )
                    model_params = context.model.customize_request_parameters(
                        model_params
                    )
            ...
            if not hasattr(e, "external_call") or e.external_call is None:
                raise RuntimeError(
                    "ExternalExecutionPause must have external_call attached by ToolManager"
                )
            ...
            context.task._continuation_state = {
                ...
                "tool_limit_reached": getattr(
                    context.agent, "_tool_limit_reached", False
                ),
                "current_messages": list(context.agent._current_messages)
                if context.agent._current_messages
                else [],
            }
...
class ResponseProcessingStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Response processed", execution_time=0.0
        )
...
class ReflectionStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        improved_output = (
            await context.agent.reflection_processor.process_with_reflection(
                context.agent, context.task, context.final_output
            )
        )
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Reflection applied", execution_time=0.0
        )
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        async with memory_manager.manage_memory() as memory_handler:
            ...
            if context.messages and new_messages_start < len(context.messages):
                context.agent._run_result.add_messages(
                    context.messages[new_messages_start:]
                )
            if context.response:
...
class ReliabilityStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        reliability_manager = ReliabilityManager(
            context.task, context.agent.reliability_layer, context.model
        )
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Reliability applied", execution_time=0.0
        )
...
class AgentPolicyStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        if (
            not context.agent.agent_policy_manager.has_policies()
            or not context.task.response
        ):
            return StepResult(
                status=StepStatus.SUCCESS,
...
class CacheStorageStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Response cached", execution_time=0.0
        )
...
class StreamModelExecutionStep(Step):
    ...
    async def execute_stream(self, context: StepContext) -> AsyncIterator[Any]:
        ...
        try:
            ...
            async for event in self._stream_with_tool_calls(
                context, model_params, accumulated_text, first_token_time
            ):
                yield event
    ...
    async def _stream_with_tool_calls(
        ...
        if (
            hasattr(context.agent, "_tool_limit_reached")
            and context.agent._tool_limit_reached
        ):
            yield FinalResultEvent(tool_name=None, tool_call_id=None)
            return
        ...
        tool_calls = [
            part for part in final_response.parts if isinstance(part, ToolCallPart)
        ]
...
class StreamMemoryMessageTrackingStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        async with memory_manager.manage_memory() as memory_handler:
            ...
            if context.messages and new_messages_start < len(context.messages):
                if context.stream_result:
                    context.stream_result.add_messages(
                        context.messages[new_messages_start:]
                    )
...
class StreamFinalizationStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Streaming finalized", execution_time=0.0
        )
...
class FinalizationStep(Step):
    ...
    async def execute(self, context: StepContext) -> StepResult:
        ...
        return StepResult(
            status=StepStatus.SUCCESS, message="Execution finalized", execution_time=0.0
        )

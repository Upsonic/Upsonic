...
class DeepAgent(Agent):
    ...
    def _setup_tools(self, task: "Task") -> None:
        ...
        deep_tools = [write_todos, ls, read_file, write_file, edit_file]
    ...
    async def _execute_subagent(self, description: str, subagent_type: str) -> str:
        """
    ...
    def _get_incomplete_todos_summary(self) -> str:
        ...
        todos = self.deep_agent_state.todos
        incomplete = [
            todo for todo in todos if todo.status in ["pending", "in_progress"]
        ]
    ...
    async def do_async(
        ...
        *,
        ...
        result = await super().do_async(
            task, model, debug, retry, state, graph_execution_id=graph_execution_id
        )
        ...
        while iteration < MAX_COMPLETION_ITERATIONS:
            ...
            if debug or self.debug:
                deep_agent_todo_completion_check(
                    iteration, completed_count, total_count
                )
        ...
        if not all_completed and total_count > 0:
            ...
            if debug or self.debug:
                deep_agent_max_iterations_warning(
                    MAX_COMPLETION_ITERATIONS, incomplete_count
                )

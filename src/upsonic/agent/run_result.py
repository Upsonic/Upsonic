...
class StreamRunResult(Generic[OutputDataT]):
    ...
    async def stream_output(self) -> AsyncIterator[str]:
        ...
        if not self._context_entered:
            raise RuntimeError(
                "StreamRunResult.stream_output() must be called within async context manager"
            )
    ...
    async def stream_events(self) -> AsyncIterator[ModelResponseStreamEvent]:
        ...
        if not self._context_entered:
            raise RuntimeError(
                "StreamRunResult.stream_events() must be called within async context manager"
            )
    ...
    def get_text_events(self) -> List[ModelResponseStreamEvent]:
        ...
        return [
            ...
            and hasattr(event.delta, "content_delta")
        ]
    ...
    def get_streaming_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "event_types": {
                type(event).__name__: sum(
                    1 for e in self._streaming_events if type(e) == type(event)
                )
                for event in self._streaming_events
            },
        }
    ...
    def get_performance_metrics(self) -> Dict[str, Optional[float]]:
        ...
        if metrics["total_duration"] and metrics["total_duration"] > 0:
            ...
            metrics["characters_per_second"] = (
                len(self._accumulated_text) / metrics["total_duration"]
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"StreamRunResult(status={status}, "

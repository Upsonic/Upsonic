...
class Agent(BaseAgent):
        ...
        *,
        ...
        if self.compression_strategy == "llmlingua":
            ...
            self._prompt_compressor = PromptCompressor(
                model_name=model_name, use_llmlingua2=True
            )
        ...
        if mode not in ("raise", "return_false"):
            raise ValueError(
                f"Invalid retry_mode '{mode}'. Must be 'raise' or 'return_false'."
            )
    ...
    def _get_model_specific_reasoning_settings(self) -> Dict[str, Any]:
        ...
        # Google models
            ...
            if (
                ...
                or self.thinking_include_thoughts is not None
            ):
                thinking_config = {}
                if self.thinking_enabled is not None:
                    thinking_config["include_thoughts"] = (
                        self.thinking_include_thoughts
                        if self.thinking_include_thoughts is not None
    ...
    def _setup_tools(self, task: "Task") -> None:
        ...
        if is_reasoning_enabled and not is_thinking_enabled:
            raise ValueError(
                "Configuration error: 'enable_reasoning_tool' cannot be True if 'enable_thinking_tool' is False."
            )
        ...
        self._builtin_tools = self.tool_manager.processor.extract_builtin_tools(
            final_tools
        )
    ...
    async def _build_model_request(
        ...
        async with (
            system_prompt_manager.manage_system_prompt(memory_handler) as sp_handler,
    ...
    def _build_model_request_parameters(self, task: "Task") -> "ModelRequestParameters":
        ...
        if (
            ...
            and task.response_format != str
            and task.response_format is not str
        ):
            if isinstance(task.response_format, type) and issubclass(
                task.response_format, BaseModel
            ):
                output_mode = "native"
        ...
        for tool_call in sequential_calls:
            try:
                ...
                if hasattr(self, "current_task") and self.current_task:
                    task_id = getattr(self.current_task, "id", None) or getattr(
                        self.current_task, "price_id", None
                    )
    ...
    async def _handle_model_response(
        self, response: "ModelResponse", messages: List["ModelRequest"]
    ) -> "ModelResponse":
        ...
        if tool_calls:
            ...
            if hasattr(self, "_tool_limit_reached") and self._tool_limit_reached:
                ...
                model_params = self._build_model_request_parameters(
                ...
                for tool_result in tool_results:
                    if hasattr(tool_result, "content"):
                        if isinstance(tool_result.content, dict):
                            final_text = str(
                                tool_result.content.get("func", tool_result.content)
                            )
                        else:
    ...
    async def _handle_cache(self, task: "Task") -> Optional[Any]:
        ...
        if self.debug:
            ...
            cache_configuration(
                ...
                if task.cache_method == "vector_search"
                else None,
        ...
        if cached_response is not None:
            similarity = None
            if (
                hasattr(task, "_last_cache_entry")
                and "similarity" in task._last_cache_entry
            ):
            ...
            cache_hit(
                ...
                if (task._original_input or task.description)
                else None,
            )
    ...
    async def _apply_user_policy(self, task: "Task") -> tuple[Optional["Task"], bool]:
        ...
        result = await self.user_policy_manager.execute_policies_async(
            policy_input, check_type="User Input Check"
        )
    ...
    async def _execute_with_guardrail(
        self,
        task: "Task",
        memory_handler: Optional["MemoryManager"],
        state: Optional["State"] = None,
        ...
        while not validation_passed and retry_counter < max_retries:
            messages = await self._build_model_request_with_input(
                task, memory_handler, current_input, temporary_message_history, state
            )
            ...
            current_model_response = await self._handle_model_response(
                response, messages
            )
            ...
            try:
                ...
                if isinstance(guardrail_result, tuple) and len(guardrail_result) == 2:
                    ...
                    result = (
                        final_text_output
                        if guardrail_result
                        else "Guardrail validation failed"
                    )
                else:
        ...
        if not validation_passed:
            ...
            if self.mode == "raise":
                ...
                error_response = ModelResponse(
                    parts=[
                        TextPart(content="Guardrail validation failed after retries")
                    ],
    ...
    def _compress_llmlingua(self, context: str) -> str:
        ...
        try:
            result = self._prompt_compressor.compress_prompt(
                context.split("\n"), instruction=instruction, ratio=ratio
            )
    ...
    async def recommend_model_for_task_async(
        ...
        try:
            ...
            use_llm_selection = (
                use_llm if use_llm is not None else self.use_llm_for_selection
            )
    ...
    async def _apply_agent_policy(self, task: "Task") -> "Task":
        ...
        """
        if (
            not self.agent_policy_manager.has_policies()
            or not task
        ...
        if response_text:
            ...
            result = await self.agent_policy_manager.execute_policies_async(
                agent_policy_input, check_type="Agent Output Check"
            )
            ...
            if result.should_block():
                ...
                task._response = (
                    result.final_output or "Response modified by agent policy."
                )
    ...
    async def do_async(
        ...
        *,
        ...
        async with self._managed_storage_connection():
            ...
            context = StepContext(task=task, agent=self, model=model, state=state)
    ...
    def do(
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run, self.do_async(task, model, debug, retry)
                )
                return future.result()
        except RuntimeError:
    ...
    async def _stream_text_output(
        ...
        async for event in self._create_stream_iterator(
            task, model, debug, retry, state, graph_execution_id
        ):
    ...
    def _extract_text_from_stream_event(
        ...
        elif isinstance(event, PartDeltaEvent) and hasattr(
    ...
    def print_stream(
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, self.print_stream_async(task, model, debug, retry)
                    )
                    return future.result()
            else:
                return loop.run_until_complete(
                    self.print_stream_async(task, model, debug, retry)
                )
        except RuntimeError:
    ...
    def continue_run(
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, self.continue_async(task, model, debug, retry)
                    )
                    return future.result()
            else:
                return loop.run_until_complete(
                    self.continue_async(task, model, debug, retry)
                )
        except RuntimeError:
    ...
    async def continue_async(
        ...
        *,
        ...
        if not task.is_paused or not task.tools_awaiting_external_execution:
            raise ValueError(
                "The 'continue_async' method can only be called on a task that is currently paused for external execution."
            )
        ...
        if hasattr(task, "_continuation_state") and task._continuation_state:
            ...
            response_with_tool_calls = task._continuation_state.get(
    ...
    async def continue_durable_async(
        ...
        if storage is None:
            raise ValueError(
                "Storage backend is required to continue durable execution"
            )
        ...
        if checkpoint is None:
            raise ValueError(
                f"No checkpoint found for execution: {durable_execution_id}"
            )
        ...
        info_log(
            f"🔄 RESUMING from checkpoint: {durable_execution_id}", "DurableRecovery"
        )

        if self.debug or debug:
            if checkpoint_status == "failed":
                info_log(
                    f"📍 Failed at step: {step_index} ({step_name})", "DurableRecovery"
                )
                info_log(
                    f"🔄 Will RETRY step: {step_index} ({step_name})", "DurableRecovery"
                )
            else:
                info_log(
                ...
                info_log(
                    f"⏭️  Will resume from step: {step_index + 1}", "DurableRecovery"
                )
        ...
        context = DurableStateSerializer.reconstruct_context(
            context_data, task=task, agent=self, model=current_model
        )
        ...
        if self.debug or debug:
            info_log(f"📋 Total pipeline steps: {len(all_steps)}", "DurableRecovery")
            info_log(
        ...
        if self.debug or debug:
            if remaining_steps:
                ...
                info_log(
                    f"🎯 Steps to execute: {', '.join(step_names)}", "DurableRecovery"
                )
        ...
        async with self._managed_storage_connection():
            pipeline = PipelineManager(steps=remaining_steps, debug=debug or self.debug)
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run,
                        self.continue_durable_async(
                            durable_execution_id, storage, model, debug, retry
                        ),
                    )
                    return future.result()
            else:

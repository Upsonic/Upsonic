...
class Direct(BaseAgent):
    ...
    def __init__(
        ...
        if mode not in ("raise", "return_false"):
            raise ValueError(
                f"Invalid retry_mode '{retry_mode}'. Must be 'raise' or 'return_false'."
            )
    ...
    def _limit_message_history(self, message_history: list) -> list:
        ...
        """
        if (
            not self.num_history_runs
            or self.num_history_runs <= 0
    ...
    async def agent_create(self, llm_model, single_task, system_prompt: str):
        ...
        processed_tools_generator = tool_processor.normalize_and_process(
            single_task.tools
        )

        for original_tool, config in processed_tools_generator:
            wrapped_tool = tool_processor.generate_behavioral_wrapper(
                original_tool, config
            )
            ...
            if isinstance(tool, type):
                ...
                if hasattr(tool, "url"):
                    ...
                    env = (
                        getattr(tool, "env", {})
                        if hasattr(tool, "env")
                        and isinstance(getattr(tool, "env", None), dict)
                        else {}
                    )
    ...
    async def _managed_storage_connection(self):
        ...
        can_connect = hasattr(self.storage, "connect") and callable(
            self.storage.connect
        )
        can_disconnect = hasattr(self.storage, "disconnect") and callable(
        ...
        try:
            ...
            if (
                can_disconnect
                and not was_connected_before
                and self.storage.is_connected()
            ):
                self.storage.disconnect()
        ...
        async with self._managed_storage_connection():
            ...
            try:
                ...
                async with llm_manager.manage_llm() as llm_handler:
                    ...
                    async with (
                        system_prompt_manager.manage_system_prompt() as sp_handler,
                        ...
                        reliability_manager = ReliabilityManager(
                            task, self.reliability_layer, selected_model
                        )

                        agent = await self.agent_create(
                            selected_model, task, sp_handler.get_system_prompt()
                        )

                        async with (
                            reliability_manager.manage_reliability() as reliability_handler
                        ):
                            async with call_manager.manage_call() as call_handler:
                                async with task_manager.manage_task() as task_handler:
                                    ...
                                    model_response = call_handler.process_response(
                                        model_response
                                    )
                                    model_response = task_handler.process_response(
                                        model_response
                                    )
                                    model_response = storage_handler.process_response(
                                        model_response
                                    )
                                    processed_task = (
                                        await reliability_handler.process_task(
                                            task_handler.task
                                        )
                                    )

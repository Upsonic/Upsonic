...
class Direct(BaseAgent):
    ...
    def __init__(
        ...
        try:
            from upsonic.models.model_pricing import initialize_agent_pricing

            initialize_agent_pricing(enable_auto_refresh=True)
        except Exception as e:
            if debug:
                print(f"Warning: Pricing initialization failed: {e}")
                ...
                async with context_manager.manage_context() as ctx_handler:
                    async with (
                        ...
                        async with memory_manager.manage_memory() as memory_handler:
                            async with call_manager.manage_call(
                                memory_handler
                            ) as call_handler:
                                async with task_manager.manage_task() as task_handler:
                                    async with agent.run_mcp_servers():
                                        model_response = await agent.run(
                                            ...
                                            message_history=memory_handler.historical_messages,
                                        )

                                    # Save the response to all contexts
                                    model_response = memory_handler.process_response(
                                        model_response
                                    )

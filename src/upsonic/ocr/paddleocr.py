...
class BasePaddleOCRProvider(OCRProvider):
    ...
    def __init__(
        self, config: Optional[Union[OCRConfig, PaddleOCRConfig]] = None, **kwargs
    ):
    ...
    def _process_image(self, image, **kwargs) -> OCRResult:
        ...
        try:
            ...
            try:
                ...
                text, blocks, confidence = self._extract_paddle_predict_result(
                    paddle_result
                )
    ...
    def _extract_paddle_predict_result(
        self, paddle_result: Any
    ) -> tuple[str, List[OCRTextBlock], float]:
        ...
        try:
                        ...
                        if i < len(dt_polys) and dt_polys[i] is not None:
                            try:
                                ...
                                if isinstance(poly, (list, tuple)) and len(poly) > 0:
                                    ...
                                    x_coords = [
                                        float(p[0]) for p in poly if len(p) >= 2
                                    ]
                                    y_coords = [
                                        float(p[1]) for p in poly if len(p) >= 2
                                    ]
            ...
            try:
                ...
                if text_str.strip():
                    text_parts = [text_str]
                    blocks = [
                        OCRTextBlock(text=text_str, confidence=1.0, page_number=1)
                    ]
                    count = 1
...
class PaddleOCRProvider(BasePaddleOCRProvider):
            ...
            if is_pdf(path):
                ...
                for page_num, image in enumerate(images, 1):
                    ...
                    try:
                        ...
                        paddle_result = self._paddle_instance.predict(
                            tmp_path, **kwargs
                        )
    ...
    def __init__(
        ...
        doc_unwarping_model_dir: Optional[str] = None,
        text_detection_model_name: Optional[str] = None,
        text_detection_model_dir: Optional[
            str
        ] = None,  # Path to custom text detection model directory
        textline_orientation_model_name: Optional[str] = None,
        textline_orientation_model_dir: Optional[str] = None,
        text_recognition_model_name: Optional[str] = None,
        text_recognition_model_dir: Optional[
            str
        ] = None,  # Path to custom text recognition model directory
        # Batch sizes
        textline_orientation_batch_size: Optional[int] = None,
        text_recognition_batch_size: Optional[int] = None,
        # Feature toggles
    ...
    def _initialize_paddle(self) -> None:
        ...
        try:
            ...
            # Build parameters
            paddle_params = self._build_paddle_params(
                self.config, **self._paddle_kwargs
            )
...
class PPStructureV3Provider(BasePaddleOCRProvider):
    ...
    def _initialize_paddle(self) -> None:
        ...
        try:
            ...
            ocr_loading(
                "PP-StructureV3", [getattr(self.config, "lang", "en")], extra_info
            )
...
class PPChatOCRv4Provider(BasePaddleOCRProvider):
    ...
    def _initialize_paddle(self) -> None:
        ...
        try:
            ...
            ocr_loading(
                "PP-ChatOCRv4", [getattr(self.config, "lang", "en")], extra_info
            )
    ...
    def visual_predict(
        ...
        **kwargs,
        ...
        params = {
            k: v
            for k, v in locals().items()
            if k not in ["self", "input", "kwargs"] and v is not None
        }
        params.update(kwargs)

        return self._paddle_instance.visual_predict(str(input), **params)

    def visual_predict_iter(self, input: Union[str, Path], **kwargs):
    ...
    def chat(
        ...
        params = {
            k: v for k, v in locals().items() if k not in ["self"] and v is not None
        }
    ...
    def load_vector(
        ...
        return self._paddle_instance.load_vector(
            data_path=str(data_path), retriever_config=retriever_config
        )

    def save_visual_info_list(
        self, visual_info: List[Dict[str, Any]], save_path: Union[str, Path]
    ) -> None:
        ...
        return self._paddle_instance.save_visual_info_list(
            visual_info=visual_info, save_path=str(save_path)
        )
...
class PaddleOCRVLProvider(BasePaddleOCRProvider):
    ...
    def _initialize_paddle(self) -> None:
        ...
        try:
            ...
            ocr_loading(
                "PaddleOCR-VL", [getattr(self.config, "lang", "en")], extra_info
            )

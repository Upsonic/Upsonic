...
class RapidOCR(OCRProvider):
    ...
    def _get_engine(self):
        ...
        if self._engine is None:
            ...
            extra_info = {"Note": "Primarily supports Chinese and English"}
    ...
    def _process_image(self, image, **kwargs) -> OCRResult:
        ...
        try:
            ...
            if result is None or not result[0]:
                # No text detected
                return OCRResult(
                    text="", blocks=[], confidence=0.0, page_count=1, provider=self.name
                )
            ...
            for item in dt_boxes:
                ...
                block = OCRTextBlock(
                    text=text, confidence=float(confidence), bbox=bbox, language=None
                )

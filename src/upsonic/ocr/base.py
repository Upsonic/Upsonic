...
class OCRConfig(BaseModel):
    ...
    confidence_threshold: float = Field(
        default=0.0, description="Minimum confidence threshold"
    )
    rotation_fix: bool = Field(
        default=False, description="Enable automatic rotation correction"
    )
    enhance_contrast: bool = Field(
        default=False, description="Enhance image contrast before OCR"
    )
    ...
    preserve_formatting: bool = Field(
        default=True, description="Try to preserve text formatting"
    )
    ...
    def process_file(self, file_path: Union[str, Path], **kwargs) -> OCRResult:
        ...
        if all_blocks:
            self._metrics.average_confidence = sum(
                b.confidence for b in all_blocks
            ) / len(all_blocks)
